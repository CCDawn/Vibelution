...
def _knowledge_steward_metadata() -> dict[str, Any]:
    return {
        ...
        "personaProfile": {
            ...
            "collaborationPreference": "通过 source_collection_stage_writeback_tool 和知识治理门禁提交审核结论，不绕过正式审核。",
        ...
        "taskProfile": {
            ...
            "constraints": "阶段私聊任务先用 source_collection_context_tool 读取资料上下文，完成、阻塞或失败都用 source_collection_stage_writeback_tool 回写；memory/knowledge_steward 阶段的 approved 候选会经后端来源审核、知识提案和审核应用门禁物化为正式 KnowledgeItem，其他阶段回写不写正式知识。",
