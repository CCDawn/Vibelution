...
def _valid_identity(uow: Any, action: PendingAction, invocation_id: str, child_run_id: str) -> Any | None:
...
def defer_knowledge_child(
    ...
    *,
    ...
    def mutate(uow: Any) -> None:
        invocation = _valid_identity(uow, action, error.invocation_id, error.child_run_id)
...
def _wake_in_uow(uow: Any, invocation_id: str, now_ms: int) -> bool:
    invocation = uow.repository.get_knowledge_invocation(invocation_id)
    if invocation is None or not _terminal_ready(invocation):
        return False
    ...
    for action_id, raw in rows:
        ...
        if cursor.get("invocationId") != invocation_id:
            continue
