...
def _snapshot_next_legal_action(report: dict[str, Any] | None) -> str:
    if report is None:
...
def get_challenge_cup_dev_control_snapshot(team_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "mode": DEV_ONLY_MODE,
        "realCampaignAllowed": False,
        "nextLegalAction": _snapshot_next_legal_action(report),
        "report": report,
        "batches": batches,
...
def run_challenge_cup_dev_readiness(team_id: str, *, mode: str = DEV_ONLY_MODE) -> dict[str, Any]:
    ...
    always cleaned up, even when the report build fails. READY is only reported
    under the existing gates of ``build_platform_flow_readiness_report`` and
...
def run_challenge_cup_dev_batch(
    ...
    dev-12/dev-125 and formal/real scopes are rejected fail-closed.
    """
