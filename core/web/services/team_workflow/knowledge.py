...
def ensure_official_model_evidence_for_receipt_refs(
    ...
    *,
    ...
    Every ``question_model_invocation_receipts`` row already proves a real
    succeeded model call for this run; the challenge program's official-call
    gate intersects the v2 output's ``invocation_evidence_refs`` with the
    evidence ids registered here (``model-invocation-receipt:{receiptId}``),
    so a run whose receipts were never mirrored can never pass that gate.
    Idempotent by evidence id: already-registered receipts are skipped,
    rows without receiptId/provider are skipped and counted (a single bad
    receipt never fails the batch), and nothing here promotes formal
    knowledge.  modelProvider keeps the receipt's concrete provider id (e.g.
    ``dashscope_main``); the gate matches it by contains-marker.
    """
    s = _service()
    ...
    with s._WORKFLOW_LOCK:
        ...
        existing = {
            ...
            for item in s._official_model_evidence_entries(evidence_store)
        }
        schema_version = evidence_store.get("schemaVersion") or s.SCHEMA_VERSION
        now = s.utc_now_iso()
        for receipt in rows:
            receipt_id = s._trim_text(receipt.get("receiptId"), max_length=160)
            provider = s._trim_text(receipt.get("provider"), max_length=120)
            ...
            stage_id = s._trim_text(scope.get("stageId"), max_length=128)
            evidence_store.setdefault("evidence", []).append(
                {
                    "schemaVersion": schema_version,
                    "evidenceId": evidence_id,
                    "teamId": normalized_team_id,
                    "workflowId": s._trim_text(workflow.get("workflowId"), max_length=160),
                    "workflowKind": s._trim_text(workflow.get("workflowKind"), max_length=80),
                    "taskType": "",
                    "workflowNode": "",
                    "candidateId": "",
                    "stageRoundId": stage_id,
                    "sourceRunId": normalized_run,
                    "taskId": s._trim_text(scope.get("taskId"), max_length=128),
                    "modelProvider": provider,
                    "modelId": s._trim_text(receipt.get("requestedModel"), max_length=160),
                    "modelName": s._trim_text(receipt.get("model"), max_length=240),
                    "modelProfileId": "",
                    "evidenceKind": s._normalize_official_model_evidence_kind("invocation_log"),
                    "artifactPath": "",
                    "screenshotPath": "",
                    "logRef": "",
                    "promptSummary": "",
                    "outputSummary": "",
                    "sourceRefs": s._normalize_ref_list([normalized_run], max_items=32),
                    "evidenceRefs": [],
                    "status": "registered",
                    "recordedByAgent": "stage_one_command_service",
                    "metadata": s._normalize_metadata(
                        {
                            "questionId": normalized_question,
                            "workflowRunId": normalized_run,
                            "stageId": stage_id,
                            "nodeRunId": s._trim_text(receipt.get("nodeRunId"), max_length=160),
                            "formalNodeId": s._trim_text(scope.get("formalNodeId"), max_length=160),
            ...
            registered += 1
        if registered:
            evidence_store["updatedAt"] = now
            s._write_json(
                s._official_model_evidence_store_path(normalized_team_id), evidence_store
            )
    if registered:
        s._record_workflow_event(
    ...
    return {
        ...
        "skipped": skipped,
        "evidenceStorePath": s._relative_path(
            s._official_model_evidence_store_path(normalized_team_id)
        ),
    }
