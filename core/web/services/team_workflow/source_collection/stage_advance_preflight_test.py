...
class StageAdvanceReadyTests(unittest.TestCase):
    def test_ingestion_blocked_when_graph_has_nodes_but_no_edges(self) -> None:
        ...
        with (
            mock.patch.object(stage_session, "_service", return_value=FakeService()),
            self.assertRaises(FakeService.TeamWorkflowOrchestrationError) as ctx,
        ):
            stage_session.assert_source_collection_stage_advance_ready(
                stage_id="ingestion",
                record_count=19,
    ...
    def test_ingestion_allowed_when_graph_healthy(self) -> None:
        ...
        with mock.patch.object(stage_session, "_service", return_value=FakeService()):
            stage_session.assert_source_collection_stage_advance_ready(
                stage_id="ingestion",
                record_count=19,
                approved_or_source_candidate_count=19,
