...
def stage_writeback_prompt_lines(stage_id: str) -> list[str]:
    ...
    if stage_id == "extraction":
        return [
            ...
            "- 正式 claim 路径的 completed 提炼回写被服务端强制校验：候选/记录的存储 `summary` 非空时，每条非 `exclude` 条目必须至少带一个逐字 quote 锚——嵌套 `claims[]`/`keyFindings[]` 项含 `quote`，或 `evidenceRefs[]` 项含 `{id, quote}`；`quote` 必须是 `candidates[].summary` 的逐字子串，从上下文原样复制，禁止改写、拼接或凭记忆重写；缺锚或改写的 completed 回写会被直接拒绝。",
