...
def stage_writeback_prompt_lines(stage_id: str) -> list[str]:
    ...
    if stage_id == "extraction":
        return [
            ...
            "- 要进入正式 `ClaimEvidenceStore` 的主张，`claims[]` 还必须提供来源中可复核的有界原文 `quote`，并且每条 `claims[]`/`keyFindings[]` 项都要自带显式事实字段 `fact`；`claim` 是归纳主张的别名，运行时只认 `fact`，不能用 `claim` 替代。没有真实 `quote` 时必须保持 `missing_evidence_anchor`，不得为通过门禁改写或补造引文。",
