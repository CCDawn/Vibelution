...
PER_CALL_MAX_MS = 800_000
# The derivation band is collapsed onto the governed cap: live meeting-speaker
# calls regularly ran 300-416s with a heavy tail past 600s (slow-model calls
# were abandoned by the fence and their output discarded), so the cap moved
# 600s -> 800s on 2026-09-03.  Receipt-derived budgets are always the governed
# fence; as a consequence the operator config/env override only accepts that
# same cap, and lower pins are rejected by the existing fail-loud override
# contract.
