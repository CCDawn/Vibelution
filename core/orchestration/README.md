...
`AgentRuntime` 是三种模式共用的具体运行时；`chat`、`supervised_evolution` 和 `self_evolution` 的行为由 `ModePolicy` 决定。类名不代表启用自进化，连续自主运行只由 `self_evolution` 的自动循环策略允许。
...
| `turn_carryover.py` | 转轮消息序列化 / 反序列化 | `AgentRuntime` 的 serialize/deserialize wrapper |
| `turn_compression.py` | 上下文压缩与账本检查点 | `AgentRuntime._compress_messages` |
| `turn_diagnostics.py` | 重试广播、cache 诊断、invocation context、stall 报告 | `AgentRuntime` 对应 wrapper |
| `tool_authorization_binding.py` | 运行时身份绑定、可见工具物化、隐藏工具文案、重启护栏 | `AgentRuntime` 对应 wrapper；policy 仍在 `core.authorization` |
| `turn_message_assembly.py` | 单 Turn 消息排序：seed normalize/sanitize、static/volatile 插入 | `AgentRuntime` 对应 wrapper；Prompt 策略仍在 `prompt_manager` / `context_engine` |
| `turn_llm_adapter.py` | Agent 主/备路由 attempt loop；UI/scene 由注入 observer 接收 | `AgentRuntime._invoke_llm` wrapper；调用/recovery/routing 仍在 `core.llm` |
...
TurnRunner (web/control-plane 入口)
  -> AgentRuntime.think_and_act / run_single_turn / _run_orchestrated_turn
