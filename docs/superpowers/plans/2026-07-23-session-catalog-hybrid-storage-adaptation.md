...
> **Implementation link:** T0-T2 complete; T3 reconcile core plus freshness primitives, T4 fail-safe shadow scaffold and T5 SQL candidate provider complete; canonical mutation wiring, runtime catalog provider registration and canary remain
...
- T3 核心：canonical snapshot 投影、孤儿 journal 隔离、TEMP candidate、源 revision 二次校验、原子发布、删除重建和 stale lease takeover 已提交。`chat_state.state_revision` 现在在所有 state 写入（含 legacy-message cleanup）中以持锁磁盘状态单调推进；dirty 记录和 `catalog.untrusted` 只会在成功 reconcile 时条件清除，晚到 mutation 会保留。真实 canonical mutation bridge 与 startup/incremental 调度尚未接入。
...
- T6 未开始：不得启用 `read_preferred`；必须先完成 canonical mutation 的 dirty invalidation wiring、runtime provider registration、真实 shadow 零差异和 Launcher canary 证据。
...
下一步建议: 在保持默认 `off` 的前提下，接入 canonical mutation dirty bridge 和 runtime catalog provider；随后以真实 shadow 对账与 Launcher canary 证据决定是否进入 `read_preferred` 评审。
