...
| **R** | HeroUI → shadcn/Radix renderer（按 primitive） | 进行中：`VButton` + `VTooltip` 已切 shadcn renderer |
...
- `web/src/components/vui/primitives/VButton.tsx` / `VTooltip.tsx`（页面 API 不变）
- 下一步候选：`VChip` / forms 仍走 HeroUI
