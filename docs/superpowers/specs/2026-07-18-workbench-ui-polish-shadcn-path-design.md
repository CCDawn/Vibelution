...
| **R** | HeroUI → shadcn/Radix renderer（按 primitive） | 进行中：Button / Tooltip / Chip / Input / Textarea |
...
- `web/src/components/vui/primitives/VButton.tsx` / `VTooltip.tsx` / `VChip.tsx` / `forms/VInput.tsx` / `forms/VTextarea.tsx`
- 下一步候选：`VSelect` / `VCheckbox` 仍可走 HeroUI
