...
### B.25 续四：打包证据三连修复实弹贯通——证据聚合/精简卡投影/三层 id 权威；㉓+23 反馈修订落账缺口定案（09-10 晨）

- **㉓+20 修复合入（e50b8df4f 含返工，亲审）**：`_evidence_card_payload` 两层读取——authority run 首读（单 run 路径字节不变），缺失/空卡时从 materialized dimension_reviews 的 evidence_refs（`evidence_card_batch://<dprun>`）推导评审实际引用的 run 集（`_cited_evidence_run_ids`，含 nested candidates 容器），逐 run 严格 scoped 读取去重合并（claimEvidenceId/evidenceId/evidence_id 三键），返回 `{aggregatedFromDimensionReviews: true}` 信封；全空则原 fail-closed 错误直通。生产口径：SCI-009 正式 run 4 个被引 run 合计 109 张卡、16 个去重 DOI。
- **缺陷㉓+21（精简卡投影，三轮返工后合入 e50b8df4f）**：聚合卡的真身是 hypothesis-first 精简卡（仅 quote/sourceId(DOI URL)/locator{kind:url}/时间戳/evidenceKind），无 title/sourceUrl/retrievedAt/source_type——`_evidence_item` 必 fail。只读干跑推翻初版设计两次：①初版按「被引 run 的候选批次」建 URL→title 索引只覆盖 10/16 DOI（旗舰 science.adr3837 等 6 条的 source_manifest 记录挂在未被引用的早期搜集 run 名下）→索引权威改为 run owner project 整体 candidate store（`resolve_research_project_workspace_root` 一次读文件，63 URL/69 记录，16/16 全覆盖；snapshot 的 projectId 别名链已核可解析）；②source_type 初版落 'other' 伞型 → 改为先用 URL 命中记录的 sourceKind 映射（paper→peer_reviewed_paper），无识别 kind 才回退 evidenceKind 伞型。投影语义=fill-missing、幂等、不造数据：locator.url→source_url（仅 kind:url）、updatedAt/createdAt→retrieved_at、解析不到的 title 保持缺失由严格生产者 raise。store 读失败→空索引→照旧 fail-closed。
- **缺陷㉓+22（三层 id 权威，4fa5a704b 合入亲审）**：08:29 前的实弹重试显示 109 张卡全部撞 `_evidence` 孤儿门——正式 run 的 authority 批次 0 id、hypothesis_set 仅 4 个 hyp-* id，而卡的 candidateId 是 42 个 candidate-2026*（记录只存在于项目级 store、scoped 到其他搜集 run）+67 个假说链 sci-009-* id（dimension_reviews 行自身的 hypothesis_id 字段才是其权威）。修复：by_id 分层并入（first-seen 保权威序：authority 批次→hypothesis_set→项目级 store 记录→dimension 行收割 id 映射空记录只证存在），孤儿门逐字保留；项目 store 经 memoized reader 单次读取同时供给 lean 索引与 id 权威，单 run 路径零读取。
- **实弹验证（08:27:34 restart 加载后）**：08:29:43 UI「重试结果打包」retry_node accepted → 打包构建连过证据聚合+精简卡投影+孤儿 id 三道门，run blocker 推进至 `canonical feedback_iterations contains no actual revision`——㉓+20/21/22 全部生产生效。
- **缺陷㉓+23（反馈修订落账缺口，修复在途）**：结果包的 feedback_iterations 段要求真实修订记录；全仓仅 2 条 artifact（旧 run），SCI-009 正式 run 0 条。定案：hypothesis_first_chain 的 `_materialize_review_feedback_iterations_authority`（会议链重放物化器）为死代码零调用方；且其形态（node_id=hypothesis_design、固定两轮 phase 契约）不匹配生产的 10 轮链。真实证据在轮次存储：10 轮单链 lineage（442b→…→5b7c），每轮 revisionEnvelope{parentCandidateId, phase:review_revision, revision{actual:true, changes[]}, feedback{humanFeedback=评审裁决理由}, revisionReceiptRef}完整。修复方向=新增 sweep 回填步：对零 feedback artifact 的正式 run，重放轮次修订链经既有 canonical writer（cross-run 形态：childRunId/parentRunId 按 lineage 锚定 formal run、node_id 独立于 same-run 验证器契约），round 1..N 连续，链不完整/无 actual 即整体 fail-closed。
- **段后风险预扫（本轮只读核完）**：feedback 之后仅剩 schema+semantic 验证；semantic 五门中 hypotheses 段走 accepted-round 投影路径（生产 hypothesis_set candidateDetails 为空触发，覆盖与维度行一致的 sci-009-* id），selection.selected_hypothesis_id 与 14 条七维评审行同源，stage1_research_plan 10 行在库——无已知阻塞。

### B.25 续五：SCI-009 第一阶段全链闭环 + ㉓+23 落账实弹验证；SCI-014 假说选择门操作完成（09-10 午）

- **㉓+23 合入（0c8396a04，orig 07a3dee45，亲审通过）**：`backfill_feedback_iterations_from_round_chain`（轮次 revisionEnvelope → canonical writer，cross-run 链形态，全链预校验后首写，fail-closed）+ `auto_backfill_missing_feedback_iterations` sweep 步骤（sweep_auto_advance_closure 步骤 0-7，仅 blocked/failed 正式 run，所有权门防跨管线重写，幂等）。实现过程中原子 agent 中途死亡（47 分钟无进展零进程），遗留 610 行实现+13 测试用例成色完好，由接续 agent 验证（13 新测试+23 feedback+137 链+206 result_package+107 sweep 文件全绿，walker 契约对真实 `_feedback_iterations` 验证）后原样提交。closeout 遭遇并行会话两次推进 main（1c29208d0/831abade0 supervision 修复），rebase+静窗重试后 merged_clean。
- **㉓+23 生产验证（10:24:16 restart 后 45 秒内）**：sweep 自动落齐 10 条 feedback_iterations artifact（round 1..10 连续、轮 1 无 parent、轮 10 child 为空锚定正式 run、nodeId=hypothesis_review_revision 不触发 same-run 验证器），全部 scoped 到 run-332a539909a6 / authority 2cd75f2c。
- **SCI-009 第一阶段闭环（本验收主目标 A 线达成）**：10:38:55 UI「重试 结果打包」retry_node accepted → **10:39:14 run-332a539909a6 succeeded，terminal_reason=stage_one_proposal_completed**（构建仅 19 秒）。最终挑战包内容：**2 条假说 + 60 条证据 + 14 条七维维度评审（7×2）+ 10 轮反馈修订 + 60 项引用核验 + 1 条 selection 落选记录**，schema+semantic 验证全过。修复链全量：㉓+18（3d5251a32 引用解析）→㉓+19（6746151b9 悬边回收）→㉓+20（3eeb6a813 证据跨 run 聚合）→㉓+21（e50b8df4f 精简卡投影，三轮返工）→㉓+22（4fa5a704b 三层 id 权威）→㉓+23（0c8396a04 修订落账），六缺陷全部「只读干跑定案→子 agent 修复→亲审（含两次打回返工）→合入→重启→真实 UI 按钮验证」。
- **SCI-014 假说选择门操作完成（B 线）**：重启后 r1 候选生成会议被 completionResume 自动复活并于 10:42:43 收口产出纪要（digest-72e59f292016f17b）；纪要确认门按钮在纪要落盘前禁用（运维注记：纪要生成约需 10 分钟，禁用是正确防抖）→ 复选框精确勾选 2/8：sci-014-cd9bcddd3（VKORC1/CYP2C9 基因型华法林剂量算法，TTR≥15%+出血降≥20% 量化可证伪）+ sci-014-cc9991e3e（按需生物打印 72h 处方-打印-体外释放 smoke gate，知识库有 SPRITAM 直接证据）→「记录选择并开启评审」提交 → 02:46:05 链记录 review_dispatch_attempt（hsel-4f50a43a647）确认受理，评审轮在 roundBudget=2 下自动推进。
- **遗留（非阻塞）**：知识搜集子流程画布仍显示 4/5 已阻塞（SCI-009 旧知识侧流 run-1ca97605acf3 reconciliation_required 的展示残留，不影响已闭环的正式 run）；并行监督进化会话（Judge Agent/候选隔离工作树）间歇占用 active-work，本窗口重启须择其静默间隙。
