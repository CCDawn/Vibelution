...
- 完整 selector 第三轮已通过前两组后端、假说链/状态组、Teams 整组 Vitest、VUI、TypeScript、changed 前端及新增测试组；最后工具/实验路由组有两条旧断言失败。缓存检索测试把紧凑回执与原始字典全等比较，现校验新 locator 确实刷新、计数准确且 context 只计算一次。实验路由测试则期待从与冻结 source 不符的 artifact 自动恢复 STOP，但现行 `iteration_route.py` 和 artifact store 在本任务中均未改动，原合同已移除这种 fallback；现测试先断言错 scope 不返回决策，再写当前 scope 后确认 STOP。没有恢复旧兼容逻辑，也没有放松真实性门。
- 展开该组回归发现另两个路由/恢复 case 也依赖同一种错误 source fallback；分别改为读取当前 scope，以及明确验证错 scope 不恢复、当前 scope 的新权威记录才允许恢复。新的 authority 使用独立 artifact identity，避免同 payload 的幂等回放仍返回旧 scope。补充搜索与实验路由组合按 selector 的并行模式共 19 项通过。上一轮 selector 累计命令耗时约 565 秒；项目 manifest 必须绑定最终 HEAD，测试修正后需重跑该门，不能手工拼接或修改失败 manifest。
