...
| 监督进化两条控制面并存（旧 bundle 闭环与 worktree 闭环双存储双流程） | 待评估：退休窗口与主路径收敛 |
| `reflection` 撞名（自进化反思记录 vs 监督闭环的改进提示） | 已澄清（2026-09）：两侧 docstring 互指 |
| `reviewer` 角色在 worktree 闭环未被使用（档案与工具契约空转） | 待评估：复用或退役 |
| 审批 Agent 身份含糊（auditor 角色解析 judge 输出标记） | 待评估：设计意图待表达 |
| 自进化"反思→候选池"未自动接线（学习链最后一环是断的） | 待修：接线，或显式降级为人工审查源 |
| 自主闭环 MAX_ITERATIONS=1（多候选迭代未实现） | 已知限制（设计未完成） |
| 主工作树内联自进化前端入口已退役、后端仍在 | 待评估：退役收口 |
