...
**Status: PROPOSED（未实施）** · 2026-09-12（§1.1 复查 2026-09-13）
...
### 1.1 复查与处置（2026-09-13）

对 §1 的两类异常做了只读诊断（判定逻辑与 `report_session_ledgers.py` 一致）并处置：

- **30 个孤儿账本已清理**。构成：28 个 2026-07-25 ~ 08-13 的旧世代会话（主体是被"退休旧挑战题实验"系列操作删除目录行后遗留的自动化阶段任务账本，另含少量早期试用会话）+ 2 个开发/测试残留（`session-beta`、一个空 assistant_message 文件）。删除合计 25.0 MB；删除清单留存于实例 `backups/orphan-ledgers-deleted-20260913.json`；删除后孤儿数归零（`report_session_ledgers.py` 可复核）。
- **"有行无账本"的行（复查时 929 个）建议保持现状**。构成：75% child 会话（研究/挑战杯工作流程序化创建）、804 已归档、745 已隐藏，绝大多数从未产生对话。属正常运行产物（不进入界面目录的可见面）；清理需要正规特性支持并处理外键一致性，收益低——如需处理应作为独立特性评估。
