...
> 日期：2026-09-11
>
> 状态：USER-REQUESTED / PROPOSED FOR REVIEW（P0/P1/P2/P2b/P2c/P4b 已执行；**G4、G5 通过**，G1 未测，无 GO/NO-GO 结论）
>
> 版本：V1.3（纳入 P2 会话连续性实测与 P4b 流式实测；G4、G5 判定通过，并记录一个独立的落盘缺陷）
>
> 文档用途：为「用 Command Code headless CLI 作为 Vibelution 的模型传输层」这一方向提供可执行的探针方案、量化 GO/NO-GO 判据，以及必须先裁决的架构阻碍清单，供决定是否进入集成设计。
>
> 当前完成范围：P0、P1、P2、P2b、P2c、P4b 已执行并留证，结果见 §7.1 与 §7.2。P3（坏窗口结构免疫，G1）与 P5 未执行，因此**本文不给出 GO/NO-GO 结论**。产品运行路径、operator config、provider 均未改动；探针脚本与原始证据只存在于会话临时目录。
>
> 证据性质：文中标注「实测」的条目为 2026-09-11 在本机直接观察所得；标注「官方文档」的条目来自 commandcode.ai 公开文档抓取；§7.1/§7.2 为真实探针输出（共消耗 11 次真实模型调用）；其余均为待验证假设，不以推测充当结论。
>
...
**当前判定状态**：G4、G5 通过；G1、G2、G3、G6、G7 待测。

G5 的通过附带一个必须进入集成设计的条件：**极短回合存在不落盘现象**（§7.2 C），而调用方无法从返回值察觉。因此集成设计不能把 CLI 会话当作唯一历史权威，需要一条可核验的落盘确认，或由产品侧保留可回退的历史副本。**在 G1 未测之前不构成 GO**——G1 仍是唯一能证伪整个方向的判据。
...
| P3 受控 A/B 采样 | G1/G2 证据：坏窗口连发 + 同任务 HTTP 对照 | 是（主要预算） | 待执行（需坏窗口时段） |
| P4 多轮工具往返 | G3 证据：客户端上传字节数与 TTFT 对照 | 是 | 待执行 |
| P4b 长答案流式 | G4 证据：长文本下文本增量是否跨越数秒到达 | 是（1 次） | **已完成且通过**，见 §7.2 |
| P5 结论回填 | 回填本文档聚合数字与 GO/NO-GO | 否 | 部分完成（P0/P1/P2/P2b/P2c/P4b 已回填） |
...
| 单例样本外推 | 落盘缺陷目前仅 2 例，成因未区分；结论中已标注样本量与候选解释 |
...
P0/P1/P2/P2b/P2c/P4b 已完成，累计 11 次调用。已确认可用：无 shell 层的调起路径（B3 解除）、事件流可观测、流式保真（G4 通过）、会话可连续且同 cwd 并发可隔离（G5 通过）。已记录的独立缺陷：极短回合可能不落盘（§7.2 C）。

仍未回答的核心问题是 **G1：坏窗口是否真的结构免疫**——这需要在坏窗口时段发起一轮采样，也是唯一能证伪整个方向的判据。G2（冷启动对照）、G3（上传量对照）与 G6/G7 尚未测。

需要用户决定的事项：

1. **P3 的时段**：坏窗口不可控，需要你指定一个已知时段，或授权探针在后台轮询等待（有界，例如 30–40 分钟）。同时请给调用次数上限，主要预算在这一步。
2. **落盘缺陷是否值得单独定位**（1–2 次调用即可区分为"异步 flush 竞态"还是"内容相关"）：它决定集成设计里要不要为每个回合加一条落盘确认。
3. **是否向 Command Code 反馈**：极短回合不落盘（0 字节 transcript、`messageCount=0`）是 1.53.0 的可复现行为，且调用方无法从返回值察觉，属于值得上游知晓的问题。
4. **B2（工具执行归属）**是否值得在 G1 通过后单独立项——它仍然是比传输替换更大的问题。

在 G1 有结论之前，不建议投入任何集成实现。
