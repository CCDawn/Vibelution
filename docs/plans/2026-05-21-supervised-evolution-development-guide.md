...
- `supervised_dashboard._record_from_payload()` 已改用共享 `build_decision_record_artifacts()` 解释 policy action、gates、case summaries、decision path 和 Gym path，减少 dashboard 对 decision record schema 的本地重复解析。
...
- Dashboard record 构造是否只消费共享 decision artifact 解释结果，不再本地重复过滤 gates/cases 或扫描 Gym path。
