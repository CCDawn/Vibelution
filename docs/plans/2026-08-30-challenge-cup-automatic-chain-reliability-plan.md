...
| 会议续轮依赖进程内 executor/job set | `CLOSED_REGRESSION` | `meeting_runtime.py:151-157` 的 `_MEETING_DISCUSSION_EXECUTOR`（max_workers=4，`fd44c0c8b`）仍是唯一调度 owner；派发前先持久化 intent（`33bcfc15b`），且 running intent 现携带有界租约（`meeting_driver_work.DEFAULT_INTENT_LEASE_MS=90s`）：存活驱动经心跳线程续租（`refresh_intent_lease` + `start_lease_heartbeat`，`meeting_runtime._run_scheduled_meeting_discussion` 在 finally 停止心跳），恢复扫描对 running intent 的失效判定为「外部 boot **或**租约过期」（`meeting_driver_work._recover_one_meeting` + `_lease_expired`），同 boot 卡死无需重启即可被 re-drive；心跳不递增 `attemptCount`，拒绝为非 running/外部 boot 的 intent 续租；同 boot dedup set 仍保证每会议至多一个存活驱动。测试：`tests/test_research_workflow_meeting_driver_recovery.py`（过期租约同 boot re-drive、新鲜租约被信任、心跳续租与拒绝规则、心跳线程、驱动包装接线） | 回归保留；digest 工作本身的 durable intent 见第 3 项 |
| `summarizing` 无 durable invocation intent | `CLOSED_REGRESSION` | digest 草稿成为第二种 durable work：`meeting_driver_work.ACTION_RUN_DIGEST`（`meeting_driver_work.py:41-42,74-78`）与 `run_discussion` 共用 append-only intent 存储、按 `actionKind` 分命名空间互不污染（`latest_intent(action_kind=...)`）。`draft_meeting_digest` 在有界 LLM 调用**之前**落 running 记录，携带绑定源文的 `sourceHash` 与会议受治理 deadline（`meeting_runtime.py:2855-2886`，`_digest_recovery_deadline_ms` 取 `challengeDeadlineAtMs`/`meetingDeadlineAtMs` 最早正值），成功落 completed、异常落 failed 后原样抛出；running 租约 `DIGEST_INTENT_LEASE_MS=480s`（略高于 450s review-runner 上限）是崩溃栅栏而非心跳窗口，并被 clamp 到会议 deadline 之内（`meeting_driver_work.py:222-227`），终态记录继承 `sourceHash`/`deadlineAtMs`（`meeting_driver_work.py:205-214`）。启动扫描把 `summarizing` 会议交给 `_recover_one_digest`（pending / 外部 boot / 租约过期 / failed 未达共享上限 `MAX_AUTO_RESCHEDULE_ATTEMPTS=3`）经 `meeting_runtime.schedule_meeting_digest_redrive` 重放**同一个幂等 summary-draft 状态机**（`_MEETING_DIGEST_JOBS` 进程内去重；`sourceHash` 命中已落盘草稿时直接 submit 复用、零二次模型调用，`meeting_runtime.py:3284-3303`；blocked 重放以 `digest_redrive_blocked:<code>` 落 failed 保持可审计，`meeting_runtime.py:1987-2047`）；无 digest intent 的遗留悬挂仍走 N1 身份分流（回填或 `legacy_orphan_closeout`）。评审侧模型调用发生在讨论驱动内，由 `run_discussion` 租约/心跳与会议 deadline 栅栏覆盖，不是独立 durable work。测试：`tests/test_research_workflow_meeting_driver_recovery.py`（外部 boot re-drive 与 blocked 收口、attempt 上限跳过、新鲜租约被信任、sourceHash 往返与 hash 复用零二次调用、租约 clamp 与身份继承、两类 intent 命名空间隔离） | 回归保留；生产面待 Launcher 重启后核对 N2 两条 `summarizing` 遗留记录按遗留孤儿收口 |
...
| attempt exhaustion 自动复活/活锁 | `CLOSED_REGRESSION` | T5 证伪测试已落地（`tests/test_research_workflow_adapter_exhaustion_falsification.py`）：时间推进的崩溃式租约循环在 `MAX_OUTBOX_LEASE_ATTEMPTS` 处由 `_fail_leases_over_attempt_gate`（`repository.py:941-978`）一次性终态化；修复扫描落盘后事件序列有界、不再增长；`reconcile_run` 对 exhausted blocked run 原样复述裁决（`plan_ledger_authority` failed-leading 分支）且不复活 adapter 行、不唤醒 worker——**未复现任何紧循环**，故无需也不授权业务代码修复 | 回归保留；若未来生产复现 tight loop，以本测试为基线先写复现用例 |
| preformal 长期使用 direct Session | `CLOSED_REGRESSION` | T7 已落地（`codex/challenge-stage1-t7-bounded-session`）：新增 `team_workflow/preformal_review_sessions.py`，`resolve_preformal_review_session` 按 (房间 scope, agentId) 解析**唯一 hidden Child Session**，父节点是 per-(teamId, agentId) 的隐藏、非激活预评审 root（`research_workflow/preformal_review_roots.json` 登记），缺失 scope 字段／身份不符／hash 不符一律 fail-closed，**永不回落到 `directSessionId`**；`meeting_runtime._resolve_preformal_candidate_review_room` 逐参与者解析并以 `participant_session_ids` 建房间，房间 participants 即绑定指针，遗留房间按 `update_chat_room` 懒式重绑（不改历史，符合「本轮不动存量」），roster 不符或房间忙则抛错。有界性来源即原契约：`_run_participant_agent` 读绑定 Session 作 `chat_history`、`_sync_group_round_to_participant_sessions` 只回写绑定 Session，因此普通 direct Session 不再收到整轮 transcript。`session_scope_key`、`normalize_discussion_scope_binding`、`_public_experiment_binding` 与 `_is_challenge_discussion_room` 的保留门由 formal/preformal **共用同一权威**，未新增第二套 key 或校验；receipt-authority 门（`chat_room_service.py:1195`）与结构化会议消息门（`4034`）仍为 formal-only。测试：`tests/test_challenge_preformal_review_sessions.py`（隐藏 child 绑定与 direct Session 纯净性、幂等与单 root、跨 team 拒绝、binding 拒绝冒领 formal 身份、投影三类形状、普通索引排除、普通房间仍解析 directSessionId）+ `tests/test_research_workflow_meeting_runtime.py` 新增 4 项（room/round 双向绑定到隐藏 child、重开复用同一 child、遗留房间脱离 direct Session 重绑、roster 不符 fail-closed） | 回归保留；存量污染数据留待计划内的 Challenge 数据重置，本轮不迁移 |
...
**复核结论：** T3 digest durable work 落地后（`run_digest` intent + sourceHash + deadline clamp 租约 + 启动扫描 re-drive），11 项中 `CLOSED_REGRESSION` 8 项（2、3、4、5、6、7、8、9）、`PARTIAL` 2 项（1、11）、`OPEN` 1 项（10）。`codex/challenge-legacy-meeting-closeout` 落地后，N1 悬空引用与 N2 生产遗留悬挂已收口。原 T3 产出中的「generation attempt 跟随 meeting」子项不属于本次闭合范围，也未列入 Phase 1 闸门条件，留待 T6/批次编排层处理。T5/T2/T3/T7 依次落地后（`codex/challenge-stage1-t7-bounded-session` 关闭第 10 项），11 项重新计数为 `CLOSED_REGRESSION` 9 项（2、3、4、5、6、7、8、9、10）、`PARTIAL` 2 项（1、11），第 1 项的剩余部分只是生产核对、第 11 项只是 T8 本体。对 [全零人工路线图](2026-09-01-challenge-cup-stage-one-zero-human-roadmap.md) Phase 1 闸门的影响：T1–T7 已全绿，**不再有未闭合的代码缺口**；剩余前置条件转为生产面——N2 两条遗留会议在下次 Launcher 重启后按 `legacy_orphan_closeout` 收口的只读核对，以及第 1 项的生产样本 deadline 核对。路线图 §9 停止条件继续有效，T8 合闸仍需显式授权。
