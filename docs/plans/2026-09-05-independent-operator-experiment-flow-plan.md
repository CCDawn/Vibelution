...
7. Windows 开发与运行时，目录层级和文件名必须保持简洁；完整路径及其派生临时文件、锁文件不得超出所用 Windows API 和工具链的路径限制。具体设计与验收要求见第 1.1 节。

本方案不把“算子更快”表述为证明 SCI-091 的计算绝对上限。方向 B 的核心是一个真实、可解释、可复现的实验反馈闭环。

### 1.1 Windows 路径简洁与长度要求（2026-09-13 补充）

- **适用范围**：第二阶段的开发工作目录、实例数据、实验产物、缓存、构建输出，以及自动生成的临时文件和锁文件。路径仍由现有 resolver 统一生成，不新增平行存储规则。
- **设计要求**：采用简短且有界的目录名、文件名和必要层级，避免把完整讨论标题、重复的项目/活动身份或多层长哈希直接拼入路径。缩短名称时必须保持身份唯一性和记录可追溯性。
- **长度预算**：以最终完整绝对路径计算，包含用户目录、实例根、扩展名、随机临时后缀和 `.lock` 等派生部分。项目生成路径采用不超过 240 个 UTF-16 代码单元的保守工程预算；所用 API、路径分量或工具链有更严格限制时按更严格者执行。240 是预留后缀与目录操作余量的项目预算，不是 Windows 的统一官方上限；传统 Win32 `MAX_PATH` 为 260，包含终止空字符，不同 API 的限制并不完全相同。
- **环境要求**：默认未启用 `LongPathsEnabled` 的 Windows 环境也必须能够完成开发与实验。开启系统长路径支持或添加 `\\?\` 前缀不能替代路径简化要求；已有局部扩展路径处理不代表本需求已完成。
- **验收证据**：覆盖实际用户/实例根路径深度及最长合法名称，核对最终产物与临时/锁文件的完整长度，并验证创建、读写、原子替换及删除。若用户选择的根目录使预算无法满足，应在创建工作区或实验之前明确指出原因并提示选择更短目录，不能等实验执行中才报文件不存在。

背景：本次 Windows 实验曾因 207 字符的目录叠加 64 位哈希和原子写入临时后缀，生成约 291 字符的路径，触发文件创建失败。本节记录后续路径布局改造及验收要求；补充文档本身不代表改造已完成，也不授权迁移历史实验数据或修改系统设置。
...
## 29. 已取消讨论的同轮恢复（2026-09-13）

用户已批准首轮受限实验的同轮恢复，继续遵守 5 元人民币、GPU 600 秒、最多 1 个实验轮次。已取消的 WorkflowRun 是历史事实，不通过修改数据库恢复为运行中；`retry_node` 对终态运行不再显示可执行。

新增活动内 `POST .../operator-experiments/{campaign_id}/rounds/{round_id}/recover`，要求服务端 operator 身份、活动版本与幂等键。仅允许最新活跃轮次的已取消运行，且仍处于讨论节点、未产生假设或后续阶段产物。恢复创建新的 Ledger run，更新本轮 runId/protocolRef 与活动 activeRunId；roundId、ordinal 和轮次数不变，previousRunIds 累积保存所有前任运行。

复用现有 run creation、固定 workflow definition、node binding、artifact readback 与活动存储锁。沿用原运行输入的预算、模型策略、基线、父候选、工作负载和观察资料；环境和协议内容不变，只复制到新运行的产物作用域。新身份由旧 runId 短哈希生成，活动保存失败后重新请求也只回收同一个后继。原有团队讨论仍通过已绑定 planner 和团队成员配置解析参与者，不新增一套会议编排。

恢复动作本身不调用模型、不执行 GPU；随后使用正式 start_node 命令启动讨论，所有节点继续受 readiness 和原活动预算约束。旧运行的费用回执、未结算预留和 GPU 记录保留，预算汇总按活动跨运行累计，不新建活动或释放未知费用绕过上限。已完成基线直接复用。

绑定解析必须显式接收该运行固定的 workflow definition。否则解析器默认按第一阶段节点集合过滤，会删除 optimization_discussion 的 node override，使恢复运行的 planner 未绑定。创建入口已改为传递固定定义，并以未替换的实际绑定解析器覆盖此回归。

验收覆盖真实 Ledger 和产物存储、同轮身份、两次恢复的历史链、固定定义与绑定、权限和错误归属拒绝、创建成功但活动写入失败后的重放、恢复讨论资料回读，以及旧模型费用仍限制新运行准入。测试通过仅证明恢复机制；真实讨论、实验和费用结算仍需经产品 API 单独验收。
