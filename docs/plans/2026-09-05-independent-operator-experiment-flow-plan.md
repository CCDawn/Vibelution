...
## 27. 逐次评价、最佳候选与下一轮证据（2026-09-13）

用户已确认多次试验的晋升规则：计划内每一次试验都通过正确性、提升门槛和逐工作负载退化检查，才更新当前最佳；任一次失败、不可比或未达标均保留父候选。策略标识为 `all_planned_trials_pass_v1`。不选择最快的一次，也不合并样本掩盖失败；每次置信区间仍只描述该次配对样本的变异，不宣称跨设备或独立会话泛化。

`operator_evaluation` 已接入 System dispatch。执行与评价共享冻结计划回读和稳定 trial 身份算法；评价逐项核对计划内测量、候选源码、活动/轮次及实际费用结算，缺少任一终态回执时不出结论。复用既有 `evaluate_measurement()`，保留每项比较、失败原因和费用；正确性标志为真但没有实际误差证据也不能晋升。评价产物绑定计划与候选引用，回读后绑定轮次 `evaluationRef`。

`optimization_feedback` 回读规范评价并核对测量派生结果，再发布独立反馈。反馈保留逐项执行结果、评价原因、预先声明的预测和反证；机制假设标为待团队讨论，不把数值加速直接写成机制已证实。反馈引用与 `bestCandidateRef` 在同一次活动更新中绑定，原始基线保持不变。重复处理不会再次晋升；产物写入后、活动更新前中断可以重试恢复。

已有 `prepare_round()` 入口继续负责下一轮创建：前一轮必须达到终态，且满足授权、预算和轮次上限。下一轮冻结保留或晋升后的父候选，同时携带上一轮评价、反馈与失败测量进入讨论。没有在反馈节点尚未结束时提前创建下一轮，也没有在本批新增终态后自动调度下一轮的策略；该自动触发仍需后续接入现有生命周期机制。

本地复用评估使用数值评价函数、规范 artifact store、活动幂等更新、原生 SystemActionAdapter 与轮次构造；沿用前序 RD-Agent 调研中失败成为反馈、执行和判断分离的设计，不引入新框架。`tests/test_operator_evaluation_feedback.py` 通过真实 SQLite 和产物存储，覆盖全达标、混合快慢、执行失败、正确性失败、误差证据缺失、错误候选、缺测量、中断恢复、原生节点核验，以及保留/晋升两种下一轮证据继承。CUDA 返回为受控数据，没有真实模型、GPU、页面或性能收益验收；产品验收前仍需受管刷新，本批不重启、不发布。

## 28. 轮次终态与自动启动下一轮（2026-09-13）

本批接通第 27 节尚未实现的终态后自动迭代。此前 optimization_feedback 交付后没有命中 adapter/graph 的整轮关闭条件，不能仅增加下一轮回调；现在该节点验证完成会写入 operator_round_completed 终态。算子工作流不进入挑战杯提交材料交付链，普通工作流和独立基线保持各自终态语义。

轮次成功与 operator_round_completed 类型的 event_publish Outbox 行在同一 Ledger 事务提交，事件绑定真实反馈节点及命令。复用已有 EventPublishWorker 的领取、重试和 ACK；没有新增轮询器、线程或后台进程。iteration.py 回读终态及规范反馈后，通过已有 prepare_round() 创建下一轮，再调用 WorkflowCommandService 启动 optimization_discussion，仍需通过正式就绪检查。

前一轮 Ledger 保存有界 operatorIteration 恢复记录：创建意图、固定命令键、下一轮 ID、启动版本及最终状态。创建后但记录前中断，按相同活动命令回读原轮次；启动命令接受后但记录前中断，以相同请求恢复原命令。重复事件不会重复建轮或开讨论。原始测量失败仍能通过评价/反馈完成本轮，因此也可进入后续讨论；没有完成反馈的失败运行不会假装具备下一轮输入。

启动前检查活动运行与授权、最大轮次数、至少可用的调优 GPU 时长、显式讨论/规划预算，以及模型账本中的实际费用与尚未结算预留。下一轮必经的讨论与规划最大预留合计无法容纳时停止，不把未知用量当零；实际模型调用仍由既有逐次预算准入负责。暂停、取消、预算不足和已被其他轮次取代均记录停止原因；正式就绪拒绝记录阻塞，不绕过命令服务。停止不会把活动写成已经完成最终留出验证。

本地复用检查采用既有 Ledger 终态/Outbox、事件 worker、模型费用计算、活动幂等命令与原生启动服务；沿用已有 RD-Agent 分工和持久化迭代设计，不接入外部调度框架。tests/test_operator_auto_round.py 使用真实 SQLite、活动、产物、事件 worker 和命令接受事务，覆盖终态只发一次、只建一轮、两种中断恢复、暂停/取消/轮次和预算停止、已预留模型费用，以及就绪拒绝；启动测试的 readiness 为受控返回，不运行后续 graph worker。实际环境/Agent 配置就绪、生产模型/GPU 和页面闭环仍需真实验收，不能据此声称持续科研实验已运行。代码进入运行时前仍需受管刷新，本批不重启、不 push、不发布。
