...
- [ ] 若动过 auto-advance 策略模板（[INDEX](./INDEX.md) 的 `[research_workflow]`）或影子文档：已运行 `python scripts/deploy_auto_advance_policy.py`，模板报告 in sync 且 shadow source 无错误（模板是权威，运行时读部署副本；`--write` 才写入并备份）
