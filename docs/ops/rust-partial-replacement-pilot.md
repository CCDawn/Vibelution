...
| 优先级 | 候选 | ROI 理由 | 风险 | 状态 |
| --- | --- | --- | --- | --- |
| **P0** | LLM usage / cache 归一（Anthropic total + hit rate） | 纯函数、行为对账清晰；修已知与主流不一致；可双实现 parity | 低 | **完成** `vibelution-usage-normalize` |
| **P1** | 路径/工作区 containment（工具写路径是否逃逸 project root） | 安全边界、可单测、热路径 | 中 | **本轮** `vibelution-path-containment` |
...
## Pilot 1 设计：`vibelution-path-containment`

```text
tools/shell_tools.resolve_agent_tool_path
    │
    ▼
core/infrastructure/path_containment.py  (Python 参考 + 可选 Rust CLI)
    │
    ▼
crates/vibelution-path-containment
```

- **输入**：`projectRoot` + `candidate`（相对或绝对）
- **输出**：`ok`, `resolved`, `relative`, `error`（`outside_root` / `empty_*` / `null_byte`）
- **语义**：词法规范化（不要求路径已存在），拒绝 `..` 逃逸与 root 外绝对路径；Windows 比较大小写不敏感
- **接入**：`resolve_agent_tool_path` 优先用 containment 结果校验允许根
