...
### 2.2 Local Reuse And Risk-Scaled Mature-Scheme Research

Before writing, always locate and evaluate the local owning surface. External mature-scheme research is required when architecture, dependencies, a complex capability, or a real reuse fork could change the implementation path; it is not a fixed tax on an already-located small fix or mechanical edit.
...
2. **Survey mature external schemes when risk requires it.** Check GitHub, papers, official standards/docs, or equivalent practice when the result could change architecture, dependencies, boundaries, security, concurrency, data, Agent/Prompt/runtime behavior, or a complex path. A confirmed small bug or mechanical edit may stop at local evidence.

3. **Rank and extract.** Score local and external candidates by **project fit first** (ownership, VUI, no-console runtime, SSOT, license, maintenance cost), then by the slices actually worth borrowing (mechanism, contract, test idea). Rank them. Reuse only the top-fitting parts, adapting them into the owning surface. Do not copy a whole GitHub repo or paper implementation into Vibelution product code. For a high-value public GitHub analog, clone the GitHub default-branch tip (depth 1, no history) into the project-memory GitHub library (`activePaths.memory/github-projects/`) first, then research the local clone. Do not treat web pages or API payloads as the conclusion, and do not dump clone bodies into formal KnowledgeItem / RAG.

Stop condition: when external comparison is required, the ranked choice, borrowed slice, and reuse/adapt/replace decision must be explicit before writing. Use `briefbound-feature-reuse-research` only while that decision is genuinely open.

For implementation-bearing diffs, persist the decision before `closeout`. Use `record --mode LOCAL_ONLY --decision BUILD_IN_HOUSE` for a confirmed small fix/mechanical edit: record feature, local owner/reuse decision, reason, boundary, and verification without candidates or source refs. Use `--mode EXTERNAL` (default) when external comparison was required; it still requires ranked candidates, borrowed slices, rejected alternatives, and fixed source refs. Candidate URL/path/SHA/license/readiness are registry-hydrated, never hand-entered.

The record lives in Git common-dir `vibelution-cache/reuse_research/<task-id>.json`. `closeout` snapshots schema 3 before expensive validation; `verify-manifest` validates local-only fields directly and external mode against pinned candidate commits/blobs. Missing evidence returns `reuse_research_missing`; malformed or drifted evidence returns `reuse_research_invalid`. Docs/tests/fixtures/examples remain exempt.
...
The local quality gate is evidence-producing, not lifecycle-owning. `scripts/local_quality_gate.py closeout` runs against the clean task worktree and binds the current claim, validated local `main` SHA, task HEAD SHA, selector-selected commands, and merge preflight into a manifest. Development claims in either `active` or `ready` status remain valid covering claims. A `passed` manifest does not mean merged. The gate never performs the root merge or cleanup.
...
Every commit uses the tracked pre-commit hook and `scripts/local_quality_gate.py commit`. This light gate reads Git-index staged content for `git diff --cached --check` and Python fatal Ruff, and rejects staged/unstaged overlap on gate-definition paths. Gate behavior tests run once in final closeout rather than again at commit. If `core.hooksPath` is wrong, `scripts/doctor.ps1` reports the read-only repair hint `git config core.hooksPath .githooks`; the doctor must not silently write Git configuration.
...
When the gates pass, invoke `scripts/task_closeout.py` from root local `main` (never with the task worktree as process cwd). It validates once, waits only a bounded interval for `integration/main`, then re-verifies and fast-forwards. If the claim remains busy, the result returns `manifest_path` plus `next_action=retry_with_manifest`; retry with that manifest and do not rerun selector tests. `--reserve-integration` is allowed only after `stale_main`, after syncing latest `main`, and with the one-use `--stale-retry-token` returned by that failed attempt. A successful merge starts cleanup immediately. `merged_cleanup_pending` means merge already succeeded: run `--cleanup-only --branch <task-branch>` from `main`; never validate or merge again. Cleanup removes only provably task-owned resources and does not wait for post-merge validation.
...
- `<active-memory>/github-projects/INDEX.md` when the task needs an external GitHub analog (generated from `registry.json`; do not hand-edit). Clone the default-branch tip into `github-projects/repos/` before treating that code as evidence;
