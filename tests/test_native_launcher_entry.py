...
def test_native_launcher_tray_menu_exposes_simplified_controls():
    source = _source()
    ...
    assert "QueueRestartAll" in source
    assert "QueueExitLauncher(true)" in source
    assert 'menu.Items.Add(MenuItem("打开控制台"' not in source
