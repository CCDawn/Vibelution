...
def test_git_memory_maintenance_report_matches_python_builder(tmp_path):
    db_path = tmp_path / "agent_brain.db"
    _create_git_memory_db(db_path)

    report = build_git_memory_maintenance_report(db_path, keep_latest=2)

    assert report["ok"] is True
    assert report["pruneDryRun"]["candidateEntityRows"] == 6
    assert "accelerator" not in report
