...
def test_adapter_commit_emits_terminal_resume_without_phase_two_handoff(
    ...
    try:
        ...
        policy = load_stage_one_completion_policy()
        closeout_calls: list[str] = []

        def accepted_closeout(*_args, **_kwargs):
            closeout_calls.append("called")
            return StageOneCloseoutOutcome(
                completion_state="STAGE1_G1_ACCEPTED",
                policy_sha256=policy.policySha256,
                artifact_refs=("hypothesis_set://accepted",),
                receipt_stages=("generation", "review", "revision"),
                receipt_refs=("gen", "review", "revision"),
                human_gate_count=2,
            )

        monkeypatch.setattr(
            "core.web.services.team_workflow.research_runtime.adapter_dispatch_worker.evaluate_ledger_stage_one_closeout",
            accepted_closeout,
        )
