...
class TestToolExecutorInit:
    ...
    def test_research_knowledge_tool_is_registered_and_llm_facing(self):
        ...
        assert "github_project_library_search_tool" in canonical_names
        assert "github_project_library_search_tool" in llm_names
        assert "github_project_library_clone_tool" in canonical_names
        assert "github_project_library_clone_tool" in llm_names
...
*** End Patch""",
    ...
    def test_low_quality_search_result_records_degraded_runtime_scene_event(self, executor, monkeypatch):
        events = []
        ...
        executor.register_tool(
            "public_web_search",
            lambda query="": "[搜索质量不足] 公开搜索未返回可采信的结果。",
            timeout=5,
        )

        result, action = executor.execute("public_web_search", {"query": "predictive coding site:arxiv.org"})

        assert action is None
        assert str(result).startswith("[搜索质量不足]")
        event = events[-1]
        assert event["eventCode"] == "tool.execute.degraded"
        assert event["level"] == "warning"
        assert event["outcome"] == "degraded"
