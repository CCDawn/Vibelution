...
def test_scope_records_keeps_preformal_selection_referenced_by_formal_review(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A promoted formal run still owns the selection that opened its review."""
    ...
    monkeypatch.setattr(
        ...
        "_question_reset_snapshot",
        lambda *_args: {
            "targetMeetingIds": {"review-formal"},
            "targetRoundIds": set(),
            "chainRecords": [
                {
                    "recordKind": "review_round_link",
            ...
            "selectionRecords": [
                ...
                {
                    "questionId": "SCI-001",
                    "selectionId": "selection-unrelated",
                    "workflowRunId": "",
                    "selectedCandidateIds": ["candidate-other"],
                },
            ],
            "meetingRecords": [
                {
    ...
    assert [item["selectionId"] for item in sources["selection_records"]] == [
        "selection-preformal"
    ]
    state_module.clear_hypothesis_first_state_v2_cache()
...
def test_current_grounded_context_clears_stale_knowledge_blocker() -> None:
    state = _stage_one_projection(
        chain_records=[*_stage_one_draft_records("r0"), {
            "recordKind": "generation_attempt", "attemptId": "attempt-r0",
            "questionId": "SCI-091", "meetingRoundId": "r0", "attemptNumber": 1,
        ...
        formal_snapshots={"run-stage-one": {
            "activeNodeIds": ["hypothesis_design"],
            "stageOneGroundedContextReady": True,
            "nodeAttempts": {"hypothesis_design": [{
                "attempt": 1, "status": "blocked", "problem": {
                    "code": "auto_advance_not_ready",
                    "detail": "knowledge_package_not_materialized; hypothesis_round_unconverged; template_baseline_missing",
    ...
    assert "open-stage-one-generation" in [a["actionId"] for a in state["allowedActions"]]
