...
def test_team_workflow_route_writebacks_source_collection_stage_session_task(tmp_path, monkeypatch):
    ...
    assert "source_search_receipt_missing" in response.json()["detail"]
    assert "parent_query_id" in response.json()["detail"]
    ...
    assert not any(item["taskId"] == task_response.json()["taskId"] and item["status"] == "completed" for item in stage_tasks)
...
def test_team_workflow_routes_create_experiment_plan_draft(tmp_path, monkeypatch):
    _use_tmp_project_root(tmp_path, monkeypatch)
    client = _client()
    team = client.post("/api/teams", json={"name": "ai科学研究团队"}).json()
    candidate_response = client.post(
    ...
    status_response = client.get(f"/api/teams/{team['teamId']}/workflow-orchestration/experiments/status")
    plan_response = client.post(
        ...
        json={
            ...
            "hypothesisCandidateIds": [candidate_response.json()["candidate"]["candidateId"]],
        },
    )
...
def test_team_workflow_routes_register_experiment_baseline_artifact(tmp_path, monkeypatch):
    ...
    assert response.status_code == 201, response.text
    payload = response.json()
    assert payload["baselineArtifact"]["status"] == "registered"
    assert payload["plan"]["baselineSelection"]["activeBaselineReady"] is True
    assert payload["plan"]["readiness"]["readyForSmoke"] is True
    assert payload["plan"]["readiness"]["readyForFullRun"] is False
    assert payload["status"]["status"] == "ready_for_smoke"
    assert payload["stageRoundStatus"]["phases"][1]["latestRound"]["planningContract"]["readyForSmoke"] is True
    assert payload["boundaries"]["autoExecution"] is False
