...
const implementationBlueprint = {
  ...
  milestones: [
    ...
    ["M0.2", "Teams 科研流程二级导航", "左侧团队栏降级为团队创建、模板和自定义/科研团队列表；科研流程索引不再与团队同级，而是进入 ai科学研究团队主工作区顶部作为二级导航。监督进化团队与自进化团队不再出现在 Teams 自定义团队列表中。", "用户先选团队，再在选中团队内部切换科研总览、团队协调、知识入库、候选图谱、候选资料、团队沟通和组织画布；资料搜集从索引跳转到独立一级工作页，组织画布仍可编辑但属于附属视图；监督进化/自进化画布归各自模式页承载。"],
