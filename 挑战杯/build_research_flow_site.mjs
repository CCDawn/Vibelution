...
const knowledgeRunbook = {
  ...
  featureMatrix: [
    ["知识搜集一级工作台", "顶部展示当前判断、未完成任务、候选资料、搜索问题、缓存状态；5 个阶段卡用颜色区分进行中/完成/失败/待处理/未进行；每张阶段卡内嵌对应功能 Agent，点击阶段卡后底部只显示当前阶段子页", "已接入前端"],
...
const implementationBlueprint = {
  ...
  activeTeam: {
    ...
    defaultView: "选中 ai科学研究团队后，团队内容区显示科研控制台：研究主题、当前阶段、知识搜集/实验/迭代三张阶段卡、每卡一个主按钮和一个阶段详情入口；知识搜集卡会按未开始、待继续搜索、搜索中、待筛选、可进入实验、等待回写显示状态，并提供“新一轮搜集”；点击知识搜集主按钮会创建/续用阶段轮次，并立即排队第一批后台资料搜索；知识搜集独立页的资料搜集、资料筛选、候选入库、候选图谱、共享记忆前审五张步骤卡各自承载开始/查看/刷新按钮和对应功能 Agent 配置槽，资料筛选主按钮会让资料质量评估 Agent 批量执行待筛候选；若已筛完，则显示 Agent 重新筛选并 force 复审已筛候选；批次列表按 teamId/startedFrom 在服务端过滤，后台 activeWorkRun 会持续显示正在团队搜索，AppShell 顶部优先显示知识搜集而不是团队群聊；知识搜集页不再把本阶段 Agent 统一堆在顶部，点击每张步骤卡才查看该步骤内容和操作；资料搜集子页把当前动作和搜集结果分开：过程区只显示当前正在执行的一条动作，结果区集中展示已收集候选资料，候选状态徽标必须横向完整显示；知识搜集启动不再依赖旧的 houmo_qwen35_9b_agent 固定 ID，而是由后端从当前模型库解析可用 KV 模型；模型证据、候选、路径、校验和内部状态默认移到阶段详情页或高级信息中。",
  ...
  milestones: [
    ...
    ["M6.41", "步骤卡 Agent 内嵌槽", "知识搜集页移除顶部统一“本阶段 Agent”面板，资料搜集、资料筛选、候选入库、候选图谱、共享记忆前审五张步骤卡各自内嵌本步骤功能 Agent 计数、状态和配置入口。", "用户能在操作某个步骤前直接看到由哪些功能 Agent 承接，不再把所有 Agent 混放到一个全局区域；点击卡片仍进入对应子页，点击 Agent 标签进入 Agent 管理配置。"],
    ["M6.42", "资料搜集过程与结果分层", "知识搜集页的资料搜集子页改为两块：当前搜集动作只显示一条正在进行或最近完成的动作，搜集结果集中展示已进入 CandidateStore 的候选资料。", "顶部阶段卡和总览指标使用“已收集、待执行、搜集结果、搜索范围”等中文业务词，避免把 record/query/assignment 等内部计数暴露成主路径信息。"],
  ...
  services: [
    ...
    ["TeamsRoute workflow workspace", "已落地入口：Teams 页顶部固定横向切换条只暴露 AI 搜索范围团队和 ai科学研究团队，左侧栏已删除；监督进化/自进化模式页各自读取系统 Team 并展示只读团队画布；选择 ai科学研究团队后直接进入 research-team 专属科研控制台；总览只显示知识搜集、实验、迭代三张阶段卡，每张卡一个主操作和一个阶段详情入口。知识搜集卡会显示当前批次、待处理、候选和查询数，主按钮按状态执行开始、下一批搜索、筛选入口或打开工作台，另有新一轮搜集入口。知识搜集页位于 /teams?team=research-team&researchView=knowledge_collection，默认展示高价值状态摘要、状态驱动主操作和对话式 source collection 过程流；点击知识搜集主按钮会通过 stage-rounds/start 创建轮次并立即排队第一批后台 source collection 搜索；执行 source collection 搜索时会写入 source_collection_run 运行快照，让 AppShell 顶部“正在进行”优先显示知识搜集；资料搜集 run 列表通过 data-processing 服务端 teamId/startedFrom 过滤，避免被其它团队批次挤掉；后台 accepted 后 activeWorkRun 会进入控制台和对话流，直到 runStatus 收口；资料搜集、资料筛选、候选入库、候选图谱、共享记忆前审五张步骤卡各自带主按钮并作为页内子页索引，同时在卡片内部显示该步骤功能 Agent 的状态和配置入口；底部只显示当前阶段内容，右侧只显示当前步骤操作、关键数量和细节；资料筛选会调用资料质量评估 Agent 批量执行，完成后展开、滚动并高亮页内筛选详情；KV/prompt cache 由后端从当前模型库解析，不再依赖旧固定模型 ID；稳定前缀、动态增量、assignment、query、存储路径和手工回写保留在可展开详情中；默认层移除解释性小字；实验/迭代页先提供规划启动、轮次状态和边界说明；旧 source_collection 参数兼容映射到知识搜集。"],
  ...
  deliveredSlice: [
    ...
    ["科研阶段轮次启动", "/api/teams/{teamId}/workflow-orchestration/stage-rounds/start", "统一启动/续做/开启新一轮知识搜集、实验、迭代；知识搜集复用 source-collection run，实验和迭代先生成 planning 轮次，并尝试自动启动 linkedChatRoom 轻量团队协调 round。"],
    ...
    ["阶段协调重试", "/api/teams/{teamId}/workflow-orchestration/stage-rounds/{stageRoundId}/coordination/retry", "当群聊缺失、忙碌或没有可发言成员导致阶段进入 needs_attention 时，可重试启动团队协调 round；成功后写回 coordinationRoomId/coordinationRoundId。"],
...
const knowledgeNodeRunbook = {
  "01": {
    ...
    agentStatus: "可复用通用 data-processing assignment / source-extraction / source-quality 后端已接入；Teams 资料搜集执行台、候选资料筛选台、科研总览控制台、KV 缓存对话轨迹、启动复用反馈、metadata-only 搜索执行闭环、高价值信息层、中文控制台、默认层去说明化、步骤卡主操作入口、步骤卡 Agent 内嵌槽、资料筛选页内反馈、资料筛选 Agent 批量执行、候选库可视化二级索引、知识搜集阶段子页控制台、资料筛选 Agent 复审入口、一键排队首批后台搜索、资料搜集当前动作和搜集结果分层、搜集结果状态徽标完整显示已接入",
    features: ["ResearchStageRound 建立", "科研控制台一键进入知识搜集", "总览页只显示三阶段主操作", "知识搜集状态驱动按钮", "知识搜集中文展示映射", "默认层去说明化", "右侧主操作唯一化", "资料筛选入口常驻", "步骤卡主操作入口", "步骤卡 Agent 内嵌槽", "步骤卡二级索引", "阶段子页控制台", "资料搜集当前动作", "搜集结果独立区", "搜集结果状态徽标", "资料筛选 Agent 复审", "候选库可视化面板", "资料筛选页内详情", "资料筛选展开高亮反馈", "资料筛选 Agent 批量执行", "source-quality/assess-batch", "source-quality force 重筛", "新一轮搜集入口", "active 知识搜集轮次复用反馈", "一键排队首批搜索", "全局运行态知识搜集优先", "步骤功能 Agent 聚焦", "自动启动团队协调 round", "DataProcessingRun 建立", "DataProcessingRun reviewing 收口", "DataSearchPlan/querySeeds 搜索计划", "KV/prompt cache 启动门禁", "稳定前缀/动态增量契约", "CollectionAssignment 资料搜集任务", "assignedQueries 角色分配", "promptCachePartition 角色分区", "Teams 启动搜集批次", "Teams 查看 run/assignment/query/KV 状态", "Teams 高价值摘要层", "Teams 搜索下一批", "Crossref 元数据搜索执行器", "搜索执行事件进入搜集对话流", "DataRecord 通用记录", "DataRecord 自动导入 source_manifest", "metadata-only 下载边界", "手工 CollectionOutput 回写", "本地资料登记", "source_manifest 写入", "文件路径/页码/来源可信度记录", "本地 PDF sha256/pageAnchors/excerpt 抽取", "sourceQualityAssessment 筛选", "可选 research_knowledge_query_tool 查重"],
