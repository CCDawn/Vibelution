...
const implementationBlueprint = {
  ...
  milestones: [
    ...
    ["M6.66", "阶段卡 Agent 会话任务桥", "知识搜集五张步骤卡的主按钮不再直接执行本地阶段动作，而是创建对应功能 Agent 的 direct session task 并跳转到会话页执行。", "任务落盘到 source_collection_runs/<runId>/stage_session_tasks.json；Chat URL 保留 returnTo/returnLabel；Agent 完成后必须通过结构化 writeback API 回写 sourceCollectionStageSessionTasks，团队阶段再展示结果。"],
  ...
  deliveredSlice: [
    ...
    ["阶段 Agent 会话任务启动", "/api/teams/{teamId}/workflow-orchestration/source-collection-runs/{runId}/stage-session-tasks", "根据 stageId/agentId/agentRole 创建 source_collection_stage_session_task，把任务消息投递到 Agent directSessionId，并返回带 returnTo/returnLabel 的 chatRoute；每次用户点击默认新建任务，只有显式相同 idempotencyKey 才复用同一次请求；不写正式 Team Knowledge/RAG/official graph。"],
    ["阶段 Agent 会话任务回写", "/api/teams/{teamId}/workflow-orchestration/stage-session-tasks/{taskId}/writeback", "只接受结构化 status/summary/result/evidenceRefs/nextActions，把结果写入 stage_session_tasks.json 并同步 ResearchStageRound.sourceCollectionStageSessionTasks；自然语言会话不是团队阶段结果来源。"],
