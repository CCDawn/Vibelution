...
const implementationBlueprint = {
  ...
  deliveredSlice: [
    ...
    ["服务文件", "core/web/services/team_workflow_orchestration_service.py", "Team 编排、资料搜集批次启动、候选登记、DataRecord 批量/单条导入、资料质量筛选、paper_note 分块计划、模型证据登记、转移请求、协调 Agent 裁决和轻量运行事件日志。"],
    ["路由文件", "core/web/routes/team_workflows.py", "提供工作流查询/确保、资料搜集批次启动、候选资料登记、DataRecord 单条导入、资料提炼批量执行、资料质量筛选状态/写回、paper_note 分块状态/计划、模型证据状态/登记、转移提交、转移裁决 API。"],
...
const currentResearchRun = {
  ...
  nextActions: [
    ...
    "资料提炼 Agent 为 source_quality_approved 来源补 page/citation anchors；Paper Note Extraction Agent 先生成 paperNoteChunkPlan，再按 chunkId 生成 paper_note 草稿。",
    ...
    <main class="shell">
      ...
      <section id="overview" class="review-dashboard">
        ...
        <aside class="dashboard-panel">
          <h2>审核摘要</h2>
          <div class="kpi-grid">
            <div class="kpi"><b>当前阶段</b><strong>M6.67</strong><span>知识搜集阶段卡每次点击都会投递新的 Agent 私聊任务，并通过结构化 writeback 回团队阶段。</span></div>
