...
const nodes = [
  ...
  },
  {
    ...
    summary: "steward_pack_draft 先作为团队待入库知识包暂存，搜集流程向知识库管理员发送入库请求并唤醒它处理最终入库。",
    ...
    actions: [
      ...
      "合格草稿先进入 steward_pack_draft，作为团队提炼后的待入库知识包暂存。",
      "知识搜集入库入口默认向知识库管理员 inbox 发送 challenge_cup_knowledge_ingestion_request 并尝试唤醒该 Agent。",
      "知识库管理员收到请求后再提交 Team Knowledge 待审队列，进入 steward_pending_knowledge_review。",
    ...
    outputs: ["steward_pack_draft 候选", "knowledge_steward_inbox_request", "SourceArtifact", "pending RefinementProposal", "proposal ratingSuggestion", "KnowledgeItem ratingSuggestion", "officialResearchGraph", "official_sync_record", "knowledge_ingestion_status", "coordination_status_panel"],
    memory: "待入库包通知阶段不创建正式 KnowledgeItem；知识库管理员后续审批通过后复用 Team Knowledge review/apply 创建正式 KnowledgeItem；状态总览只读聚合 formalKnowledgeItemCount、pendingProposalCount 和 Agent 通知结果。",
...
const knowledgeRunbook = {
  ...
  stateFlow: [
    ...
    ["knowledge_steward_agent_notified", "已通知知识库管理员", "steward_pack_draft 已写入 CandidateStore，并向知识库管理员 inbox 发送入库请求。"],
    ["steward_pending_knowledge_review", "待审入库对象", "知识库管理员已创建 SourceArtifact、pending RefinementProposal，可选 pending ratingSuggestion；等待审批门禁。"],
  ...
  artifacts: [
    ...
    ["steward_ingestion_pack_YYYYMMDD_NNN.json", "知识治理摄取包", "治理建议、评级建议、目标知识域和正式入库申请；默认先作为待入库包通知知识库管理员。"],
    ["agent_inbox_messages.jsonl", "知识库管理员入库请求", "challenge_cup_knowledge_ingestion_request 消息记录 teamId、stewardPackCandidateId、knowledgeBaseId 和最终入库边界。"],
  ...
  gates: [
    ...
    ["Agent 通知门", "资料搜集流程只发送入库请求并激活知识库管理员，不直接创建 SourceArtifact/Proposal/KnowledgeItem", "knowledge_steward_agent_notified", "停在 steward_pack_draft"],
    ["治理门", "知识库管理员收到请求后只创建 SourceArtifact + pending RefinementProposal + 可选 pending ratingSuggestion，不直接创建 KnowledgeItem", "steward_pending_knowledge_review", "退回 steward_needs_revision"],
  ...
  teamWorkflowOrchestration: {
    ...
    landedApis: [
      ...
      ["POST", "/api/teams/{team_id}/workflow-orchestration/knowledge-collection/ingest", "批量审查候选、生成候选图谱和 steward_pack_draft；默认只发送知识库管理员入库请求并唤醒 Agent，不直接正式入库。"],
...
const implementationBlueprint = {
  ...
  milestones: [
    ...
    ["M6.52", "候选图谱与前审一键 Agent 节点", "候选图谱步骤卡从刷新改为 Agent 生成图谱，只使用已通过资料质量筛选的候选；知识库管理员入库审核步骤卡从检查状态改为 Agent 完成前审，自动汇总通过候选和候选图谱生成 steward_pack_draft 前审包。", "这两个节点都不要求用户逐条选择资料；噪声候选会被自动过滤，产物仍停在 CandidateStore candidate-only 层，不写正式 Team Knowledge、不写 RAG、不写 official graph。"],
    ...
    ["M6.55", "知识搜集四步闭环入库", "知识搜集阶段从旧五步收敛为搜索资料、资料提炼、资料审查、资料入库四张用户步骤卡；候选关系图和入库包不再作为独立用户阶段，而是合并进资料入库的 Agent 动作。", "第一阶段可从资料搜索结果推进到待入库知识包：搜索资料写 DataRecord/source_manifest，资料提炼展示候选资料，资料审查批量写 sourceQualityAssessment，资料入库复用候选关系和 steward_pack_draft，并向知识库管理员发送入库请求；正式 Team Knowledge/RAG/official graph 仍由知识库管理员后续走审核门禁。"],
    ...
    ["M6.58", "资料入库入口贯通候选资料", "资料入库步骤不再只看已通过资料数量；只要本轮已有候选资料，就允许点击 Agent 提炼并通知知识库管理员。", "前端把候选资料数作为待入库包输入上限；后端 knowledge-collection/ingest 会先调用资料审查，再生成候选关系和 steward_pack_draft，并通过 Agent Inbox 向知识库管理员发送入库请求，避免用户被卡在必须先手动跑完每个中间门禁，同时不绕过最终知识库管理员。"],
  ...
  services: [
    ...
    ["memory_ingestion_bridge", "已复用 Agent Inbox、Team Knowledge create_ingestion_package、review_refinement_proposal、rating suggestion review/create 和 KnowledgeItem metadata patch；steward_pack_draft 默认先通知知识库管理员，Agent 后续可提交 pending proposal，审批通过后创建正式 KnowledgeItem、迁移待审评分建议，并写入 officialResearchGraph 正式科研边。"],
...
const knowledgeNodeRunbook = {
  ...
  "07": {
    ...
    localModelUse: "本地 9B 模型只能生成 steward_pack_draft 草稿，正式建议仍由知识库管理员检查；禁止在输出中请求正式写入。",
