...
const nodes = [
  ...
  },
  {
    ...
    actions: [
      ...
      "优先使用 sourceExtraction.excerpt 作为本地研究模型 evidence 输入。",
...
const knowledgeRunbook = {
  ...
  projectAlignment: [
    ...
    ["PDF/本地资料解析", "Team Workflow 已新增 source-extraction API；知识 ingestion 工具仍要求传入已有 excerpt/source_ref，不负责解析 PDF", "部分接入", "01 已能把本地 PDF 解析成 sourceExtraction.pageAnchors/excerpt；02 自动 paper_note 生成链路仍待接。"],
...
const implementationBlueprint = {
  ...
  milestones: [
    ...
    ["M2", "paper_note 与 PDF 锚点", "已新增 paper_note 输出契约与 Citation Anchor 校验：paper_note_draft 必须含 keyFindings、methods、limitations、citations；每条 keyFinding 必须有 sourceRef 和 page/citation/evidenceRef。本地 PDF 页码锚点可由 source-extraction 提供。", "合格本地模型输出进入 paper_note_draft；缺 citation/page anchor 时进入 paper_note_needs_revision，不能自然推进到 mechanism_candidate；自动从 excerpt 生成 paper_note 的端到端链路仍待接。"],
  ...
  services: [
    ...
    ["team_workflows API", "已落地：/api/teams/{team_id}/workflow-orchestration、candidates/source、candidates/{candidate_id}/source-extraction、candidates、candidates/validation、candidate-graph、transfers、decide。"],
...
const knowledgeNodeRunbook = {
  "01": {
    ...
    agentStatus: "可复用 research_broad / source-extraction 后端已接入",
    features: ["本地资料登记", "source_manifest 写入", "文件路径/页码/来源可信度记录", "本地 PDF sha256/pageAnchors/excerpt 抽取", "可选 research_knowledge_query_tool 查重"],
    tools: ["research_knowledge_query_tool", "/api/teams/{teamId}/workflow-orchestration/candidates/{candidateId}/source-extraction"],
    localModelUse: "本地 9B 模型适合做标题/摘要/片段初筛，输出 relevanceScore、topicTags、excludeReason，不直接决定正式纳入。",
    humanGate: "确认资料允许分析、来源可信度和是否纳入本轮。",
    gap: "本地 PDF 页码锚点提取已接入 CandidateStore；长 PDF 自动分批、章节识别和非 PDF 资料解析仍待补。",
    entry: "用户提供 PDF、论文、赛题或补充资料；文件可在挑战杯工作区定位。",
    operation: "建立 source_manifest 记录；对本地 PDF 运行 source-extraction，计算 sha256、生成 pageAnchors/excerpt，不改写原文，不自动混入联网资料。",
    exit: "每个 PDF 资料项都有 id、path、type、allowedForAnalysis、sha256、pageScope 和 sourceExtraction.pageAnchors。",
    fallback: "路径缺失、来源不明、权限不清、抽取失败或解析器不可用时停在 source_needs_confirmation。",
  },
  "02": {
    ...
    tools: ["research_knowledge_query_tool", "sourceExtraction.excerpt", "本地研究模型 task/output API"],
    localModelUse: "本地 9B 模型适合按章节或 chunk 生成 paper_note 草稿；32k 内保留 18k-22k 原文证据和输出预留。",
    humanGate: "必要时确认摘要是否保守、是否遗漏关键章节。",
    gap: "paper_note schema 与页码引用校验已接入 CandidateStore；本地 PDF 页码摘录已可由 source-extraction 提供，长文拆分和自动生成 paper_note 仍待接。",
