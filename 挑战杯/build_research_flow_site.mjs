...
const implementationBlueprint = {
  ...
  milestones: [
    ...
    ["M6.42", "资料搜集过程与结果分层", "知识搜集页的资料搜集子页改为两块：当前搜集动作只显示一条正在进行或最近完成的动作，已收集资料集中展示进入 CandidateStore 的候选资料。", "顶部阶段卡和总览指标使用“已搜到、搜索问题、候选资料、已通过筛选、待 Agent 筛选”等中文业务词，避免把 record/query/assignment 等内部计数暴露成主路径信息。"],
...
const knowledgeNodeRunbook = {
  "01": {
    ...
    agentStatus: "可复用通用 data-processing assignment / source-extraction / source-quality 后端已接入；Teams 资料搜集执行台、候选资料筛选台、科研总览控制台、KV 缓存对话轨迹、启动复用反馈、metadata-only 搜索执行闭环、高价值信息层、中文控制台、默认层去说明化、步骤卡主操作入口、右侧当前步骤 Agent 配置、资料筛选页内反馈、资料筛选 Agent 批量执行、候选库可视化二级索引、知识搜集阶段子页控制台、资料筛选 Agent 复审入口、一键排队首批后台搜索、资料搜集当前动作和已收集资料分层、搜集结果状态徽标完整显示、搜集结果 DOI/网页/本地文件来源可溯源、阶段协调显式启动已接入",
    features: ["ResearchStageRound 建立", "科研控制台一键进入知识搜集", "总览页只显示三阶段主操作", "知识搜集状态驱动按钮", "知识搜集中文展示映射", "默认层去说明化", "右侧主操作唯一化", "资料筛选入口常驻", "步骤卡主操作入口", "右侧当前步骤 Agent 配置", "步骤卡二级索引", "阶段子页控制台", "资料搜集当前动作", "已收集资料结果区", "搜集结果来源溯源", "搜集结果状态徽标", "资料筛选 Agent 复审", "候选库可视化面板", "资料筛选页内详情", "资料筛选展开高亮反馈", "资料筛选 Agent 批量执行", "source-quality/assess-batch", "source-quality force 重筛", "新一轮搜集入口", "active 知识搜集轮次复用反馈", "一键排队首批搜索", "全局运行态知识搜集优先", "步骤功能 Agent 聚焦", "阶段协调显式启动", "DataProcessingRun 建立", "DataProcessingRun reviewing 收口", "DataSearchPlan/querySeeds 搜索计划", "KV/prompt cache 启动门禁", "稳定前缀/动态增量契约", "CollectionAssignment 资料搜集任务", "assignedQueries 角色分配", "promptCachePartition 角色分区", "Teams 启动搜集批次", "Teams 查看 run/assignment/query/KV 状态", "Teams 高价值摘要层", "Teams 搜索下一批", "Crossref 元数据搜索执行器", "搜索执行事件进入搜集对话流", "DataRecord 通用记录", "DataRecord 自动导入 source_manifest", "metadata-only 下载边界", "手工 CollectionOutput 回写", "本地资料登记", "source_manifest 写入", "文件路径/页码/来源可信度记录", "本地 PDF sha256/pageAnchors/excerpt 抽取", "sourceQualityAssessment 筛选", "可选 research_knowledge_query_tool 查重"],
