...
const nodes = [
  ...
  },
  {
    ...
    outputs: ["steward_pack_draft 候选", "SourceArtifact", "pending RefinementProposal", "pending ratingSuggestion", "official_sync_record"],
    memory: "待审阶段不创建正式 KnowledgeItem；审批通过后复用 Team Knowledge review/apply 创建正式 KnowledgeItem。",
    graph: "审批通过后正式 KnowledgeItem 可经 Memory Graph 视图展示；本轮不另写挑战杯专用正式图谱边文件。",
    risks: ["绕过审批门禁", "目标知识域错误", "把 ingestion pack 草稿误当正式入库记录"],
    openQuestions: ["下一步是否把 pending ratingSuggestion 自动转成 KnowledgeItem rating review。"],
  },
  {
    id: "08",
...
const knowledgeRunbook = {
  ...
  artifacts: [
    ...
    ["steward_ingestion_pack_YYYYMMDD_NNN.json", "知识治理摄取包", "治理建议、评级建议、目标知识域和正式入库申请。"],
  ...
  teamWorkflowOrchestration: {
    ...
    landedApis: [
      ...
      ["POST", "/api/teams/{team_id}/workflow-orchestration/steward-packs/{candidate_id}/knowledge-ingestion/review", "审批 pending steward pack；approved 复用 Team Knowledge review/apply 创建正式 KnowledgeItem 并写 officialSyncRecord，rejected 退回 steward_needs_revision。"],
    ],
    integrations: [
...
const implementationBlueprint = {
  ...
  milestones: [
    ...
    ["M5", "知识治理与正式同步", "steward_pack_draft 门禁、待审入库桥和 Ingestion Approval Gate 已落地：有效草稿可提交为待审对象，批准后创建正式 KnowledgeItem，拒绝后退回修订。", "正式 RAG/Graph 通过已审核 KnowledgeItem 的现有读取面可检索/可见；本轮不另写挑战杯专用图谱边或向量索引。"],
  ],
  schemas: [
    "source_manifest",
    "paper_note",
  ...
  services: [
    ...
    ["memory_ingestion_bridge", "已复用 Team Knowledge create_ingestion_package 与 review_refinement_proposal；steward_pack_draft 可进入 pending proposal，审批通过后创建正式 KnowledgeItem。"],
  ],
  communicationRoles: [
  ...
  deliveredSlice: [
    ...
    ["steward_pack 审批门禁", "/api/teams/{teamId}/workflow-orchestration/steward-packs/{candidateId}/knowledge-ingestion/review", "只审批 steward_pending_knowledge_review；approved 创建正式 KnowledgeItem 并进入 official_synced，rejected 退回 steward_needs_revision。"],
...
const knowledgeNodeRunbook = {
  ...
  "07": {
    ...
    features: ["sourceTrace 校验", "proposalPayload", "ratingSuggestion", "approvalRequired=true", "SourceArtifact", "pending RefinementProposal", "officialSyncRecord", "official write 禁止项"],
    ...
    gap: "待补 pending ratingSuggestion 到正式 KnowledgeItem rating 的自动承接，以及更细的正式图谱边生成规则。",
    entry: "review_record 或候选图谱已给出可追溯 candidateIds，且具备目标知识域。",
    operation: "生成带 sourceTrace、riskSummary、proposalPayload、ratingSuggestion、approvalRequired=true 的 CandidateStore 草稿包，并可提交到指定 knowledgeBaseId 的待审入库队列；审批门禁复用 Team Knowledge review/apply。",
    exit: "合格草稿提交后进入 steward_pending_knowledge_review；批准后进入 official_synced 并记录 officialSyncRecord，拒绝后进入 steward_needs_revision。",
  ...
  "09": {
    ...
    features: ["正式审核确认", "正式 KnowledgeItem 创建/同步", "RAG 可检索", "official_sync_record", "Memory Graph 可见"],
    tools: ["knowledge_query_tool", "knowledge_rag_retrieve_tool", "Team Knowledge 审核/应用流程（平台能力）"],
    humanGate: "必须通过授权审批门禁批准 ingestion pack。",
    gap: "需要更细的正式 supports/maps_to/inspires 图谱边生成规则，以及 ratingSuggestion 自动迁移到 KnowledgeItem 评级。",
    entry: "授权审批门禁已批准 pending RefinementProposal。",
    operation: "复用 Team Knowledge review/apply 创建 KnowledgeItem；RAG 通过 reviewed Team Knowledge 检索，Memory Graph 通过正式 KnowledgeItem 视图展示。",
    exit: "knowledgeItemIds、ragStatus、graphStatus、reviewedByAgentId 和 officialSyncRecord 均已记录。",
