...
const nodes = [
  ...
  },
  {
    ...
    summary: "在交给记忆平台前先做领域与工程风险过滤，并把返工/拒绝/待转移纳入团队协调队列。",
    ...
    actions: [
      ...
      "GET coordination/status 只读聚合 pendingTransfers、needsRework、blocked、stewardship 和 active 队列，供团队协调/组织层分派下一步。",
    ],
  ...
  },
  {
    ...
    actions: [
      ...
      "Teams 工作台同时读取 coordination/status，把待转移、待返工、待治理和阻塞候选集中显示，但不提供自动调转按钮。",
    ],
...
const knowledgeRunbook = {
  ...
  featureMatrix: [
    ...
    ["团队协调队列", "coordination/status 聚合 pendingTransfers、needsRework、stewardship、blocked、active 队列；Teams 科研流程面板已显示协调状态与只读策略边界", "已接入前端/API"],
  ...
  projectAlignment: [
    ...
    ["团队协调聚合", "Team Workflow 新增 coordination/status；读取 CandidateStore + transfer_records，按 pendingTransfers/needsRework/stewardship/blocked/active 分队列", "已接入首版", "适合作为组织人员和协调人员的只读工作面；本轮不做自动调转、不要求用户确认、不写正式知识。"],
...
const implementationBlueprint = {
  ...
  milestones: [
    ...
    ["M6.1", "团队协调状态队列", "coordination/status 只读聚合 API 和 Teams 工作台协调队列已接入，把 pendingTransfers、needsRework、stewardship、blocked、active 汇成队列、summary、actionItems 和 coordinationPolicy。", "Research Coordination Agent/组织层可集中看到待决转移、返工候选、治理待审和阻塞项；本轮不自动调转，不提供审批按钮，不要求用户确认。"],
  ],
  schemas: [
    "source_manifest",
    "paper_note",
  ...
  services: [
    ...
    ["team_workflows API", "已落地：/api/teams/{team_id}/workflow-orchestration、candidates/source、candidates/{candidate_id}/source-extraction、candidates/{candidate_id}/paper-note-draft、candidates、candidates/validation、candidate-graph、transfers、decide、knowledge-ingestion/status、coordination/status。"],
    ["TeamsRoute workflow panel", "已落地只读入口：research-team 右侧 inspector 展示 workflow 当前阶段、candidateStore 摘要、coordination queue、knowledge ingestion status、validationSummary、候选图谱和最近候选，不做状态写操作。"],
