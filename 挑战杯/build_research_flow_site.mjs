...
const nodes = [
  ...
  },
  {
    ...
    status: "待知识治理审查",
    statusKind: "active",
    role: "知识库管理员 · agent-knowledge-steward",
    summary: "知识库管理员在 memory 阶段审核本轮候选；approved 回写会由后端经 Team Knowledge 治理门禁创建正式 KnowledgeItem。",
    objective: "把已通过资料质量筛选的本轮候选交给知识库管理员审核，并通过阶段回写触发 SourceArtifact、pending RefinementProposal、review/apply 和正式 KnowledgeItem 闭环。",
    ...
    actions: [
      ...
      "只采纳本轮 source_manifest 且 source_quality approved 的候选，拒绝跨轮或未质检通过资料进入正式链路。",
      "禁止绕过 source_collection_stage_writeback_tool、Team Knowledge source review、proposal review/apply gate 直接写库。",
      "知识搜集入库入口进入知识库管理员 direct session task，而不是只发送 inbox 请求后等待人工二次启动。",
      "知识库管理员在 memory 阶段回写 approved 后，后端构建或复用 steward_pack_draft，并提交 Team Knowledge 待审队列。",
    ...
    memory: "知识库管理员 memory 阶段 approved 回写会经治理门禁创建正式 KnowledgeItem；非知识库管理员阶段仍只写候选/任务结果，不写正式 Team Knowledge、RAG 或 official graph。",
...
const knowledgeRunbook = {
  ...
  stateFlow: [
    ...
    ["knowledge_steward_agent_notified", "知识库管理员任务已投递", "阶段任务已投递到知识库管理员 direct session，等待 memory 阶段结构化回写。"],
    ["steward_pending_knowledge_review", "待审入库对象", "知识库管理员 approved 回写已创建 SourceArtifact、pending RefinementProposal，可选 pending ratingSuggestion，并推动审批门禁进入正式入库闭环。"],
  ...
  artifacts: [
    ...
    ["steward_ingestion_pack_YYYYMMDD_NNN.json", "知识治理摄取包", "治理建议、评级建议、目标知识域和正式入库申请；由知识库管理员 memory 阶段 approved 回写触发后端构建或复用。"],
    ["stage_session_tasks.json", "知识库管理员阶段任务", "记录 teamId、runId、stageId=memory、agentRole=knowledge_steward、approved candidateIds 和 materializedKnowledgeIngestion 闭环结果。"],
  ...
  gates: [
    ...
    ["知识库管理员任务门", "资料入库阶段必须投递给 knowledge_steward 的 memory stage-session task，并通过 source_collection_stage_writeback_tool 回写 approved 候选", "knowledge_steward_agent_notified", "停在 steward_pack_draft"],
  ...
  teamWorkflowOrchestration: {
    ...
    landedApis: [
      ...
      ["POST", "/api/teams/{team_id}/workflow-orchestration/knowledge-collection/ingest", "批量审查候选、生成候选图谱和 steward_pack_draft；旧聚合入口只准备治理证据，正式入库以知识库管理员 memory 阶段 writeback 或 steward pack review gate 为准。"],
  ...
  roles: [
    ...
    ["知识库管理员 / agent-knowledge-steward", "审核资料入库步骤；approved 回写触发治理门禁和正式 KnowledgeItem 闭环，不绕过审核。"],
  ...
  projectAlignment: [
    ...
    ["知识治理 Agent", "agent-knowledge-steward 固定存在；ToolPolicy 含 knowledge proposal/ingestion/governance/rating 工具；边界为知识库管理员受治理门禁入库", "高度匹配", "07 节点已对齐 Team Knowledge 待审摄取包；只有知识库管理员的 memory 阶段 approved 回写可以触发后端治理门禁并形成正式入库闭环。"],
...
const implementationBlueprint = {
  ...
  activeTeam: {
    ...
    note: "挑战杯科研流程直接绑定当前 Vibelution ai科学研究团队，不另建新团队；Teams 页顶部只保留 AI 搜索范围团队和 ai科学研究团队两个固定入口，左侧栏删除，不再展示创建团队、模板创建或完整团队列表；选择 ai科学研究团队后直接进入团队专属科研工作台，默认只显示科研控制台：知识搜集、实验、迭代三张阶段卡各保留一个主操作和阶段详情入口；知识搜集阶段统一收敛为搜索资料、资料筛选、候选入库、候选图谱、知识库管理员入库审核五张步骤卡，组织画布仅保留为附属结构视图；知识搜集一级工作台只展示当前判断、关键数量、阶段颜色和下一步按钮，详细追踪放入详情区。",
    workspaceEntry: "/teams?team=research-team",
    sourceCollectionEntry: "/teams?team=research-team&researchView=knowledge_collection",
    defaultView: "选中 ai科学研究团队后，团队内容区显示科研控制台：研究主题、当前阶段、知识搜集/实验/迭代三张阶段卡、每卡一个主按钮和一个阶段详情入口；知识搜集卡会按未开始、待继续搜索、搜索中、待审查、可入库、可进入实验、等待回写显示状态，并提供“新一轮搜集”；点击知识搜集主按钮会创建/续用阶段轮次，并在需要新 run 时排队第一批后台资料搜索；知识搜集状态读取会根据最新 source_collection_run 终态自动修复旧 running，避免页面假显示 Agent 仍在运行；知识搜集独立页展示搜索资料、资料筛选、候选入库、候选图谱、知识库管理员入库审核五张步骤卡，每张卡只承载状态、关键数量、主操作和 Agent 私聊入口，当前步骤的功能 Agent 名称、模型、状态和配置入口集中显示在右侧详情底部；五张步骤卡的主按钮会启动对应功能 Agent 的 direct session task，带 returnTo/returnLabel 跳转到会话页执行，并要求 Agent 完成后通过 source_collection_stage_session_task_writeback 结构化回写到团队阶段；每次点击步骤卡主按钮都会创建新的私聊任务消息，搜索/提炼/审查/入库结果继续叠加到同一候选资料仓库并由既有去重收口；旧本地执行按钮和手工回写只作为详情页/调试兜底入口保留，不再是阶段卡主路径。资料审查任务由资料质量评估 Agent 在会话内处理待审候选或复审结论；入库审核任务由知识库管理员在会话内检查待入库门槛、生成审核结论和下一步建议；候选图谱任务由候选图谱 Agent 在会话内生成候选关系快照；正式知识由知识库管理员后续处理；批次列表按 teamId/startedFrom 在服务端过滤，后台 activeWorkRun 会持续显示正在团队搜索，AppShell 顶部优先显示知识搜集而不是团队群聊；知识搜集页不再把本阶段 Agent 统一堆在顶部，点击每张步骤卡才查看该步骤内容和操作；搜索资料子页把当前动作和原始资料记录分开：过程区只显示当前正在执行的一条动作，结果区只展示本轮 DataRecord 原始记录，并明确标出已入候选多少、还有多少待导入候选；资料筛选和候选入库子页单独展示本轮 source_manifest 候选资料，避免把原始记录数和候选数混在一起；搜索资料结果列表和资料筛选/候选入库列表都有独立纵向滚动区，分页控件保留在列表下方，长批次不会被外层布局裁掉；Agent 重新提炼只重新同步 DataRecord 到候选资料，不会自动完成资料审查，候选仍会显示为待 Agent 审查并引导进入资料审查步骤；资料结果统计使用原始记录、已入候选、可点击来源和本地文件，列表内 DOI 单独显示为 DOI 链接，网页显示可点击来源，本地文件显示路径，缺少来源直接标出；每条资料卡片可点击选中，右侧详情把资料来源和搜集证据分开：主操作只打开论文 DOI、网页来源或本地文件，OpenAlex/arXiv/GitHub 等公开索引或机器 API 放入“查看搜索证据”折叠区，并保留打开批次目录、搜索步骤、搜集记录和候选镜像入口；候选 metadata 统一保留 sourceRunId、sourceRecordId、sourceCategory、doi、sourceUrl、sourcePath、query、assignmentId 和 searchProvider，供五个子页过滤与溯源；知识搜集启动不再依赖旧的 houmo_qwen35_9b_agent 固定 ID，而是由后端从当前模型库解析可用 KV 模型；过程流把 KV 缓存模型和执行 Agent 模型分开展示，避免把缓存兜底模型误认为搜索资料 Agent；模型证据、候选、路径、校验和内部状态默认移到阶段详情页或高级信息中。",
  ...
  milestones: [
    ...
    ["M6.30", "步骤卡主操作入口", "知识搜集独立页移除顶部操作条和右侧下一步面板的重复按钮，把资料搜集、资料筛选、候选入库、候选图谱、知识库管理员入库审核五个步骤卡改成主操作入口。", "点击资料筛选不再跳回通用候选页，而是在知识搜集工作台内打开页内筛选详情；每个步骤继续按进行中、已完成、失败、未进行、待处理显示客观状态，正式知识/RAG/official graph 写入边界不变。"],
    ...
    ["M6.33", "候选库可视化二级索引", "知识搜集阶段的资料搜集、资料筛选、候选入库、候选图谱、知识库管理员入库审核五张步骤卡都可点击进入对应页内详情；候选入库主按钮改为查看可视化候选库，不再直接打开 candidate_store/index.json。", "底层 candidate_store/index.json 仍保留在“更多证据文件”中作为维护入口；用户主路径只看到候选卡片、筛选状态、图谱预览和知识库管理员入库审核，不再被 JSON 文件打断。"],
    ["M6.35", "知识搜集阶段子页控制台", "知识搜集阶段的五张步骤卡升级为页内子页索引，点击资料搜集、资料筛选、候选入库、候选图谱或知识库管理员入库审核时，底部只显示该阶段内容。", "右侧控制台按当前阶段切换主操作按钮和关键数量；资料搜集保留配置/批次/回写折叠区，其余阶段只显示对应查看、筛选、刷新或前审动作，避免所有阶段信息同屏堆叠。"],
    ...
    ["M6.41", "步骤 Agent 右侧配置槽", "知识搜集页移除顶部统一“本阶段 Agent”面板，也不再把 Agent 列表塞进五张步骤卡；资料搜集、资料筛选、候选入库、候选图谱、知识库管理员入库审核的功能 Agent 跟随当前步骤显示在右侧详情底部。", "用户操作某个步骤时，只在对应详情里看到该步骤 Agent 的职责、Agent 名称、模型、状态和配置入口；顶部步骤卡只保留状态、数量和主按钮，避免重复信息挤占流程阅读空间。"],
    ...
    ["M6.44", "搜索运行态与后续任务拆分", "source collection work-run 和 Teams 控制台拆分 searchOpenAssignmentCount、downstreamOpenAssignmentCount；只有资料发现/来源获取这类真实搜索任务运行时才显示“正在团队搜索”。", "内容提炼、资料筛选、候选图谱和知识库管理员入库审核的待处理不再把顶部状态误置为搜索中；控制台展示可搜索任务、后续任务和搜集结果，用户能判断当前是否真的有 Agent 在跑。"],
    ...
    ["M6.49", "五阶段完整结果与来源过滤", "资料搜集、资料筛选、候选入库、候选图谱、知识库管理员入库审核五个子页统一接入来源类型过滤：全部、PDF、论文网页/DOI、数据集、本地文件、缺少来源。", "资料筛选和候选入库不再只显示少量预览；候选图谱按过滤条件裁剪节点/边并补节点列表；知识库管理员入库审核显示候选资料和前审问题，所有卡片保留 DOI、网页、本地文件或候选详情追溯入口。"],
    ["M6.50", "知识搜集可读操作台根治", "知识搜集独立页改为实体工作台：主题背景不再穿透正文，顶部五阶段卡只承担导航、状态和主按钮，不再展示输入/输出小框。", "当前阶段子页只保留结果工作台；Agent 审查与执行过程通过阶段深链跳转到对应 Agent 私有对话页，右侧控制区固定宽度，只保留下一步操作、当前资料详情和当前步骤 Agent 配置。"],
    ...
    ["M6.52", "候选图谱与前审一键 Agent 节点", "候选图谱步骤卡从刷新改为 Agent 生成图谱，只使用已通过资料质量筛选的候选；知识库管理员入库审核步骤卡从检查状态改为 Agent 完成前审，自动汇总通过候选和候选图谱生成 steward_pack_draft 前审包。", "候选图谱仍停在 CandidateStore candidate-only 层；知识库管理员 memory 阶段 approved writeback 可经治理门禁写入正式 Team Knowledge，不给普通阶段 Agent 正式入库权限。"],
    ["M6.53", "KV 缓存模型与执行 Agent 模型分层", "知识搜集过程流不再写“本轮使用某模型”来描述 prompt-cache policy，而是明确显示 KV 缓存模型；真实执行资料搜集的功能 Agent 和模型仍在右侧当前步骤 Agent 配置中展示。", "当后端因请求模型不支持 prompt cache 而自动兜底到其他 KV 模型时，页面会标注“自动兜底”；用户不会再把 relay/openai/gpt 类缓存模型误认为资料发现、来源获取或内容提炼 Agent 的执行模型。"],
    ["M6.54", "阶段 Agent 与私聊深链契约", "候选图谱步骤绑定 candidate_graph / Candidate Graph Preview Agent，前审步骤绑定 knowledge_steward / 知识库管理员入库审核 Agent；阶段页不再内嵌 Agent 过程列表。", "Teams 知识搜集页为每个阶段提供对应 Agent 私聊深链；用户点击进入私有对话页审查真实 Agent 执行过程，候选图谱和前审不再借用资料质量评估 Agent。"],
    ["M6.55", "知识搜集四步闭环入库", "知识搜集阶段从旧五步收敛为搜索资料、资料提炼、资料审查、资料入库四张用户步骤卡；候选关系图和入库包不再作为独立用户阶段，而是合并进资料入库的 Agent 动作。", "第一阶段可从资料搜索结果推进到正式入库闭环：搜索资料写 DataRecord/source_manifest，资料提炼展示候选资料，资料审查批量写 sourceQualityAssessment，资料入库由知识库管理员审核 approved 后经 Team Knowledge 治理门禁创建 KnowledgeItem。"],
    ...
    ["M6.63", "挑战杯 Agent 提示词按能力收敛", "Agent Directory 为 challenge_cup_coordinator、data_discovery、source_acquisition、content_extraction 和 source_quality 绑定专属 prompt template 与 persona/task profile。", "提示词明确各角色现有工具边界：协调人只读协调，资料发现只做搜索线索，资料获取负责来源元数据，资料提炼输出 source_manifest 候选，资料审查只做入库前审；均不声称拥有 Shell、Git、文件写入或正式知识入库权限。"],
    ["M6.64", "五阶段 Agent 私聊入口与来源元数据标准化", "知识搜集五张步骤卡新增 Agent 私聊入口，候选图谱恢复为独立步骤，前审步骤显示为知识库管理员入库审核；DataRecord 导入 source_manifest 时统一写入 sourceRunId、sourceRecordId、sourceCategory、doi、sourceUrl、sourcePath、query、assignmentId 和 searchProvider。", "用户可从每个阶段直接进入对应 Agent 私有对话页审查执行过程；五个子页可按 PDF、论文网页/DOI、数据集、本地文件和缺少来源过滤，并能从资料卡追溯 DOI、网页、本地路径或搜索证据。"],
    ["M6.65", "科研团队 Agent 结构性重绑", "research-team 增加标准 7 角色修复入口：科研协调、资料发现、来源获取、内容提炼、资料质量评估、候选图谱、知识库管理员；旧成员、旧画布节点和重复 challengeCupTeamId Agent 可被物理清理。", "阶段 Agent 必须拥有 directSessionId 且在会话页可见；资料入库步骤只绑定知识库管理员，不再保留独立资料入库 Agent；阶段私聊入口只跳转到对应 Agent 私有对话页，缺少私聊会话时提示修复团队 Agent，不再退回 Agent 管理页。"],
    ["M6.66", "阶段卡 Agent 会话任务桥", "知识搜集五张步骤卡的主按钮不再直接执行本地阶段动作，而是创建对应功能 Agent 的 direct session task 并跳转到会话页执行。", "任务落盘到 source_collection_runs/<runId>/stage_session_tasks.json；Chat URL 保留 returnTo/returnLabel；Agent 先用 source_collection_context_tool 读取受控资料上下文，再用 source_collection_stage_writeback_tool 结构化回写 sourceCollectionStageSessionTasks，团队阶段再展示结果；不通过 file:// 或 localhost fetch 猜本地资料。"],
    ["M6.67", "阶段卡重复点击新任务", "知识搜集五张步骤卡每次主按钮点击都创建新的 Agent direct session task，并把新的任务消息投递到对应私聊会话。", "同一轮搜索、提炼、审查或入库的结果继续叠加到候选资料仓库，依赖既有 DOI/URL/标题指纹去重；stage-session-tasks 只在调用方显式传入同一 idempotencyKey 时复用同一次请求，避免后续点击被旧任务拦截。"],
    ["M6.68", "阶段任务受控上下文与工具回写", "新增 source_collection_context_tool 和 source_collection_stage_writeback_tool，资料提炼/资料审查 Agent 可在私聊任务里读取本轮 DataRecord/source_manifest 摘要并回写完成、阻塞或失败。", "阶段任务消息不再把本地路径或 localhost API 当作主要执行入口；Agent 工具策略允许资料提炼和资料审查角色直接调用上下文/回写工具，避免任务长期停留 running。"],
  ...
  services: [
    ...
    ["TeamsRoute workflow workspace", "已落地入口：Teams 页顶部固定横向切换条只暴露 AI 搜索范围团队和 ai科学研究团队，左侧栏已删除；监督进化/自进化模式页各自读取系统 Team 并展示只读团队画布；选择 ai科学研究团队后直接进入 research-team 专属科研控制台；总览只显示知识搜集、实验、迭代三张阶段卡，每张卡一个主操作和一个阶段详情入口。知识搜集卡会显示当前批次、待处理、候选和查询数，主按钮按状态进入知识搜集工作台或启动对应阶段 Agent 任务。知识搜集页位于 /teams?team=research-team&researchView=knowledge_collection，默认展示高价值状态摘要和状态驱动主操作；首次资料搜索任务可通过 stage-rounds/start 创建轮次并排队第一批后台 source collection 搜索；执行 source collection 搜索时会写入 source_collection_run 运行快照，让 AppShell 顶部“正在进行”优先显示知识搜集；资料搜集 run 列表通过 data-processing 服务端 teamId/startedFrom 过滤，避免被其它团队批次挤掉；后台 accepted 后 activeWorkRun 会进入控制台，直到 runStatus 收口；资料搜集、资料筛选、候选入库、候选图谱、知识库管理员入库审核五张步骤卡各自带主按钮并作为页内子页索引，顶部卡片只保留状态、关键数量和主操作，不再显示输入/输出小框；五张步骤卡主按钮会调用 stage-session-tasks，创建对应功能 Agent 的 direct session task，再带 returnTo/returnLabel 跳转到 Chat；底部只显示当前阶段结果内容，不再内嵌 Agent 过程；每个阶段详情头部保留进入对应 Agent 私有对话页的深链，作为查看/调试入口；Agent 执行结果以 writeback 合同回写 sourceCollectionStageSessionTasks 后挂回 ResearchStageRound；右侧只显示当前步骤操作、当前资料详情、关键数量和当前步骤 Agent 配置；KV/prompt cache 由后端从当前模型库解析，不再依赖旧固定模型 ID；稳定前缀、动态增量、assignment、query、存储路径和手工回写保留在可展开详情中；默认层移除解释性小字且使用实体工作台背景，避免主题壁纸压住正文；实验页新增实验计划账本，可生成计划草稿并登记 active baseline artifact、smoke result、full-run result，full-run passed 后可通知知识库管理员处理 experiment_result_pack；实验页和迭代页共享 Research Loop 模板面板，可按算法模型、仿真模拟、数据集基准、环境探针等模板登记证据和决策；旧 source_collection 参数兼容映射到知识搜集。"],
  ...
  deliveredSlice: [
    ...
    ["阶段 Agent 会话任务索引", "workspace/teams/<teamId>/source_collection_runs/<runId>/stage_session_tasks.json", "保存五张知识搜集步骤卡启动的 direct session task、writebackContract、Chat turn 摘要和结构化回写结果；任务引用同步挂回 ResearchStageRound.sourceCollectionStageSessionTasks。"],
...
const currentResearchRun = {
  ...
  nextActions: [
    "Source Quality Assessment Agent 对 source_manifest 做可靠性、可访问性、相关性和抽取风险审查。",
...
const knowledgeNodeRunbook = {
  "01": {
    ...
    agentStatus: "可复用通用 data-processing assignment / source-extraction / source-quality 后端已接入；Teams 搜索资料执行台、资料提炼页、资料审查台、科研总览控制台、KV 缓存门禁、启动复用反馈、metadata-only 搜索执行闭环、高价值信息层、中文控制台、默认层去说明化、步骤卡主操作入口、阶段 Agent direct session task、结构化 writeback 回团队阶段、右侧当前步骤 Agent 配置、资料审查页内反馈、资料审查 Agent 批量执行、候选库可视化二级索引、知识搜集阶段四步控制台、资料审查 Agent 复审入口、一键排队首批后台搜索、搜索资料当前动作和已收集资料分层、搜集结果状态徽标完整显示、搜集结果 DOI/网页/本地文件来源可溯源、资料提炼结果滚动与待审查交接、四步完整列表、来源类型过滤、阶段结果工作台、Agent 私聊深链、阶段输入输出交接、结果分页、实体可读工作台、阶段卡降噪、知识库管理员生成关系并通知知识库管理员、资料入库可从候选资料触发审查并发送入库请求、KV 缓存模型与执行 Agent 模型分层、阶段协调显式启动已接入",
    features: ["ResearchStageRound 建立", "科研控制台一键进入知识搜集", "总览页只显示三阶段主操作", "知识搜集状态驱动按钮", "知识搜集中文展示映射", "默认层去说明化", "右侧主操作唯一化", "资料审查入口常驻", "步骤卡主操作入口", "阶段 Agent 会话任务启动", "受控上下文工具", "结构化 writeback 工具", "结构化 writeback 回团队阶段", "Chat 返回链接", "右侧当前步骤 Agent 配置", "步骤卡二级索引", "四步阶段控制台", "阶段结果工作台", "Agent 私聊深链", "实体可读工作台", "阶段卡降噪", "阶段输入输出交接", "KV 缓存模型分层展示", "执行 Agent 模型右侧展示", "四步结果分页", "结果区独立滚动", "搜索资料当前动作", "已收集资料结果区", "搜集结果来源溯源", "搜集结果状态徽标", "资料提炼待审查交接", "资料审查 Agent 复审", "候选库可视化面板", "知识库管理员通知入库", "资料入库候选触发", "资料审查页内详情", "资料审查展开高亮反馈", "资料审查 Agent 批量执行", "source-quality/assess-batch", "source-quality force 重审", "新一轮搜集入口", "active 知识搜集轮次复用反馈", "一键排队首批搜索", "全局运行态知识搜集优先", "步骤功能 Agent 聚焦", "四步完整列表", "来源类型过滤", "阶段协调显式启动", "DataProcessingRun 建立", "DataProcessingRun reviewing 收口", "DataSearchPlan/querySeeds 搜索计划", "KV/prompt cache 启动门禁", "稳定前缀/动态增量契约", "CollectionAssignment 搜索资料任务", "assignedQueries 角色分配", "promptCachePartition 角色分区", "Teams 启动搜集批次", "Teams 查看 run/assignment/query/KV 状态", "Teams 高价值摘要层", "Teams 搜索下一批", "无 key 多源搜索执行器", "搜索结果质量门禁", "DataRecord 通用记录", "DataRecord 自动导入 source_manifest", "metadata-only 下载边界", "手工 CollectionOutput 回写", "本地资料登记", "source_manifest 写入", "文件路径/页码/来源可信度记录", "本地 PDF sha256/pageAnchors/excerpt 抽取", "sourceQualityAssessment 审查", "可选 research_knowledge_query_tool 查重"],
    ...
    operation: "用户在 ai科学研究团队科研控制台点击知识搜集主按钮，stage-rounds/start 创建或续用 ResearchStageRound，并复用 source-collection run 建立 DataProcessingRun；新建知识搜集轮次可继续排队第一批后台 source collection 搜索并在响应中返回 sourceCollectionSearchExecution.activeWorkRun。总览页只提示当前阶段、下一步和阶段详情入口，不默认展示模型证据、路径、query、KV 分区或校验详情。知识搜集卡会显示未开始、待继续搜索、搜索中、待审查、可入库、可进入实验或等待回写，并显示当前批次、待处理、候选和查询数；进入知识搜集页后，搜索资料、资料提炼、资料审查、候选图谱、知识库管理员入库审核五张步骤卡的主按钮会调用 source-collection-runs/{runId}/stage-session-tasks，创建对应功能 Agent 的 direct session task，并带 returnTo/returnLabel 跳转到 Chat 会话页执行；每次用户点击都会投递新的私聊任务消息，同一轮新增结果继续进入候选资料仓库并由 DOI/URL/标题指纹去重；Agent 接收任务后先调用 source_collection_context_tool 读取指定 team/run/task 的 bounded DataRecord、source_manifest、assignment 和 writebackContract，不通过 web_fetch_tool 访问 file:// 本地路径或 localhost API；Agent 完成或阻塞后必须调用 source_collection_stage_writeback_tool 提交结构化 status/summary/result/evidenceRefs/nextActions，团队阶段只以 writeback 结果更新 sourceCollectionStageSessionTasks。旧本地执行按钮、手工 CollectionOutput 回写和 Agent 私聊入口保留为详情/调试/兜底路径，不再作为五张阶段卡主路径。如果已有 active knowledge_collection，接口返回 continuedSourceRunRef、run 和 assignments，知识搜集阶段页直接显示 runId、记录数、待回写任务数和搜索执行状态。资料搜索 run 列表由 /api/data-processing/runs 使用 teamId + startedFrom 服务端过滤，不再先取全局最近批次再本地筛选，避免其它任务把当前科研批次挤出 UI。阶段启动只记录显式协商契约，不会自动打开团队群聊；需要协商时再由用户显式启动 coordination/retry。系统基于 topic/goal/scope/querySeeds 生成 searchPlan、queries、roleAssignmentInputs 和 resultWritebackContract，让 data_discovery/source_acquisition/content_extraction/source_quality 等功能 Agent 领取带 assignedQueries 的 CollectionAssignment。知识搜集页首屏作为流程控制台：左上角显示正在团队搜索、待继续搜索、待审查、可入库或失败；后台搜索 accepted 后 activeWorkRun 会合成到控制台和对话流，AppShell 顶部运行态在知识搜集与团队群聊并发时优先显示知识搜集，搜索资料卡保持进行中直到 runStatus 进入 reviewing 或终态。搜索资料、资料提炼、资料审查、候选图谱、知识库管理员入库审核五张步骤卡只展示状态、关键数量和主按钮；点击卡片本体切换页内子页，底部只显示当前阶段内容，右侧保留当前阶段主操作、关键数量和当前步骤细节，并在详情底部展示该步骤功能 Agent 的职责、Agent 名称、模型、状态和配置入口。当前阶段子页只保留结果工作台，按 8 条分页展示该阶段产物；阶段标题区同步显示本阶段输入、输出和下一步，Agent 的任务过程进入对应会话页审查。四个子页统一提供来源类型过滤：全部、PDF、论文网页/DOI、数据集、本地文件、缺少来源；过滤只改变当前页结果列表，不改变候选状态。搜索资料子页左侧主内容拆成当前搜索动作和已收集资料：当前动作只保留正在执行或最近完成的一条，已收集资料集中展示进入候选仓库的资料清单，让用户一眼区分流程和结果；每条结果必须显示可溯源入口：DOI 以 DOI 标签和 doi.org 链接展示，网页以可点击来源展示，本地资料显示文件路径，缺少 DOI/链接/路径时标为缺少来源；候选资料状态以横向徽标显示，避免“需补资料”“已通过”等中文状态在窄列中只露出单字。搜索资料步骤聚焦 data_discovery、source_acquisition、content_extraction，不把协调人显示成执行 Agent；Agent 配置入口复用 Agent 管理页，不复制第二套 Agent 配置表。资料审查任务由资料质量评估 Agent 在会话里处理待审 source_manifest 或复审结论；知识库管理员入库审核任务只能由知识库管理员在会话里形成 approved/blocked 结构化结论；approved 回写会触发后端标准化 steward_pack_draft，并经 SourceArtifact、pending RefinementProposal、review/apply gate 创建正式 KnowledgeItem/officialResearchGraph，不再保留独立入库 Agent 或要求用户逐条选择资料。人工通过审查、退回补资料和生成分块保留为覆盖修正入口；五个步骤按进行中、已完成、失败、未进行、待处理显示为不同颜色。processing/reviewing/open/planned、Agent 角色、DataRecord/source_manifest/Query seeds 等内部字段通过中文展示映射转成业务表述；assignment、query、KV 分区、存储路径、更多证据文件和手工回写收进可展开详情，默认层不再放解释性小字。用户可在详情兜底路径通过 source-collection-runs/{runId}/search/execute 继续执行无 key 多源 metadata 查询，优先使用公开论文/项目/网页索引并对低相关结果执行质量门禁，生成 search.executed、storage.data_record_written、storage.source_manifest_imported 等对话式事件，把结果写成 DataRecord 并幂等导入 source_manifest 候选；执行台仍支持手工回写一条 CollectionOutput。所有采集 assignment 关闭且已有资料记录后，通用 DataProcessingRun 收口到 reviewing，表示后台搜索已停止、下一步进入资料审查。资料审查批量执行/复审复用 source-quality/assess 写入 sourceQualityAssessment/sourceQualityAssessments，并标记通过审查、退回补资料或拒绝。对本地 PDF 运行 source-extraction，计算 sha256、生成 pageAnchors/excerpt，不改写原文，不自动混入联网全文。",
  ...
  "07": {
    ...
    agent: "知识库管理员 / agent-knowledge-steward",
    ...
    localModelUse: "本地 9B 模型只能生成 steward_pack_draft 草稿；正式入库只能由知识库管理员 memory 阶段 approved writeback 触发后端治理门禁，禁止绕过 gate 请求正式写入。",
    humanGate: "必须经知识库管理员 / Ingestion Approval Gate 确认，不能由科研执行 Agent 或本地模型直接入库。",
    ...
    operation: "生成带 sourceTrace、riskSummary、proposalPayload、ratingSuggestion、approvalRequired=true 的 CandidateStore 草稿包；知识库管理员 memory 阶段回写 approved 候选后，后端提交到指定 knowledgeBaseId 的待审入库队列并执行治理门禁；status API 汇总 pendingProposalCount、formalKnowledgeItemCount、actionItems 和 officialBoundary。",
  ...
  "08": {
    ...
    agentStatus: "后端/API 已可生成候选图谱快照；Teams 工作台绑定独立 candidate_graph Agent，并把 agentProcess 显示到当前步骤过程页。",
    features: ["candidate_graph 候选快照生成", "候选节点/边预览 payload", "断链报告", "未审节点清单", "候选/正式边界标识", "Teams SVG 候选图谱面板", "candidate_graph Agent 过程追踪"],
