...
const nodes = [
  ...
  },
  {
    ...
    summary: "steward_pack_draft 先作为团队待入库知识包暂存，搜集流程向 Knowledge Steward Agent 发送入库请求并唤醒它处理最终入库。",
    ...
    actions: [
      ...
      "知识搜集入库入口默认向 Knowledge Steward Agent inbox 发送 challenge_cup_knowledge_ingestion_request 并尝试唤醒该 Agent。",
      "Knowledge Steward Agent 收到请求后再提交 Team Knowledge 待审队列，进入 steward_pending_knowledge_review。",
    ...
    memory: "待入库包通知阶段不创建正式 KnowledgeItem；Knowledge Steward Agent 后续审批通过后复用 Team Knowledge review/apply 创建正式 KnowledgeItem；状态总览只读聚合 formalKnowledgeItemCount、pendingProposalCount 和 Agent 通知结果。",
  ...
  },
  {
    ...
    title: "实验迭代闭环",
    status: "Research Loop 模板化流程与 Teams 实验面板已接入",
    statusKind: "active",
    role: "Research Loop Agent / Experiment Ledger Service",
    summary: "把实验与迭代合成一个模板化 Research Loop：按算法模型、仿真模拟、数据集基准、环境探针、资料审查等问题类型切换模板，先登记人工证据和决策，不接沙盒 runner。",
    objective: "根据研究问题选择验证模板，记录所需证据、指标、日志和迭代决策；把可推进结论转成下一轮模板建议，而不是自动训练或自动入库。",
    ...
    actions: [
      ...
      "POST experiments/plans/{planId}/full-run-result 只登记人工 full-run 结果证据；Knowledge Steward 入库请求仍走显式审批边界。",
      "POST experiments/plans/{planId}/knowledge-ingestion-request 生成 experiment_result_pack 并通知 Knowledge Steward Agent，正式知识/RAG 仍等待复核。",
    ...
    outputs: ["experiment_plan 草稿", "active_baseline_artifact", "smoke_result", "full_run_result 手工账本", "experiment_result_pack", "research_loop", "evidenceRecords", "decision", "iterationProposal", "readinessChecklist"],
    memory: "实验计划和 Research Loop 只进入 Team workflow runtime memory；实验结果是否入库仍取决于 full-run 证据、证据复核 Agent 和知识治理 Agent。",
    graph: "Research Loop 是候选流程状态，不创建正式图谱边；正式边仍等待 Knowledge Steward 审批后从 ExperimentResult/KnowledgeItem trace 生成。",
    risks: ["计划草稿被误认为已训练", "commandPreview 被误当已执行", "模板证据缺口没有补齐就推进迭代", "baseline 不公平"],
    openQuestions: ["沙盒 runner、训练 runner 和自动仿真执行器明确后置；当前阶段只做模板、证据、决策和 Steward 入库请求链路。"],
  },
  {
    id: "11",
...
const knowledgeRunbook = {
  ...
  stateFlow: [
    ...
    ["knowledge_steward_agent_notified", "已通知知识库 Agent", "steward_pack_draft 已写入 CandidateStore，并向 Knowledge Steward Agent inbox 发送入库请求。"],
    ["steward_pending_knowledge_review", "待审入库对象", "Knowledge Steward Agent 已创建 SourceArtifact、pending RefinementProposal，可选 pending ratingSuggestion；等待审批门禁。"],
  ...
  artifacts: [
    ...
    ["steward_ingestion_pack_YYYYMMDD_NNN.json", "知识治理摄取包", "治理建议、评级建议、目标知识域和正式入库申请；默认先作为待入库包通知 Knowledge Steward Agent。"],
    ["agent_inbox_messages.jsonl", "知识库 Agent 入库请求", "challenge_cup_knowledge_ingestion_request 消息记录 teamId、stewardPackCandidateId、knowledgeBaseId 和最终入库边界。"],
  ...
  gates: [
    ...
    ["Agent 通知门", "资料搜集流程只发送入库请求并激活 Knowledge Steward Agent，不直接创建 SourceArtifact/Proposal/KnowledgeItem", "knowledge_steward_agent_notified", "停在 steward_pack_draft"],
    ["治理门", "Knowledge Steward Agent 收到请求后只创建 SourceArtifact + pending RefinementProposal + 可选 pending ratingSuggestion，不直接创建 KnowledgeItem", "steward_pending_knowledge_review", "退回 steward_needs_revision"],
  ...
  teamWorkflowOrchestration: {
    ...
    landedApis: [
      ...
      ["POST", "/api/teams/{team_id}/workflow-orchestration/knowledge-collection/ingest", "批量审查候选、生成候选图谱和 steward_pack_draft；默认只发送知识库 Agent 入库请求并唤醒 Agent，不直接正式入库。"],
      ["POST", "/api/teams/{team_id}/workflow-orchestration/steward-packs/{candidate_id}/knowledge-ingestion", "由 Knowledge Steward Agent 把有效 steward_pack_draft 提交为 SourceArtifact + pending RefinementProposal，可选 pending ratingSuggestion；不创建 KnowledgeItem/RAG/正式图谱。"],
  ...
  featureMatrix: [
    ...
    ["实验结果入库请求", "experiments/plans/{planId}/knowledge-ingestion-request 生成 experiment_result_pack 并通知 Knowledge Steward Agent；正式 Team Knowledge/RAG/official graph 仍由 Steward 复核后写入", "已接入前端/API"],
    ["Research Loop 模板", "research-loop/templates/status/loops/evidence/decision 支持算法模型、仿真模拟、数据集基准、环境探针等模板；Teams 实验页和迭代页共享同一模板切换、证据登记和决策面板", "已接入前端/API"],
...
const implementationBlueprint = {
  ...
  target: "复用 Vibelution 现有 Research、Team Knowledge、RAG、Knowledge Steward 和 Memory Graph 能力，把 01-09 做成可运行 MVP，并把 10 实验阶段先推进到可审查计划账本。",
  activeTeam: {
  ...
  milestones: [
    ...
    ["M6.52", "候选图谱与前审一键 Agent 节点", "候选图谱步骤卡从刷新改为 Agent 生成图谱，只使用已通过资料质量筛选的候选；共享记忆前审步骤卡从检查状态改为 Agent 完成前审，自动汇总通过候选和候选图谱生成 steward_pack_draft 前审包。", "这两个节点都不要求用户逐条选择资料；噪声候选会被自动过滤，产物仍停在 CandidateStore candidate-only 层，不写正式 Team Knowledge、不写 RAG、不写 official graph。"],
    ...
    ["M6.54", "阶段 Agent 与私聊深链契约", "候选图谱步骤绑定 candidate_graph / Candidate Graph Preview Agent，前审步骤绑定 knowledge_steward / 共享记忆前审 Agent；阶段页不再内嵌 Agent 过程列表。", "Teams 知识搜集页为每个阶段提供对应 Agent 私聊深链；用户点击进入私有对话页审查真实 Agent 执行过程，候选图谱和前审不再借用资料质量评估 Agent。"],
    ["M6.55", "知识搜集四步闭环入库", "知识搜集阶段从旧五步收敛为搜索资料、资料提炼、资料审查、资料入库四张用户步骤卡；候选关系图和入库包不再作为独立用户阶段，而是合并进资料入库的 Agent 动作。", "第一阶段可从资料搜索结果推进到待入库知识包：搜索资料写 DataRecord/source_manifest，资料提炼展示候选资料，资料审查批量写 sourceQualityAssessment，资料入库复用候选关系和 steward_pack_draft，并向 Knowledge Steward Agent 发送入库请求；正式 Team Knowledge/RAG/official graph 仍由 Knowledge Steward Agent 后续走审核门禁。"],
    ...
    ["M7.3", "Full-run 与知识入库请求", "新增 Teams 实验账本 full-run 结果登记和 knowledge-ingestion-request 前端入口，复用已有 full-run-result 与 Knowledge Steward 通知 API。", "smoke passed 后可登记外部 full-run 证据；full-run passed 后可生成 experiment_result_pack 并通知 Knowledge Steward Agent；正式知识/RAG/official graph 仍等待 Steward 审批。"],
  ...
  services: [
    ...
    ["memory_ingestion_bridge", "已复用 Agent Inbox、Team Knowledge create_ingestion_package、review_refinement_proposal、rating suggestion review/create 和 KnowledgeItem metadata patch；steward_pack_draft 默认先通知 Knowledge Steward Agent，Agent 后续可提交 pending proposal，审批通过后创建正式 KnowledgeItem、迁移待审评分建议，并写入 officialResearchGraph 正式科研边。"],
  ...
  deliveredSlice: [
    ...
    ["Full-run 结果登记", "/api/teams/{teamId}/workflow-orchestration/experiments/plans/{planId}/full-run-result", "登记外部 full-run 指标、结果路径、日志和配置引用；passed 后开放 Knowledge Steward 入库请求。"],
    ["实验结果入库请求", "/api/teams/{teamId}/workflow-orchestration/experiments/plans/{planId}/knowledge-ingestion-request", "生成 experiment_result_pack 并通知 Knowledge Steward Agent；正式知识、RAG 和 official graph 仍由 Steward 审批。"],
