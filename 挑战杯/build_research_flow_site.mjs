...
const implementationBlueprint = {
  ...
  activeTeam: {
    ...
    defaultView: "选中 ai科学研究团队后，团队内容区显示科研控制台：研究主题、当前阶段、知识搜集/实验/迭代三张阶段卡、每卡一个主按钮和一个阶段详情入口；知识搜集卡会按未开始、搜集中、搜索中、待筛选、可进入实验、等待回写显示状态，并提供“新一轮搜集”；模型证据、候选、路径、校验和内部状态默认移到阶段详情页或高级信息中。",
  ...
  milestones: [
    ...
    ["M6.23", "控制台状态驱动操作卡", "知识搜集卡接入 source collection run 状态，显示未开始、搜集中、搜索中、待筛选、可进入实验、等待回写，并展示当前批次、待处理、候选和查询数。", "主按钮不再叫笼统的继续搜集，而是按状态变为开始知识搜集、执行下一批搜索、进入资料筛选或进入搜集工作台；旁边保留新一轮搜集和阶段详情，方便在总览操控流程、在详情查看内容。"],
  ],
  schemas: [
    "DataProcessingRun",
    "DataRecord",
  ...
  services: [
    ...
    ["TeamsRoute workflow workspace", "已落地入口：Teams 页顶部固定横向切换条只暴露 AI 搜索范围团队和 ai科学研究团队，左侧栏已删除；监督进化/自进化模式页各自读取系统 Team 并展示只读团队画布；选择 ai科学研究团队后直接进入 research-team 专属科研控制台；总览只显示知识搜集、实验、迭代三张阶段卡，每张卡一个主操作和阶段详情入口。知识搜集卡会显示当前批次、待处理、候选和查询数，主按钮按状态执行开始、下一批搜索、筛选入口或打开工作台，另有新一轮搜集入口。知识搜集页位于 /teams?team=research-team&researchView=knowledge_collection，默认展示高价值状态摘要、下一步动作和对话式 source collection 过程流；KV/prompt cache、稳定前缀、动态增量、assignment、query、存储路径和手工回写保留在可展开详情中；实验/迭代页先提供规划启动、轮次状态和边界说明；旧 source_collection 参数兼容映射到知识搜集。"],
    ...
    <main class="shell">
      ...
      <section id="overview" class="review-dashboard">
        ...
        <aside class="dashboard-panel">
          <h2>审核摘要</h2>
          <div class="kpi-grid">
            <div class="kpi"><b>当前阶段</b><strong>M6.23</strong><span>控制台知识搜集卡已改为状态驱动，可执行下一批搜索或开启新一轮搜集。</span></div>
          ...
          <div class="focus-list">
            <div class="focus-item"><b>优先看</b><span>知识搜集卡是否能一眼判断是否进行中、下一步是什么，以及如何重新搜集。</span></div>
