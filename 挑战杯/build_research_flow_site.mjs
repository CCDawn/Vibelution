...
const nodes = [
  ...
  },
  {
    ...
    summary: "已把可用算法假设推进到实验计划草稿和可审查缺口面板；真实训练 runner 仍由后续切片承接。",
    ...
    actions: [
      ...
      "Teams 实验阶段页展示实验计划账本、候选算法假设、readyForSmoke/readyForFullRun 门禁和阻塞项。",
      "强制边界：不创建 ExperimentResult，不触发训练 runner，不写正式 Team Knowledge/RAG/official graph。",
    ],
    outputs: ["experiment_plan 草稿", "active_baseline_artifact", "readinessChecklist", "baselineSelection", "promotion/reject 前置门禁"],
    memory: "实验计划只进入 Team workflow runtime memory；实验结果是否入库仍取决于后续 smoke/full run、证据复核 Agent 和知识治理 Agent。",
    graph: "本切片不创建 ExperimentResult 边；未来边仍是 AlgorithmHypothesis evaluated_by ExperimentResult。",
    risks: ["计划草稿被误认为已训练", "active baseline artifact 缺失", "smoke 指标不能验证假设", "baseline 不公平"],
    openQuestions: ["下一刀是否先接 active baseline artifact 登记，再接轻量 smoke runner。"],
  },
  {
    id: "11",
