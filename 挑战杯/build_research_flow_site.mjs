...
const implementationBlueprint = {
  ...
  services: [
    ...
    ["team_workflows API", "已落地：/api/teams/{team_id}/workflow-orchestration、stage-rounds/status、stage-rounds/start、source-collection-runs（含 searchPlan/querySeeds/assignedQueries/resultWritebackContract）、candidates/source、data-processing/runs/{runId}/records/{recordId}/source-candidate、candidates/{candidate_id}/source-extraction、candidates/{candidate_id}/paper-note-draft、candidates、candidates/validation、candidate-graph、transfers、decide、knowledge-ingestion/status、coordination/status；coordination/status 返回 communicationBrief 但不发送消息。"],
  ...
  deliveredSlice: [
    ...
    ["科研阶段轮次启动", "/api/teams/{teamId}/workflow-orchestration/stage-rounds/start", "统一启动/续做/开启新一轮知识搜集、实验、迭代；知识搜集复用 source-collection run，实验和迭代先生成 planning 轮次。"],
    ...
    ["阶段轮次索引", "workspace/teams/<teamId>/research_stage_rounds/index.json", "保存 ResearchStageRound、sourceRunIds、upstreamRoundIds、teamMemoryRecord 和 coordinationContract。"],
...
const knowledgeNodeRunbook = {
  "01": {
    ...
    agentStatus: "可复用通用 data-processing assignment / source-extraction 后端已接入；Teams 资料搜集执行台和三阶段启动台已接入",
    features: ["ResearchStageRound 建立", "三阶段启动台一键进入知识搜集", "DataProcessingRun 建立", "DataSearchPlan/querySeeds 搜索计划", "CollectionAssignment 资料搜集任务", "assignedQueries 角色分配", "Teams 启动搜集批次", "Teams 查看 run/assignment/query 状态", "手工 CollectionOutput 回写", "DataRecord 通用记录", "DataRecord 自动导入 source_manifest", "本地资料登记", "source_manifest 写入", "文件路径/页码/来源可信度记录", "本地 PDF sha256/pageAnchors/excerpt 抽取", "可选 research_knowledge_query_tool 查重"],
    tools: ["/api/teams/{teamId}/workflow-orchestration/stage-rounds/status", "/api/teams/{teamId}/workflow-orchestration/stage-rounds/start", "/api/teams/{teamId}/workflow-orchestration/source-collection-runs", "/api/data-processing/runs", "/api/data-processing/runs/{runId}/collection-assignments", "/api/data-processing/runs/{runId}/collection-assignments/{assignmentId}/outputs", "/api/teams/{teamId}/workflow-orchestration/data-processing/runs/{runId}/records/{recordId}/source-candidate", "research_knowledge_query_tool", "/api/teams/{teamId}/workflow-orchestration/candidates/{candidateId}/source-extraction"],
    localModelUse: "本地 9B 模型适合做标题/摘要/片段初筛，输出 relevanceScore、topicTags、excludeReason，不直接决定正式纳入。",
    humanGate: "确认资料允许分析、来源可信度和是否纳入本轮。",
    gap: "科研三阶段启动台、资料搜集阶段轮次、资料搜集批次启动、DataSearchPlan/query seed 契约、通用资料搜集 assignment/output、Teams 执行台和 DataRecord 到 source_manifest 导入桥已接入；真实外部搜索执行器、长 PDF 自动分批、章节识别和非 PDF 资料解析仍待补。",
