...
const knowledgeNodeRunbook = {
  "01": {
    ...
    features: ["DataProcessingRun 建立", "DataSearchPlan/querySeeds 搜索计划", "CollectionAssignment 资料搜集任务", "assignedQueries 角色分配", "DataRecord 通用记录", "本地资料登记", "source_manifest 写入", "文件路径/页码/来源可信度记录", "本地 PDF sha256/pageAnchors/excerpt 抽取", "可选 research_knowledge_query_tool 查重"],
    ...
    gap: "资料搜集批次启动、DataSearchPlan/query seed 契约、通用资料搜集 assignment/output 和 DataRecord 到 source_manifest 导入桥已接入；真实外部搜索执行器、长 PDF 自动分批、章节识别和非 PDF 资料解析仍待补。",
    entry: "用户提供 PDF、论文、赛题或补充资料；文件可在挑战杯工作区定位。",
    operation: "由 Research Coordination Agent 调用 source-collection-runs 启动批次，系统基于 topic/goal/scope/querySeeds 生成 searchPlan、queries、roleAssignmentInputs 和 resultWritebackContract；同时建立通用 DataProcessingRun，让 data_discovery/source_acquisition/content_extraction 等功能 Agent 领取带 assignedQueries 的 CollectionAssignment、提交 CollectionOutput；再通过 Team workflow 导入桥把合格 DataRecord 幂等转成 source_manifest 候选。对本地 PDF 运行 source-extraction，计算 sha256、生成 pageAnchors/excerpt，不改写原文，不自动混入联网资料。",
