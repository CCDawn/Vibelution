...
const implementationBlueprint = {
  ...
  activeTeam: {
    ...
    note: "挑战杯科研流程直接绑定当前 Vibelution ai科学研究团队，不另建新团队；Teams 页顶部只保留 AI 搜索范围团队和 ai科学研究团队两个固定入口，左侧栏删除，不再展示创建团队、模板创建或完整团队列表；选择 ai科学研究团队后直接进入团队专属科研工作台，默认只显示科研控制台：知识搜集、实验、迭代三张阶段卡各保留一个主操作和阶段详情入口；知识搜集阶段统一收敛为搜索资料、资料提炼、资料审查、资料入库四步，组织画布仅保留为附属结构视图；知识搜集一级工作台只展示当前判断、关键数量、阶段颜色和下一步按钮，详细追踪放入详情区。",
    workspaceEntry: "/teams?team=research-team",
    sourceCollectionEntry: "/teams?team=research-team&researchView=knowledge_collection",
    defaultView: "选中 ai科学研究团队后，团队内容区显示科研控制台：研究主题、当前阶段、知识搜集/实验/迭代三张阶段卡、每卡一个主按钮和一个阶段详情入口；知识搜集卡会按未开始、待继续搜索、搜索中、待审查、可入库、可进入实验、等待回写显示状态，并提供“新一轮搜集”；点击知识搜集主按钮会创建/续用阶段轮次，并立即排队第一批后台资料搜索；知识搜集状态读取会根据最新 source_collection_run 终态自动修复旧 running，避免页面假显示 Agent 仍在运行；知识搜集独立页只展示搜索资料、资料提炼、资料审查、资料入库四张步骤卡，每张卡只承载状态、关键数量和主操作，当前步骤的功能 Agent 名称、模型、状态和配置入口集中显示在右侧详情底部；资料审查主按钮会让资料质量评估 Agent 批量执行待审候选，已审完时显示 Agent 重新审查并 force 复审已审候选；资料入库主按钮会让入库 Agent 生成入库关系和待入库知识包，并向知识库 Agent 发送入库请求，不再要求用户逐条选择资料；若尚无通过资料但已有候选资料，资料入库按钮显示提炼并通知 Agent，后端先批量审查再生成关系、入库包和 Agent inbox 请求，正式知识由知识库 Agent 后续处理；批次列表按 teamId/startedFrom 在服务端过滤，后台 activeWorkRun 会持续显示正在团队搜索，AppShell 顶部优先显示知识搜集而不是团队群聊；知识搜集页不再把本阶段 Agent 统一堆在顶部，点击每张步骤卡才查看该步骤内容和操作；搜索资料子页把当前动作和原始资料记录分开：过程区只显示当前正在执行的一条动作，结果区只展示本轮 DataRecord 原始记录，并明确标出已入候选多少、还有多少待导入候选；资料提炼子页单独展示本轮 source_manifest 候选资料，避免把原始记录数和候选数混在一起；搜索资料结果列表和资料提炼候选列表都有独立纵向滚动区，分页控件保留在列表下方，长批次不会被外层布局裁掉；Agent 重新提炼只重新同步 DataRecord 到候选资料，不会自动完成资料审查，候选仍会显示为待 Agent 审查并引导进入资料审查步骤；资料结果统计使用原始记录、已入候选、可点击来源和本地文件，列表内 DOI 单独显示为 DOI 链接，网页显示可点击来源，本地文件显示路径，缺少来源直接标出；每条资料卡片可点击选中，右侧详情把资料来源和搜集证据分开：主操作只打开论文 DOI、网页来源或本地文件，Crossref/OpenAlex 等机器 API 放入“查看搜索证据”折叠区，并保留打开批次目录、搜索步骤、搜集记录和候选镜像入口；知识搜集启动不再依赖旧的 houmo_qwen35_9b_agent 固定 ID，而是由后端从当前模型库解析可用 KV 模型；过程流把 KV 缓存模型和执行 Agent 模型分开展示，避免把缓存兜底模型误认为搜索资料 Agent；模型证据、候选、路径、校验和内部状态默认移到阶段详情页或高级信息中。",
    ...
    <main class="shell">
      ...
      <section id="overview" class="review-dashboard">
        ...
        <aside class="dashboard-panel">
          <h2>审核摘要</h2>
          <div class="kpi-grid">
            <div class="kpi"><b>当前阶段</b><strong>M6.62</strong><span>阶段状态同步根因已修复，旧 running 轮次会在状态读取时按最新 work-run 终态自恢复。</span></div>
