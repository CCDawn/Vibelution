...
const nodes = [
  {
    ...
    summary: "建立稳定证据源，不把未确认信息混入候选知识链。",
    ...
    outputs: ["资料清单", "sourceFiles 引用", "Paper 原始来源节点"],
    memory: "不进入正式记忆库；只作为后续 paper_note 的 sourceFiles。",
    graph: "可作为候选图谱的 Paper source 节点，不进入正式知识图谱。",
    risks: ["资料来源不明", "PDF 页码不可追溯", "联网搜索结果混入第一版"],
    openQuestions: ["是否需要单独维护 source_manifest.json。"],
  },
  {
    id: "02",
    slug: "paper-note",
    ...
    inputs: ["资料清单", "PDF 页码", "论文片段", "补充备注"],
    actions: [
      "生成 summary、keyFindings、methods、limitations。",
      "每条关键发现尽量绑定页码、章节或 citation。",
      "保留 uncertainty，避免把摘要写成确定事实。",
    ],
    ...
    openQuestions: ["是否需要为长论文拆分多个 paper_note。"],
  },
  {
    id: "03",
  ...
  },
  {
    ...
    title: "科研审稿",
    status: "prefilter 已落地",
    statusKind: "done",
    role: "Evidence Review Agent",
    summary: "在交给记忆平台前先做领域与工程风险过滤。",
    objective: "先用 review_record 候选记录证据、类比、可测性和成本风险，不写最终审稿裁决。",
    ...
    outputs: ["review_record 候选", "review_prefiltered 状态", "review_needs_revision 状态"],
    memory: "审稿记录仍是候选工作区内容。",
    graph: "候选图谱可展示 reviewed_by、rejected_by、needs_revision 边。",
    risks: ["证据复核 Agent 只做形式审核", "缺少明确退回项", "风险标记不可复用"],
    openQuestions: ["最终 review decision 由 Evidence Review Agent 独立写，还是由 Ingestion Approval Gate 统一裁决。"],
  },
  {
    id: "07",
    ...
    summary: "steward_pack_draft 可提交到 Team Knowledge 待审队列，并由审批门禁批准或退回。",
    ...
    outputs: ["steward_pack_draft 候选", "SourceArtifact", "pending RefinementProposal", "proposal ratingSuggestion", "KnowledgeItem ratingSuggestion", "official_sync_record"],
    memory: "待审阶段不创建正式 KnowledgeItem；审批通过后复用 Team Knowledge review/apply 创建正式 KnowledgeItem，并把评分建议承接到正式 KnowledgeItem 的待审评级队列。",
    graph: "审批通过后正式 KnowledgeItem 可经 Memory Graph 视图展示；本轮不另写挑战杯专用正式图谱边文件。",
    risks: ["绕过审批门禁", "目标知识域错误", "把 ingestion pack 草稿误当正式入库记录"],
    openQuestions: ["正式图谱是否需要单独生成 supports/maps_to/inspires 细粒度边。"],
  },
  {
    id: "08",
    ...
    outputs: ["candidate_graph 候选快照", "missingLinks 断链报告", "unreviewedNodes 未审节点清单"],
    memory: "候选图谱只进入 CandidateStore 草稿区，不进入正式 RAG。",
    graph: "只展示候选图谱；正式图谱同步必须等 ingested。",
    risks: ["候选图谱被误读为正式知识", "links 指向不存在对象"],
    openQuestions: ["后续是否导出独立 candidate_graph.json，或直接由前端读取 CandidateStore graph payload。"],
  },
  {
    id: "09",
    slug: "official-graph",
    ...
    summary: "只有审核入库后的知识进入正式图谱与 RAG。",
    objective: "将 ingested 知识同步到 Team Knowledge、正式 RAG 和正式知识图谱。",
    ...
    outputs: ["正式知识图谱节点", "正式 RAG 可检索上下文", "officialKnowledgeItemId"],
    memory: "只同步 ingested 内容。",
    graph: "正式图谱必须能追溯到 paper_note 和原始来源。",
    risks: ["候选误同步", "sourceFiles 丢失", "正式图谱缺审核记录"],
    openQuestions: ["正式图谱同步由记忆平台自动触发还是审批门禁触发。"],
  },
  {
    id: "10",
...
const knowledgeRunbook = {
  ...
  stateFlow: [
    ...
    ["official_synced", "正式同步完成", "授权审批门禁通过后进入 Team Knowledge、RAG、正式图谱视图，并保留 KnowledgeItem 级待审评分建议。"],
  ],
  artifacts: [
  ...
  teamWorkflowOrchestration: {
    ...
    landedApis: [
      ...
      ["POST", "/api/teams/{team_id}/workflow-orchestration/steward-packs/{candidate_id}/knowledge-ingestion/review", "审批 pending steward pack；approved 复用 Team Knowledge review/apply 创建正式 KnowledgeItem、迁移评分建议并写 officialSyncRecord，rejected 退回 steward_needs_revision。"],
    ],
    integrations: [
  ...
  featureMatrix: [
    ...
    ["PDF/资料解析", "读取本地 PDF、页码锚点、摘录范围和 sourceFiles 绑定", "待接入"],
    ...
    ["候选图谱预览", "candidate_graph.json、断链报告、候选/正式边界显示；现有 Memory Graph 只能承接正式知识结构视图", "需新设计"],
    ["正式图谱同步", "approved_to_ingest 后进入 Team Knowledge/RAG；正式图谱可借 /api/memory/knowledge-graph 展示结构", "可复用+接线"],
  ],
  projectAlignment: [
    ...
    ["记忆知识图谱", "/api/memory/knowledge-graph 和 MemoryRoute 知识图谱视图已存在", "部分匹配", "适合展示正式 Team/Agent/KnowledgeBase/KnowledgeItem 结构；候选图谱 candidate_graph.json 仍需独立设计。"],
    ["PDF/本地资料解析", "当前知识 ingestion 工具要求传入已有 excerpt/source_ref，不负责解析 PDF", "待接入", "01-02 需要补 PDF 解析、页码锚点和 source_manifest 管理。"],
...
const implementationBlueprint = {
  ...
  architecture: [
    ...
    ["图谱展示层", "候选图谱只补 candidate_graph.json；正式图谱复用 /api/memory/knowledge-graph。"],
  ],
  milestones: [
    ...
    ["M1", "候选数据基座", "已新增 CandidateStore 列表查询、校验报告、source_manifest/PDF 最小字段校验；PDF 缺路径、sha256 或 allowedForAnalysis=true 会进入 source_needs_confirmation。", "能登记 PDF source_manifest，按 candidateType/currentState/qualityStatus 查询候选，并查看 invalid/error/warning 统计；仍不写正式 Team Knowledge/RAG/知识图谱。"],
    ...
    ["M5", "知识治理与正式同步", "steward_pack_draft 门禁、待审入库桥、Ingestion Approval Gate 和评分建议迁移已落地：有效草稿可提交为待审对象，批准后创建正式 KnowledgeItem 并承接待审评级，拒绝后退回修订。", "正式 RAG/Graph 通过已审核 KnowledgeItem 的现有读取面可检索/可见；本轮不另写挑战杯专用图谱边或向量索引。"],
  ],
  schemas: [
    "source_manifest",
    "paper_note",
  ...
  services: [
    ...
    ["team_workflows API", "已落地：/api/teams/{team_id}/workflow-orchestration、candidates/source、candidates、candidates/validation、candidate-graph、transfers、decide。"],
    ...
    ["source_parser", "待接入本地脚本/模块，PDF/资料解析与页码锚点。"],
    ["candidate_validator", "已落地 source_manifest/PDF 字段校验、paper_note citation anchor 校验、neuro_mechanism 证据/术语风险校验、mechanism_mapping fact/inference/overAnalogyRisk 校验、algorithm_hypothesis experimentPlan 校验、candidate_graph 边界校验和 CandidateStore 校验报告。"],
    ["candidate_graph_builder", "已落地后端/API：生成 candidate_graph 候选快照、断链报告、未审节点清单和 candidate_only officialBoundary；前端图谱读取仍待接。"],
    ["research_agent_binding", "复用 research_service、research flow canvas、prompt-research-* 和研究组织治理工具。"],
    ["memory_ingestion_bridge", "已复用 Team Knowledge create_ingestion_package、review_refinement_proposal 和 rating suggestion review/create；steward_pack_draft 可进入 pending proposal，审批通过后创建正式 KnowledgeItem，并把 proposal 级评分建议迁移为 KnowledgeItem 级待审评分建议。"],
  ],
  communicationRoles: [
  ...
  deliveredSlice: [
    ["服务文件", "core/web/services/team_workflow_orchestration_service.py", "Team 编排、候选登记、转移请求、协调 Agent 裁决和轻量运行事件日志。"],
    ["路由文件", "core/web/routes/team_workflows.py", "提供工作流查询/确保、候选资料登记、转移提交、转移裁决 API。"],
...
const knowledgeNodeRunbook = {
  "01": {
    ...
    agentStatus: "可复用 research_broad / 待绑定挑战杯资料登记职责",
    features: ["本地资料登记", "source_manifest 写入", "文件路径/页码/来源可信度记录", "可选 research_knowledge_query_tool 查重"],
    tools: ["research_knowledge_query_tool", "本地文件读取/PDF 解析能力（待接入流程）"],
    localModelUse: "本地 9B 模型适合做标题/摘要/片段初筛，输出 relevanceScore、topicTags、excludeReason，不直接决定正式纳入。",
    humanGate: "确认资料允许分析、来源可信度和是否纳入本轮。",
    gap: "需要候选资料 manifest 的落盘规则和 PDF 页码锚点提取能力。",
    entry: "用户提供 PDF、论文、赛题或补充资料；文件可在挑战杯工作区定位。",
    operation: "建立 source_manifest 记录，不改写原文，不自动混入联网资料。",
    exit: "每个资料项都有 id、path、type、sourceTrust、allowedForAnalysis 和 pageScope。",
    fallback: "路径缺失、来源不明或权限不清时停在 source_needs_confirmation。",
  },
  "02": {
    state: "paper_note_draft",
    agent: "Paper Note Extraction Agent",
    agentStatus: "可复用 research_broad / 需要资料摘要模板",
    features: ["PDF/资料摘录", "paper_note 候选 JSON 生成", "citation anchors", "uncertainty 字段"],
    tools: ["research_knowledge_query_tool", "本地 PDF 解析/摘录能力（待接入流程）"],
    localModelUse: "本地 9B 模型适合按章节或 chunk 生成 paper_note 草稿；32k 内保留 18k-22k 原文证据和输出预留。",
    humanGate: "必要时确认摘要是否保守、是否遗漏关键章节。",
    gap: "paper_note schema 与页码引用校验已接入 CandidateStore；长文拆分规则和本地 PDF 页码摘录仍待接。",
  ...
  "06": {
    ...
    agentStatus: "review prefilter 后端已接入；最终裁决门禁仍待接",
    features: ["证据审查", "类比风险审查", "可测性/成本审查", "review_record 生成"],
    ...
    gap: "最终 review.decision 写入、riskFlags 枚举和 rejectionArchive 规则仍待接。",
  ...
  "07": {
    ...
    agentStatus: "后端草稿包门禁、Team Knowledge 待审入库桥和审批门禁已接入",
    features: ["sourceTrace 校验", "proposalPayload", "ratingSuggestion", "approvalRequired=true", "SourceArtifact", "pending RefinementProposal", "KnowledgeItem rating suggestion migration", "officialSyncRecord", "official write 禁止项"],
    ...
    gap: "pending ratingSuggestion 到正式 KnowledgeItem rating review 的承接已接入；更细的正式图谱边生成规则仍待补。",
    entry: "review_record 或候选图谱已给出可追溯 candidateIds，且具备目标知识域。",
    operation: "生成带 sourceTrace、riskSummary、proposalPayload、ratingSuggestion、approvalRequired=true 的 CandidateStore 草稿包，并可提交到指定 knowledgeBaseId 的待审入库队列；审批门禁复用 Team Knowledge review/apply 与 rating suggestion review/create。",
    exit: "合格草稿提交后进入 steward_pending_knowledge_review；批准后进入 official_synced、记录 officialSyncRecord，并创建 KnowledgeItem 级 pending ratingSuggestion；拒绝后进入 steward_needs_revision。",
  ...
  "08": {
    ...
    agentStatus: "后端/API 已可生成候选图谱快照；前端可视化读取仍待接",
    features: ["candidate_graph 候选快照生成", "候选节点/边预览 payload", "断链报告", "未审节点清单", "候选/正式边界标识"],
    tools: ["/api/teams/{teamId}/workflow-orchestration/candidate-graph", "CandidateStore", "图结构渲染能力（待接入前端）"],
    humanGate: "确认候选图谱只作预览，不作为正式知识证据使用。",
    gap: "图谱页面读取逻辑和独立 candidate_graph.json 导出仍待接。",
    entry: "CandidateStore 中存在 paper_note / neuro_mechanism / mechanism_mapping / algorithm_hypothesis 等候选。",
    operation: "从 paperNoteIds、neuroMechanismIds、mechanismMappingIds 重建候选链路。",
    exit: "candidate_graph 候选快照能列出 nodes、edges、missingLinks、unreviewedNodes 和 officialBoundary。",
  ...
  "09": {
    ...
    agentStatus: "授权审批门禁已接入 Team Knowledge review/apply；平台负责正式 KnowledgeItem 的读取、RAG 检索和 Memory Graph 展示",
    features: ["正式审核确认", "正式 KnowledgeItem 创建/同步", "KnowledgeItem rating review", "RAG 可检索", "official_sync_record", "Memory Graph 可见"],
    tools: ["knowledge_query_tool", "knowledge_rag_retrieve_tool", "Team Knowledge 审核/应用流程（平台能力）"],
    humanGate: "必须通过授权审批门禁批准 ingestion pack。",
    gap: "ratingSuggestion 自动迁移到 KnowledgeItem 待审评级已接入；仍需要更细的正式 supports/maps_to/inspires 图谱边生成规则。",
    entry: "授权审批门禁已批准 pending RefinementProposal。",
    operation: "复用 Team Knowledge review/apply 创建 KnowledgeItem；旧 proposal 级 ratingSuggestion 标记 applied，新 KnowledgeItem 级 ratingSuggestion 保持 pending 等待评级审阅；RAG 通过 reviewed Team Knowledge 检索，Memory Graph 通过正式 KnowledgeItem 视图展示。",
    exit: "knowledgeItemIds、ratingSuggestionMigration、ragStatus、graphStatus、reviewedByAgentId 和 officialSyncRecord 均已记录。",
