...
const nodes = [
  ...
  },
  {
    ...
    summary: "steward_pack_draft 可提交到 Team Knowledge 待审队列，正式入库仍由审批门禁控制。",
    ...
    outputs: ["steward_pack_draft 候选", "SourceArtifact", "pending RefinementProposal", "pending ratingSuggestion", "steward_pending_knowledge_review 状态"],
    memory: "写入 Team Knowledge 待审对象；不创建正式 KnowledgeItem，不写正式 RAG/Graph，等待 Ingestion Approval Gate。",
    graph: "不写正式知识图谱；候选图谱只能作为 sourceTrace/candidateGraphId 追溯来源。",
    risks: ["绕过审批门禁", "目标知识域错误", "把 ingestion pack 草稿误当正式入库记录"],
    openQuestions: ["下一步如何审批 pending proposal 并生成 official_sync_record。"],
  },
  {
    id: "08",
...
const knowledgeRunbook = {
  ...
  stateFlow: [
    ...
    ["steward_pending_knowledge_review", "待审入库对象", "已创建 SourceArtifact、pending RefinementProposal，可选 pending ratingSuggestion；仍未写正式知识。"],
...
const implementationBlueprint = {
  ...
  milestones: [
    ...
    ["M5", "知识治理与正式同步", "steward_pack_draft 后端草稿包门禁与待审入库桥已落地：有效草稿可提交为 SourceArtifact + pending RefinementProposal，可选 pending ratingSuggestion。", "当前只写 Team Knowledge 待审对象；未通过授权审批门禁不能创建正式 KnowledgeItem/RAG/正式图谱。"],
  ],
  schemas: [
    "source_manifest",
    "paper_note",
  ...
  services: [
    ...
    ["memory_ingestion_bridge", "已复用 Team Knowledge create_ingestion_package，把 steward_pack_draft 映射为 SourceArtifact + pending RefinementProposal；可选创建 pending ratingSuggestion。"],
  ],
  communicationRoles: [
...
const knowledgeNodeRunbook = {
  ...
  "07": {
    ...
    agentStatus: "后端草稿包门禁与 Team Knowledge 待审入库桥已接入；正式审批/应用仍待接",
    features: ["sourceTrace 校验", "proposalPayload", "ratingSuggestion", "approvalRequired=true", "SourceArtifact", "pending RefinementProposal", "official write 禁止项"],
    ...
    gap: "已能创建待审 SourceArtifact/RefinementProposal；仍需补 Ingestion Approval Gate、正式 review/apply 和 official_sync_record。",
    entry: "review_record 或候选图谱已给出可追溯 candidateIds，且具备目标知识域。",
    operation: "生成带 sourceTrace、riskSummary、proposalPayload、ratingSuggestion、approvalRequired=true 的 CandidateStore 草稿包，并可提交到指定 knowledgeBaseId 的待审入库队列。",
    exit: "合格草稿提交后进入 steward_pending_knowledge_review；metadata.knowledgeIngestion 记录 knowledgeBaseId、sourceArtifactId、proposalId、ratingSuggestionId 和 officialBoundary=false。",
  ...
  "09": {
    ...
    agentStatus: "授权审批门禁为硬门；平台负责正式 Team Knowledge/RAG/Graph 同步",
    features: ["正式审核确认", "正式 KnowledgeItem 创建/同步", "RAG 索引更新", "official_sync_record", "正式图谱边生成"],
    tools: ["knowledge_query_tool", "knowledge_rag_retrieve_tool", "Team Knowledge 审核/应用流程（平台能力）"],
    humanGate: "必须通过授权审批门禁批准 ingestion pack。",
    gap: "需要正式图谱同步触发规则，以及 approved_to_ingest 到 official_synced 的证据记录。",
    entry: "授权审批门禁已批准 ingestion pack。",
    operation: "同步 Team Knowledge、正式 RAG 和正式知识图谱，并保留候选来源链。",
    exit: "officialKnowledgeItemId、ragStatus、graphStatus 和 approvedBy 均已记录。",
