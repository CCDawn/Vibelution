...
const implementationBlueprint = {
  ...
  milestones: [
    ...
    ["M6.55", "知识搜集四步闭环入库", "知识搜集阶段从旧五步收敛为搜索资料、资料提炼、资料审查、资料入库四张用户步骤卡；候选关系图和入库包不再作为独立用户阶段，而是合并进资料入库的一键 Agent 动作。", "第一阶段可从资料搜索结果一路推进到团队知识库正式 KnowledgeItem：搜索资料写 DataRecord/source_manifest，资料提炼展示候选资料，资料审查批量写 sourceQualityAssessment，资料入库复用候选关系、入库包、Team Knowledge source/proposal/review 门禁完成正式入库；正式 RAG/official graph 仍遵守现有审核边界。"],
    ...
    ["M6.58", "资料入库入口贯通候选资料", "资料入库步骤不再只看已通过资料数量；只要本轮已有候选资料，就允许点击 Agent 审查并入库。", "前端把候选资料数作为一键入库输入上限；后端 knowledge-collection/ingest 会先调用资料审查，再生成候选关系、资料入库包和团队知识库正式 KnowledgeItem，避免用户被卡在必须先手动跑完每个中间门禁。"],
