...
const implementationBlueprint = {
  ...
  milestones: [
    ...
    ["M6.54", "阶段 Agent 与过程契约根治", "候选图谱步骤绑定 candidate_graph / Candidate Graph Preview Agent，前审步骤绑定 knowledge_steward / 共享记忆前审 Agent；后端 candidate_graph 与 steward_pack_draft 产物写入 agentProcess。", "Teams 知识搜集页的 Agent过程只显示当前步骤过程，候选图谱和前审不再借用资料质量评估 Agent；用户能看到本步骤输入、输出、下一步和负责 Agent。"],
  ...
  services: [
    ...
    ["TeamsRoute workflow workspace", "已落地入口：Teams 页顶部固定横向切换条只暴露 AI 搜索范围团队和 ai科学研究团队，左侧栏已删除；监督进化/自进化模式页各自读取系统 Team 并展示只读团队画布；选择 ai科学研究团队后直接进入 research-team 专属科研控制台；总览只显示知识搜集、实验、迭代三张阶段卡，每张卡一个主操作和一个阶段详情入口。知识搜集卡会显示当前批次、待处理、候选和查询数，主按钮按状态执行开始、下一批搜索、筛选入口或打开工作台，另有新一轮搜集入口。知识搜集页位于 /teams?team=research-team&researchView=knowledge_collection，默认展示高价值状态摘要、状态驱动主操作和对话式 source collection 过程流；点击知识搜集主按钮会通过 stage-rounds/start 创建轮次并立即排队第一批后台 source collection 搜索；执行 source collection 搜索时会写入 source_collection_run 运行快照，让 AppShell 顶部“正在进行”优先显示知识搜集；资料搜集 run 列表通过 data-processing 服务端 teamId/startedFrom 过滤，避免被其它团队批次挤掉；后台 accepted 后 activeWorkRun 会进入控制台和对话流，直到 runStatus 收口；资料搜集、资料筛选、候选入库、候选图谱、共享记忆前审五张步骤卡各自带主按钮并作为页内子页索引，顶部卡片只保留状态、关键数量和主操作，不再显示输入/输出小框；底部只显示当前阶段内容，并拆成结果与 Agent过程；右侧只显示当前步骤操作、当前资料详情、关键数量和当前步骤 Agent 配置；资料筛选会调用资料质量评估 Agent 批量执行，完成后展开、滚动并高亮页内筛选详情；KV/prompt cache 由后端从当前模型库解析，不再依赖旧固定模型 ID，过程流把 KV 缓存模型与执行 Agent 模型分层展示；稳定前缀、动态增量、assignment、query、存储路径和手工回写保留在可展开详情中；默认层移除解释性小字且使用实体工作台背景，避免主题壁纸压住正文；实验页新增实验计划账本，可生成计划草稿并登记 active baseline artifact、smoke result、full-run result，full-run passed 后可通知 Knowledge Steward Agent 处理 experiment_result_pack；实验页和迭代页共享 Research Loop 模板面板，可按算法模型、仿真模拟、数据集基准、环境探针等模板登记证据和决策；旧 source_collection 参数兼容映射到知识搜集。"],
...
const knowledgeNodeRunbook = {
  "01": {
    ...
    agentStatus: "可复用通用 data-processing assignment / source-extraction / source-quality 后端已接入；Teams 搜索资料执行台、资料提炼页、资料审查台、科研总览控制台、KV 缓存对话轨迹、启动复用反馈、metadata-only 搜索执行闭环、高价值信息层、中文控制台、默认层去说明化、步骤卡主操作入口、右侧当前步骤 Agent 配置、资料审查页内反馈、资料审查 Agent 批量执行、候选库可视化二级索引、知识搜集阶段四步控制台、资料审查 Agent 复审入口、一键排队首批后台搜索、搜索资料当前动作和已收集资料分层、搜集结果状态徽标完整显示、搜集结果 DOI/网页/本地文件来源可溯源、资料提炼结果滚动与待审查交接、四步完整列表、来源类型过滤、阶段结果/Agent过程双页、阶段输入输出交接、Agent 执行过程流水、结果分页、实体可读工作台、阶段卡降噪、资料入库 Agent 生成关系并通知知识库 Agent、资料入库可从候选资料触发审查并发送入库请求、KV 缓存模型与执行 Agent 模型分层、阶段协调显式启动已接入",
    features: ["ResearchStageRound 建立", "科研控制台一键进入知识搜集", "总览页只显示三阶段主操作", "知识搜集状态驱动按钮", "知识搜集中文展示映射", "默认层去说明化", "右侧主操作唯一化", "资料审查入口常驻", "步骤卡主操作入口", "右侧当前步骤 Agent 配置", "步骤卡二级索引", "四步阶段控制台", "阶段结果/Agent过程双页", "实体可读工作台", "阶段卡降噪", "阶段输入输出交接", "Agent 执行过程流水", "KV 缓存模型分层展示", "执行 Agent 模型右侧展示", "四步结果分页", "结果区独立滚动", "搜索资料当前动作", "已收集资料结果区", "搜集结果来源溯源", "搜集结果状态徽标", "资料提炼待审查交接", "资料审查 Agent 复审", "候选库可视化面板", "资料入库 Agent 通知入库", "资料入库候选触发", "资料审查页内详情", "资料审查展开高亮反馈", "资料审查 Agent 批量执行", "source-quality/assess-batch", "source-quality force 重审", "新一轮搜集入口", "active 知识搜集轮次复用反馈", "一键排队首批搜索", "全局运行态知识搜集优先", "步骤功能 Agent 聚焦", "四步完整列表", "来源类型过滤", "阶段协调显式启动", "DataProcessingRun 建立", "DataProcessingRun reviewing 收口", "DataSearchPlan/querySeeds 搜索计划", "KV/prompt cache 启动门禁", "稳定前缀/动态增量契约", "CollectionAssignment 搜索资料任务", "assignedQueries 角色分配", "promptCachePartition 角色分区", "Teams 启动搜集批次", "Teams 查看 run/assignment/query/KV 状态", "Teams 高价值摘要层", "Teams 搜索下一批", "Crossref 元数据搜索执行器", "搜索执行事件进入搜集对话流", "DataRecord 通用记录", "DataRecord 自动导入 source_manifest", "metadata-only 下载边界", "手工 CollectionOutput 回写", "本地资料登记", "source_manifest 写入", "文件路径/页码/来源可信度记录", "本地 PDF sha256/pageAnchors/excerpt 抽取", "sourceQualityAssessment 审查", "可选 research_knowledge_query_tool 查重"],
