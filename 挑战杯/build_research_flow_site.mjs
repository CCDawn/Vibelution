...
const implementationBlueprint = {
  ...
  deliveredSlice: [
    ...
    ["Team 前端入口", "/teams?team=research-team", "读取 /workflow-orchestration、/stage-rounds/status、/official-model-evidence/status、/candidates?limit=8、/knowledge-ingestion/status 和通用 data-processing run/status/assignment；左侧只做团队选择/创建，research-team 页面顶部提供科研流程二级索引条，当前节点内容区优先展示科研三阶段启动台，并按索引节点单页展示科研总览、模型证据链、资料搜集执行台、候选仓库、校验摘要、知识入库漏斗、actionItems 和 officialBoundary；非科研团队显示占位，不初始化 workflow。"],
...
const knowledgeNodeRunbook = {
  "01": {
    ...
    gap: "科研三阶段启动台、资料搜集阶段轮次、资料搜集批次启动、DataSearchPlan/query seed 契约、通用资料搜集 assignment/output、Teams 执行台、DataRecord 到 source_manifest 导入桥和 source-quality 筛选台已接入；真实外部搜索执行器、长 PDF 自动分批、章节识别和非 PDF 资料解析仍待补。",
    ...
    <main class="shell">
      ...
      <section id="overview" class="review-dashboard">
        ...
        <aside class="dashboard-panel">
          <h2>审核摘要</h2>
          <div class="kpi-grid">
            <div class="kpi"><b>当前阶段</b><strong>M6.11</strong><span>资料搜集执行台、资料质量筛选、模型证据链和 paper_note 分块计划已接入。</span></div>
          ...
          <div class="focus-list">
            ...
            <div class="focus-item"><b>下一步</b><span>按 chunkId 执行 paper_note 提炼，补多 chunk 草稿合并策略和机制提炼执行台。</span></div>
          </div>
        </aside>
      </section>
