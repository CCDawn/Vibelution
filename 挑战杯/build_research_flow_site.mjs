...
const knowledgeNodeRunbook = {
  "01": {
    ...
    gap: "科研三阶段启动台、资料搜集阶段轮次、资料搜集批次启动、DataSearchPlan/query seed 契约、KV/prompt cache 启动门禁、缓存分区对话轨迹、通用资料搜集 assignment/output、Teams 执行台、DataRecord 到 source_manifest 导入桥和 source-quality 筛选台已接入；真实外部搜索执行器、搜索/下载事件的自动回写、长 PDF 自动分批、章节识别和非 PDF 资料解析仍待补。",
    ...
    <main class="shell">
      ...
      <section id="overview" class="review-dashboard">
        ...
        <aside class="dashboard-panel">
          <h2>审核摘要</h2>
          <div class="kpi-grid">
            <div class="kpi"><b>当前阶段</b><strong>M6.15</strong><span>知识搜集页已把 KV/prompt cache 门禁和缓存分区放入对话式搜集轨迹，后续真实搜索可复用稳定前缀。</span></div>
