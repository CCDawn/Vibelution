...
const implementationBlueprint = {
  ...
  activeTeam: {
    ...
    defaultView: "选中 ai科学研究团队后，主工作区顶部显示科研流程二级导航：科研总览、团队协调、知识入库、候选图谱、候选资料、团队沟通。",
  ...
  milestones: [
    ...
    ["M0.2", "Teams 科研流程二级导航", "左侧团队栏降级为团队创建、模板和团队列表；科研流程索引不再与团队同级，而是进入 ai科学研究团队主工作区顶部作为二级导航。", "用户先选团队，再在选中团队内部切换科研总览、资料搜集执行、团队协调、知识入库、候选图谱、候选资料、团队沟通和组织画布；组织画布仍可编辑但属于附属视图。"],
    ...
    ["M6.6", "Teams 资料搜集执行台", "research-team 页面新增资料搜集执行面板，可启动 source collection run、查看最近批次、run status、assignment、assignedQueries，并手工提交 CollectionOutput。", "手工回写会先写入 DataRecord，再通过 Team workflow 导入桥转成 source_manifest 候选；仍不触发真实外部搜索，不写正式 Team Knowledge/RAG/official graph。"],
  ],
  schemas: [
    "DataProcessingRun",
    "DataRecord",
  ...
  deliveredSlice: [
    ...
    ["服务文件", "core/web/services/team_workflow_orchestration_service.py", "Team 编排、资料搜集批次启动、候选登记、DataRecord 导入、转移请求、协调 Agent 裁决和轻量运行事件日志。"],
    ["路由文件", "core/web/routes/team_workflows.py", "提供工作流查询/确保、资料搜集批次启动、候选资料登记、DataRecord 导入、转移提交、转移裁决 API。"],
    ...
    ["Team 前端入口", "/teams?team=research-team", "读取 /workflow-orchestration、/candidates?limit=8、/knowledge-ingestion/status 和通用 data-processing run/status/assignment；左侧只做团队选择/创建，research-team 主工作区顶部提供科研流程二级导航，展示科研流程状态、资料搜集执行台、候选仓库、校验摘要、知识入库漏斗、actionItems 和 officialBoundary；非科研团队显示占位，不初始化 workflow。"],
    ["资料搜集执行台", "TeamsRoute source collection panel", "可在 research-team 页面启动 source collection run、查看 DataSearchPlan/querySeeds/assignment/assignedQueries，并手工提交一条 CollectionOutput；提交后自动导入 source_manifest 候选。"],
    ...
    ["验证", "tests/test_data_processing_service.py + tests/test_data_processing_routes.py + tests/test_team_workflow_orchestration_service.py + tests/test_team_workflow_routes.py + TeamsRoute.layout.test.ts", "覆盖通用数据处理首切、source-collection run 启动、DataSearchPlan/query seed 契约、DataRecord 导入 source_manifest、Teams 资料搜集执行台契约、主路径、非 ownerAgent 不能写最终状态、本地模型任务包、输出校验和知识入库状态总览。"],
  ],
};
...
const knowledgeNodeRunbook = {
  "01": {
    ...
    agentStatus: "可复用通用 data-processing assignment / source-extraction 后端已接入；Teams 资料搜集执行台已接入",
    features: ["DataProcessingRun 建立", "DataSearchPlan/querySeeds 搜索计划", "CollectionAssignment 资料搜集任务", "assignedQueries 角色分配", "Teams 启动搜集批次", "Teams 查看 run/assignment/query 状态", "手工 CollectionOutput 回写", "DataRecord 通用记录", "DataRecord 自动导入 source_manifest", "本地资料登记", "source_manifest 写入", "文件路径/页码/来源可信度记录", "本地 PDF sha256/pageAnchors/excerpt 抽取", "可选 research_knowledge_query_tool 查重"],
    tools: ["/api/teams/{teamId}/workflow-orchestration/source-collection-runs", "/api/data-processing/runs", "/api/data-processing/runs/{runId}/collection-assignments", "/api/data-processing/runs/{runId}/collection-assignments/{assignmentId}/outputs", "/api/teams/{teamId}/workflow-orchestration/data-processing/runs/{runId}/records/{recordId}/source-candidate", "research_knowledge_query_tool", "/api/teams/{teamId}/workflow-orchestration/candidates/{candidateId}/source-extraction"],
    localModelUse: "本地 9B 模型适合做标题/摘要/片段初筛，输出 relevanceScore、topicTags、excludeReason，不直接决定正式纳入。",
    humanGate: "确认资料允许分析、来源可信度和是否纳入本轮。",
    gap: "资料搜集批次启动、DataSearchPlan/query seed 契约、通用资料搜集 assignment/output、Teams 执行台和 DataRecord 到 source_manifest 导入桥已接入；真实外部搜索执行器、长 PDF 自动分批、章节识别和非 PDF 资料解析仍待补。",
    entry: "用户提供 PDF、论文、赛题或补充资料；文件可在挑战杯工作区定位。",
    operation: "由 Research Coordination Agent 在 Teams 资料搜集执行台或 API 调用 source-collection-runs 启动批次，系统基于 topic/goal/scope/querySeeds 生成 searchPlan、queries、roleAssignmentInputs 和 resultWritebackContract；同时建立通用 DataProcessingRun，让 data_discovery/source_acquisition/content_extraction/source_quality 等功能 Agent 领取带 assignedQueries 的 CollectionAssignment、提交 CollectionOutput。执行台支持手工回写一条 CollectionOutput，随后自动通过 Team workflow 导入桥把新 DataRecord 幂等转成 source_manifest 候选。对本地 PDF 运行 source-extraction，计算 sha256、生成 pageAnchors/excerpt，不改写原文，不自动混入联网资料。",
    exit: "每个资料批次有 dataSearchPlanRef、planned queries、assignedQueries 和 resultWritebackContract；每个资料项先有 DataRecord/sourceRef/rawLocation/title/qualitySignals/collectionTrace，再有 source_manifest 的 id、path、type、allowedForAnalysis、sha256、pageScope 和 sourceExtraction.pageAnchors。",
