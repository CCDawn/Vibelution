...
const implementationBlueprint = {
  ...
  milestones: [
    ...
    ["M6.2", "通用数据处理底座", "计划新增 domain-neutral data-processing substrate，把 intake、extract、transform、validate、review、publish 做成通用 pipeline/profile/adapter；挑战杯科研只是其中一个 profile。", "通用层不写 challenge cup、神经学、steward 或 qwen 专名；正式 Team Knowledge/RAG/图谱写入通过 publish adapter 和审批门禁完成。"],
  ],
  schemas: [
    "source_manifest",
    "paper_note",
  ...
  services: [
    ...
    ["data_processing_substrate", "待实施：通用数据处理底座，提供 DataPipelineProfile、DataProcessingRun、DataRecord、DataArtifact、ValidationReport、ReviewDecision、PublishTarget 和 ProcessingTrace；挑战杯通过 profile/adapter 复用。"],
  ],
  communicationRoles: [
  ...
  deliveredSlice: [
    ...
    ["通用数据处理底座计划", "docs/plans/2026-06-07-general-data-processing-substrate.md", "下一步先实现通用 data-processing API/存储/状态机/适配器/UI，再把挑战杯科研流程作为 challenge_cup_research_ingestion profile 接入；不把通用能力写死到科研或神经学场景。"],
