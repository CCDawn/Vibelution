...
const implementationBlueprint = {
  ...
  milestones: [
    ...
    ["M6.42", "资料搜集过程与结果分层", "知识搜集页的资料搜集子页改为两块：当前搜集动作只显示一条正在进行或最近完成的动作，已收集资料集中展示进入 CandidateStore 的候选资料。", "顶部阶段卡和总览指标使用“原始资料、搜索问题、候选资料、已通过筛选、待 Agent 筛选”等中文业务词，避免把 record/query/assignment 等内部计数暴露成主路径信息。"],
    ...
    ["M6.45", "知识搜集信息架构降噪", "知识搜集页顶部五张步骤卡删除重复 Agent 槽，右侧详情底部集中展示当前步骤 Agent 配置；搜集结果区改名为已收集资料，并把统计拆成原始资料记录、候选资料、已通过筛选和待 Agent 筛选。", "用户不需要理解 DataRecord、query、source_manifest 等内部术语，也能判断哪些是流程步骤、哪些是实际资料结果、当前步骤由哪个 Agent 和模型承接。"],
  ],
  schemas: [
    "DataProcessingRun",
    "DataRecord",
