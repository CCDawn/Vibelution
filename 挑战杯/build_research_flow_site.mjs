...
const implementationBlueprint = {
  ...
  activeTeam: {
    ...
    note: "挑战杯科研流程直接绑定当前 Vibelution ai科学研究团队，不另建新团队；Teams 页左上角只保留 AI 搜索范围团队和 ai科学研究团队两个固定入口，不再展示创建团队、模板创建或完整团队列表；选择 ai科学研究团队后直接进入团队专属科研工作台，团队内容区只露出知识搜集、实验、迭代三阶段入口；资料搜集已折叠进知识搜集阶段页，组织画布仅保留为附属结构视图。",
  ...
  architecture: [
    ...
    ["团队沟通复用层", "复用 Team registry、Team canvas、linkedChatRoom、ChatRoom round 和 research_coordination purpose；Teams 团队页左上角固定切换 AI 搜索范围团队 / ai科学研究团队，research-team 主工作区保留三阶段入口，组织画布作为附属结构视图保留。"],
  ...
  milestones: [
    ...
    ["M6.17", "知识搜集搜索执行最小闭环", "新增 source-collection-runs/{runId}/search/execute，知识搜集页提供“执行下一批搜索”按钮；系统可按 assignment/query 调用 Crossref 元数据搜索，生成执行事件、DataRecord，并自动导入 source_manifest 候选。", "这是 metadata-only 最小执行器：只下载来源元数据和引用，不抓取全文网页/PDF；结果仍停在候选资料仓库，不写正式 Team Knowledge、不写 RAG、不写 official graph。"],
    ...
    ["M6.19", "固定团队入口下拉", "Teams 左侧团队管理区替换为左上角固定团队下拉，只暴露 AI 搜索范围团队和 ai科学研究团队；选择项会更新 team URL 并进入对应团队页面。", "当前页面不再显示创建团队表单、模板创建或完整团队列表，避免自定义团队入口挤占科研团队主工作区；后端团队创建和模板 API 不删除，仍可由其它管理面承接。"],
  ],
  schemas: [
    "DataProcessingRun",
    "DataRecord",
  ...
  services: [
    ...
    ["TeamsRoute workflow workspace", "已落地入口：Teams 页左上角固定下拉只暴露 AI 搜索范围团队和 ai科学研究团队；监督进化/自进化模式页各自读取系统 Team 并展示只读团队画布；选择 ai科学研究团队后直接进入 research-team 专属科研工作台；科研一级索引统一为知识搜集、实验、迭代三阶段。知识搜集页位于 /teams?team=research-team&researchView=knowledge_collection，默认展示高价值状态摘要、下一步动作和对话式 source collection 过程流；KV/prompt cache、稳定前缀、动态增量、assignment、query、存储路径和手工回写保留在可展开详情中；实验/迭代页先提供规划启动、轮次状态和边界说明；旧 source_collection 参数兼容映射到知识搜集。"],
  ...
  deliveredSlice: [
    ...
    ["Team 前端入口", "/teams?team=research-team", "读取 /workflow-orchestration、/stage-rounds/status、/official-model-evidence/status、/candidates?limit=8、/knowledge-ingestion/status 和通用 data-processing run/status/assignment；左上角只用固定下拉选择 AI 搜索范围团队或 ai科学研究团队，点击 research-team 后直接进入团队专属科研工作台，内容区只展示知识搜集、实验、迭代三阶段入口和三阶段启动台；非这两个团队不在当前入口暴露。"],
...
const knowledgeNodeRunbook = {
  "01": {
    ...
    agentStatus: "可复用通用 data-processing assignment / source-extraction / source-quality 后端已接入；Teams 资料搜集执行台、候选资料筛选台、三阶段启动台、KV 缓存对话轨迹、启动复用反馈、metadata-only 搜索执行闭环和高价值信息层已接入",
    features: ["ResearchStageRound 建立", "三阶段启动台一键进入知识搜集", "active 知识搜集轮次复用反馈", "自动启动团队协调 round", "DataProcessingRun 建立", "DataSearchPlan/querySeeds 搜索计划", "KV/prompt cache 启动门禁", "稳定前缀/动态增量契约", "CollectionAssignment 资料搜集任务", "assignedQueries 角色分配", "promptCachePartition 角色分区", "Teams 启动搜集批次", "Teams 查看 run/assignment/query/KV 状态", "Teams 高价值摘要层", "Teams 执行下一批搜索", "Crossref 元数据搜索执行器", "搜索执行事件进入搜集对话流", "DataRecord 通用记录", "DataRecord 自动导入 source_manifest", "metadata-only 下载边界", "手工 CollectionOutput 回写", "本地资料登记", "source_manifest 写入", "文件路径/页码/来源可信度记录", "本地 PDF sha256/pageAnchors/excerpt 抽取", "sourceQualityAssessment 筛选", "可选 research_knowledge_query_tool 查重"],
    ...
    gap: "科研三阶段启动台、资料搜集阶段轮次、资料搜集批次启动、active 轮次复用反馈、DataSearchPlan/query seed 契约、KV/prompt cache 启动门禁、缓存分区对话轨迹、通用资料搜集 assignment/output、Teams 高价值摘要层、metadata-only 搜索执行器、DataRecord 到 source_manifest 导入桥和 source-quality 筛选台已接入；全文网页/PDF 下载、长 PDF 自动分批、章节识别和非 PDF 资料解析仍待补。",
    entry: "用户提供 PDF、论文、赛题或补充资料；文件可在挑战杯工作区定位。",
    operation: "用户在 ai科学研究团队右侧顶部点击“启动知识搜集”或“开启新一轮”，stage-rounds/start 创建或续用 ResearchStageRound，并复用 source-collection run 建立 DataProcessingRun；如果已有 active knowledge_collection，接口返回 continuedSourceRunRef、run 和 assignments，页面直接提示已复用的 runId、记录数、待回写任务数和搜索执行状态。随后尝试基于 linkedChatRoom 启动轻量团队协调 round。系统基于 topic/goal/scope/querySeeds 生成 searchPlan、queries、roleAssignmentInputs 和 resultWritebackContract，让 data_discovery/source_acquisition/content_extraction/source_quality 等功能 Agent 领取带 assignedQueries 的 CollectionAssignment。知识搜集页默认展示当前轮状态、下一步动作、候选/待处理/查询/KV 摘要和对话式搜集过程；assignment、query、KV 分区、存储路径和手工回写收进可展开详情。用户可点击“执行下一批搜索”，通过 source-collection-runs/{runId}/search/execute 执行 metadata-only Crossref 查询，生成 search.executed、storage.data_record_written、storage.source_manifest_imported 等对话式事件，把结果写成 DataRecord 并幂等导入 source_manifest 候选；执行台仍支持手工回写一条 CollectionOutput。候选资料页可调用 source-quality/assess 写入 sourceQualityAssessment 并标记通过筛选或退回补资料。对本地 PDF 运行 source-extraction，计算 sha256、生成 pageAnchors/excerpt，不改写原文，不自动混入联网全文。",
    ...
    <main class="shell">
      ...
      <section id="overview" class="review-dashboard">
        ...
        <aside class="dashboard-panel">
          <h2>审核摘要</h2>
          <div class="kpi-grid">
            <div class="kpi"><b>当前阶段</b><strong>M6.19</strong><span>Teams 左侧入口已收敛为 AI 搜索范围团队 / ai科学研究团队固定下拉，不再展示创建、模板和完整团队列表。</span></div>
