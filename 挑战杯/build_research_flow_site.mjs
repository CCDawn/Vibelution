...
const nodes = [
  {
    ...
    title: "资料进入工作区",
    status: "Evidence Ledger 与前端证据入口已接入",
    statusKind: "active",
    role: "资料寻找 Agent",
    summary: "建立稳定证据源，并把本地 PDF 转成可引用的页码锚点和证据账本。",
    ...
    actions: [
      ...
      "把 candidateExtractions / recordExtractions 中的 claims、keyFindings、citations、sourceRefs 和 evidenceRefs 物化为 metadata.contentExtraction.evidenceLedger。",
      "用 evidence_ready / missing_evidence_anchor 标记每条结构化提炼是否具备 citation/page/source/evidence 锚点。",
      "Teams 资料卡、资料提炼详情和资料关系整理节点显示 Evidence Ledger 状态，缺锚点条目进入补证据视图。",
    ...
    outputs: ["资料清单", "sourceFiles 引用", "sourceExtraction.pageAnchors", "contentExtraction.evidenceLedger", "source_collection_run 运行快照", "query 级 CollectionOutput", "Paper 原始来源节点"],
  ...
  },
  {
    ...
    slug: "paper-note",
    title: "生成 paper_note",
    status: "自动草稿桥、分块计划与 Evidence Ledger 已接入",
    statusKind: "active",
  ...
  },
  {
    ...
    graph: "资料关系整理边：paper_note supports neuro_mechanism。",
  ...
  },
  {
    ...
    graph: "资料关系整理可展示 NeuroMechanism maps_to MechanismMapping，正式图谱同步仍由审批门禁控制。",
  ...
  },
  {
    ...
    graph: "资料关系整理边：mechanism_mapping inspires algorithm_hypothesis，占位保留 neuro_mechanism inspires algorithm_hypothesis。",
  ...
  },
  {
    ...
    graph: "资料关系整理可展示 reviewed_by、needs_revision 边；rejected 归档候选从可推进图谱中隔离，并在 summary 中计数。",
  ...
  },
  {
    ...
    status: "待资料入库审核",
    statusKind: "active",
    role: "资料入库 Agent · source_ingestor",
    summary: "source_ingestor 在 ingestion 阶段审核本轮候选；approved 回写会由后端经 Team Knowledge 治理门禁创建正式 KnowledgeItem。",
    objective: "把已通过资料提炼并具备关系上下文的本轮候选交给 source_ingestor 审核，并通过阶段回写触发 SourceArtifact、pending RefinementProposal、review/apply 和正式 KnowledgeItem 闭环。",
    inputs: ["review_record 候选", "algorithm_hypothesis 候选", "candidate_graph 快照", "目标知识域"],
    actions: [
      "校验回写中的 approved candidateIds、targetDomain、sourceTrace、riskSummary、proposalPayload、ratingSuggestion。",
      "只采纳本轮 source_manifest 且已被资料提炼阶段保留的候选，拒绝跨轮或无有效内容资料进入正式链路。",
      "禁止绕过 source_collection_stage_writeback_tool、Team Knowledge source review、proposal review/apply gate 直接写库。",
      "知识搜集入库入口进入 source_ingestor direct session task，而不是只发送 inbox 请求后等待人工二次启动。",
      "source_ingestor 在 ingestion 阶段回写 approved 后，后端构建或复用 steward_pack_draft，并提交 Team Knowledge 待审队列。",
      "后端复用 source review、RefinementProposal review/apply 和 ingestion approval gate；批准后进入 official_synced，拒绝后回到 steward_needs_revision。",
      "重复 approved 回写会复用已有 official sync，不重复创建 KnowledgeItem。",
    ...
    outputs: ["source_collection_stage_task", "materializedKnowledgeIngestion", "steward_pack_draft 候选", "SourceArtifact", "pending RefinementProposal", "proposal ratingSuggestion", "KnowledgeItem", "KnowledgeItem ratingSuggestion", "officialResearchGraph", "official_sync_record", "knowledge_ingestion_status", "coordination_status_panel"],
    memory: "source_ingestor ingestion 阶段 approved 回写会经治理门禁创建正式 KnowledgeItem；资料寻找、资料提炼和资料关系整理阶段仍只写候选/任务结果，不写正式 Team Knowledge、RAG 或 official graph。",
  ...
  },
  {
    ...
    outputs: ["candidate_graph 候选快照", "Teams 资料关系整理面板", "missingLinks 断链报告", "unreviewedNodes 未审节点清单"],
    memory: "资料关系整理只进入 CandidateStore 草稿区，不进入正式 RAG。",
    graph: "只展示资料关系整理；正式图谱同步必须等 ingested。",
    risks: ["资料关系整理被误读为正式知识", "links 指向不存在对象"],
  ...
  },
  {
    ...
    title: "实验迭代闭环",
    status: "实验规划与证据账本操作 Agent 已接入",
    statusKind: "active",
    role: "Experiment Planner Agent / Experiment Ledger Agent",
    summary: "实验阶段拆成两个操作型 Agent：实验规划 Agent 只生成可审查计划，实验证据 Agent 只登记 baseline、smoke、full-run 和入库请求证据；两者都不接沙盒 runner，不自动训练。",
    objective: "根据研究问题选择验证模板，记录所需证据、指标、日志和实验边界；把实验结果整理为后续迭代输入，而不是自动训练或自动入库。",
    inputs: ["experiment ResearchStageRound", "algorithm_hypothesis", "experimentPlan", "baseline/metric/dataset/smokePlan", "researchQuestion", "templateId"],
    actions: [
      "Experiment Planner Agent 使用 challenge_cup_experiment_context_tool 读取实验轮次、算法假设、计划草稿和缺口。",
      "Experiment Planner Agent 使用 challenge_cup_experiment_writeback_tool(create_plan) 写入 experiment_plan 草稿，只落 Team workflow ledger。",
      "Experiment Ledger Agent 使用 challenge_cup_experiment_writeback_tool 登记 baseline_artifact、smoke_result、full_run_result 或 knowledge_ingestion_request。",
      "GET experiments/status 聚合实验轮次、算法假设候选、计划草稿、baseline/metric/dataset/smoke 缺口。",
      "POST experiments/plan 生成 experiment_plan 草稿，写入 experiment_plans/index.json 并挂回 ResearchStageRound。",
      "POST experiments/plans/{planId}/baseline-artifact 登记 active baseline artifact，解除 baseline blocker 并开放 smoke gate。",
      "POST experiments/plans/{planId}/smoke-result 登记 smoke 指标、日志和结果路径；passed 才解除 full-run blocker。",
      "POST experiments/plans/{planId}/full-run-result 只登记人工 full-run 结果证据；知识库管理员 入库请求仍走显式审批边界。",
      "POST experiments/plans/{planId}/knowledge-ingestion-request 生成 experiment_result_pack 并通知知识库管理员，正式知识/RAG 仍等待复核。",
      "GET research-loop/templates 暴露算法模型、仿真模拟、数据集基准、环境探针、深度资料审查、论文结论复核模板。",
      "POST research-loop/loops 按 templateId 创建实验迭代 loop，可关联 stageRoundId、planId、candidateIds 和研究问题。",
      "POST research-loop/loops/{loopId}/evidence 登记人工证据、指标、artifact/log/source/dataset/environment refs 和 commandPreview。",
      "POST research-loop/loops/{loopId}/decision 在证据齐备后登记 promote/repair/more-evidence/reject/writeup 决策，并生成下一轮 iterationProposal。",
      "强制边界：不接沙盒 runner，不触发训练 runner，不自动写 ExperimentResult，不自动写正式 Team Knowledge/RAG/official graph。",
    ],
    outputs: ["experiment_plan 草稿", "active_baseline_artifact", "smoke_result", "full_run_result 手工账本", "experiment_result_pack", "research_loop", "evidenceRecords", "decision", "iterationProposal", "readinessChecklist", "operation_agent_writeback"],
    memory: "实验计划、实验结果证据和 Research Loop 只进入 Team workflow runtime memory / team_workflow_ledger；实验结果是否入库仍取决于 full-run 证据、证据复核 Agent 和知识治理 Agent。",
    graph: "实验操作 Agent 不创建正式图谱边；正式边仍等待 知识库管理员 审批后从 ExperimentResult/KnowledgeItem trace 生成。",
    risks: ["计划草稿被误认为已训练", "commandPreview 被误当已执行", "模板证据缺口没有补齐就推进迭代", "baseline 不公平", "实验 Agent 被误授搜索或代码执行工具"],
    openQuestions: ["沙盒 runner、训练 runner 和自动仿真执行器明确后置；当前阶段只做计划、证据账本、决策和 Steward 入库请求链路。"],
  },
  {
    id: "11",
    slug: "iteration-versioning",
    title: "持续迭代与版本化",
    status: "迭代决策与候选版本账本操作 Agent 已接入",
    statusKind: "active",
    role: "Iteration Planner Agent / Candidate Versioning Agent",
    summary: "迭代阶段拆成两个操作型 Agent：迭代决策 Agent 只登记 Research Loop 证据与决策，版本治理 Agent 只维护 candidate version ledger、supersede/derive/reject 关系和拒绝归档。",
    objective: "管理候选修订、拒绝原因、同类假设合并和下一轮实验建议，同时保持候选版本账本与正式知识/RAG/official graph 的边界。",
    inputs: ["research_loop", "experiment_result_pack", "candidateIds", "review_record", "versionHistory", "ingested 知识状态"],
    actions: [
      "Iteration Planner Agent 使用 challenge_cup_iteration_context_tool 读取 Research Loop、实验计划和证据缺口。",
      "Iteration Planner Agent 使用 challenge_cup_iteration_writeback_tool(create_loop/record_evidence/record_decision) 写入 Research Loop 账本。",
      "Candidate Versioning Agent 使用 challenge_cup_versioning_context_tool 读取 candidate_versions/index.json 中的 versionHistory、relations 和 rejectionArchive。",
      "Candidate Versioning Agent 使用 challenge_cup_versioning_writeback_tool(record_version/supersede/derive/reject) 维护候选版本链。",
      "强制边界：不自动应用迭代决策，不写正式知识，不写 RAG，不写 official graph，不创建训练任务。",
    ],
    outputs: ["research_loop", "evidenceRecords", "decision", "iterationProposal", "candidate_versions/index.json", "versionHistory", "relations", "rejectionArchive"],
    memory: "迭代决策和版本治理只进入 Team workflow runtime memory / team_workflow_ledger；正式知识仍由 知识库管理员 与审批门禁负责。",
    graph: "候选版本关系只作为账本关系存在；不创建正式图谱边，后续正式边必须来自已审批 KnowledgeItem trace。",
    risks: ["同类假设重复", "拒绝原因丢失", "正式知识与候选版本混淆", "迭代 Agent 被误授搜索、代码执行或正式知识写入工具"],
    openQuestions: ["版本治理当前是轻量账本，不替代后续正式 ExperimentResult schema 或交付材料版本库。"],
  },
  {
    id: "12",
  ...
  },
  {
    ...
    graph: "不直接写入资料关系整理或正式图谱；它约束图谱相关开发变化必须更新对应流程节点。",
...
const knowledgeRunbook = {
  scope: "只跑通知识搜集第一阶段：搜索资料、资料提炼、资料审查、资料入库；不进入训练实验、算法实现或比赛交付。",
  minimalPath: [
  ...
  stateFlow: [
    ...
    ["source_ingestor_agent_notified", "资料入库任务已投递", "阶段任务已投递到 source_ingestor direct session，等待 ingestion 阶段结构化回写。"],
    ["steward_pending_knowledge_review", "待审入库对象", "source_ingestor approved 回写已创建 SourceArtifact、pending RefinementProposal，可选 pending ratingSuggestion，并推动审批门禁进入正式入库闭环。"],
    ["knowledge_ingestion_needs_review", "入库状态待审", "状态总览显示 pendingProposalCount、actionItems 和 officialBoundary，便于团队协调员分派审核。"],
    ["candidate_graph_visible", "资料关系整理可见", "只进入资料关系整理，不能当正式事实。"],
  ...
  artifacts: [
    ...
    ["steward_ingestion_pack_YYYYMMDD_NNN.json", "知识治理摄取包", "治理建议、评级建议、目标知识域和正式入库申请；由 source_ingestor ingestion 阶段 approved 回写触发后端构建或复用。"],
    ["stage_session_tasks.json", "资料入库阶段任务", "记录 teamId、runId、stageId=ingestion、agentRole=source_ingestor、approved candidateIds 和 materializedKnowledgeIngestion 闭环结果。"],
    ["candidate_graph.json", "资料关系整理预览", "展示候选链路、断链和未审节点。"],
  ...
  gates: [
    ...
    ["资料入库任务门", "资料入库阶段必须投递给 source_ingestor 的 ingestion stage-session task，并通过 source_collection_stage_writeback_tool 回写 approved 候选", "source_ingestor_agent_notified", "停在 steward_pack_draft"],
    ["治理门", "后端只接受本轮且 source_quality approved 的 source_manifest，并复用 Team Knowledge source review、proposal review/apply gate", "steward_pending_knowledge_review", "退回 steward_needs_revision"],
    ["入库门", "授权审批门禁确认正式摄取；approved 创建正式 KnowledgeItem，rejected 回到 steward_needs_revision", "official_synced", "停在资料关系整理，不进正式图谱"],
  ],
  transferPrinciples: [
    ...
    "流程转移关闭后必须更新 status_digest、候选 index 和必要的资料关系整理断链报告。",
  ],
  transferMatrix: [
    ["资料不足", "paper_note_needs_revision / mechanism_needs_revision", "01 source_needs_confirmation", "资料寻找 Agent", "transfer_request、missing_source_list"],
  ...
  teamWorkflowOrchestration: {
    ...
    principles: [
      ...
      "CandidateStore 负责物：资料、笔记、机制、假设、审稿记录和资料关系整理。",
    ...
    landedApis: [
      ...
      ["POST", "/api/teams/{team_id}/workflow-orchestration/knowledge-collection/ingest", "批量审查候选、生成资料关系整理和 steward_pack_draft；旧聚合入口只准备治理证据，正式入库以知识库管理员 memory 阶段 writeback 或 steward pack review gate 为准。"],
      ["POST", "/api/teams/{team_id}/workflow-orchestration/steward-packs/{candidate_id}/knowledge-ingestion", "由知识库管理员把有效 steward_pack_draft 提交为 SourceArtifact + pending RefinementProposal，可选 pending ratingSuggestion；正式 KnowledgeItem/RAG/正式图谱只能继续经 review/apply 门禁产生。"],
      ...
      ["GET", "/api/teams/{team_id}/workflow-orchestration/source-collection/summary?runId={runId}", "当前资料搜集批次的唯一闭环投影：scope=source_run，返回四阶段 phaseCloseGate；status=ready 只表示可继续操作，不能替代 gate 的 closed_loop。"],
    ...
    integrations: [
      ...
      ["CandidateStore", "保存候选资料本体和资料关系整理。"],
      ["Team Knowledge", "只接收审核后的正式知识对象。"],
    ],
  },
  roles: [
    ["资料寻找 Agent", "登记资料，不做正式入库。"],
    ["资料提炼 Agent", "筛选 source_manifest 的可靠性、可访问性、相关性和抽取风险，只写 CandidateStore。"],
    ...
    ["Experiment Planner Agent", "读取挑战杯实验上下文，只写实验计划账本，不执行训练。"],
    ["Experiment Ledger Agent", "登记 baseline、smoke、full-run 和入库请求证据，只写 Team workflow ledger。"],
    ["Iteration Planner Agent", "登记 Research Loop 证据与迭代决策，不自动应用改动。"],
    ["Candidate Versioning Agent", "维护候选版本链、supersede/derive/reject 关系和拒绝归档，不写正式图谱。"],
    ["资料入库 Agent / source_ingestor", "审核资料入库步骤；approved 回写触发治理门禁和正式 KnowledgeItem 闭环，不绕过审核。"],
  ...
  featureMatrix: [
    ...
    ["挑战杯实验操作 Agent", "experiment_planner / experiment_ledger 绑定 challenge_cup_experiment_context_tool + challenge_cup_experiment_writeback_tool；只读实验上下文并写实验计划/证据账本，禁止搜索、正式知识写入和代码执行", "已接入 Agent/工具策略"],
    ["挑战杯迭代操作 Agent", "iteration_planner / iteration_versioning 绑定 challenge_cup_iteration_* 与 challenge_cup_versioning_* 工具；只写 Research Loop 和 candidate version ledger，不自动应用迭代决策", "已接入 Agent/工具策略"],
    ["实验计划账本", "experiments/status 与 experiments/plan 聚合实验轮次、算法假设、计划草稿和 baseline/metric/dataset/smoke 缺口；Teams 实验页可生成计划草稿并明确 full-run 与知识入库阻塞", "已接入前端/API"],
    ["Active baseline artifact", "experiments/plans/{planId}/baseline-artifact 登记 baseline 工件路径、复现命令、评估命令和指标快照；登记后 readyForSmoke=true，但 full-run 仍等待 smoke 结果", "已接入前端/API"],
    ["Smoke 结果登记", "experiments/plans/{planId}/smoke-result 登记 smoke 指标、结果路径、日志引用和 gateDecision；Teams 实验账本可登记和回显 active smoke，passed 后 readyForFullRun=true，但仍不自动训练", "已接入前端/API"],
    ["Full-run 结果登记", "experiments/plans/{planId}/full-run-result 登记外部 full-run 指标、结果路径、日志和配置引用；Teams 实验账本只记录手工证据，不触发训练 runner", "已接入前端/API"],
    ["实验结果入库请求", "experiments/plans/{planId}/knowledge-ingestion-request 生成 experiment_result_pack 并通知知识库管理员；正式 Team Knowledge/RAG/official graph 仍由 Steward 复核后写入", "已接入前端/API"],
    ["Research Loop 模板", "research-loop/templates/status/loops/evidence/decision 支持算法模型、仿真模拟、数据集基准、环境探针等模板；Teams 实验页和迭代页共享同一模板切换、证据登记和决策面板，Iteration Planner Agent 通过专用工具写回", "已接入前端/API/Agent"],
    ["评级建议", "knowledge_rating_suggestion_tool 只提交 reviewable rating suggestion", "已有能力"],
    ["资料关系整理预览", "CandidateStore candidate_graph payload、断链报告、候选/正式边界显示；Teams 工作台已接入首版 SVG 读取面，现有 Memory Graph 只承接正式知识结构视图", "已接入首版"],
  ...
  projectAlignment: [
    ["Research Agent 池", "prompt-research-broad/deep/review/themes/card 模板存在；research_service 可确保实例", "可复用+需绑定", "当前 active research pool 是 CEO/组织顾问/能力管家，不是挑战杯执行型功能岗位；需把 资料寻找 Agent、Neuro Mechanism Extraction Agent、Evidence Review Agent 等绑定到模板或新建 Agent。"],
    ...
    ["资料入库 Agent", "source_ingestor 固定存在于知识搜集团队；ToolPolicy 含 knowledge proposal/ingestion/governance/rating 工具；边界为资料入库 Agent 受治理门禁入库", "高度匹配", "07 节点已对齐 Team Knowledge 待审摄取包；只有 source_ingestor 的 ingestion 阶段 approved 回写可以触发后端治理门禁并形成正式入库闭环。"],
    ...
    ["正式 RAG", "knowledge_rag_retrieve_tool 与 /api/knowledge/rag/retrieve 已存在，且只读正式知识", "已有能力", "03-06 可用它查正式背景知识；不能用它读取 pending proposal 或资料关系整理。"],
    ...
    ["记忆知识图谱", "/api/memory/knowledge-graph 和 MemoryRoute 知识图谱视图已存在", "部分匹配", "适合展示正式 Team/Agent/KnowledgeBase/KnowledgeItem 结构；资料关系整理首版在 Teams 工作台读取 CandidateStore 快照，不混入正式 Memory Graph。"],
...
const implementationBlueprint = {
  ...
  activeTeam: {
    ...
    note: "挑战杯科研流程直接绑定当前 Vibelution ai科学研究团队，不另建新团队；Teams 页顶部只保留 AI 搜索范围团队和 ai科学研究团队两个固定入口。选择 ai科学研究团队后直接进入科研控制台，知识搜集阶段统一收敛为资料寻找、资料提炼、资料关系整理、资料入库四张步骤卡；每张卡只绑定一个功能 Agent，组织画布保留为附属结构视图。",
    workspaceEntry: "/teams?team=research-team",
    sourceCollectionEntry: "/teams?team=research-team&researchView=knowledge_collection",
    defaultView: "选中 ai科学研究团队后，团队内容区显示科研控制台：研究主题、当前阶段、知识搜集/实验/迭代三张阶段卡，每卡一个主按钮和一个阶段详情入口。知识搜集独立页只展示资料寻找、资料提炼、资料关系整理、资料入库四张步骤卡，分别绑定 source_finder、source_extractor、source_relation_mapper、source_ingestor；卡片主按钮创建对应 Agent 的 direct session task，带 returnTo/returnLabel 跳转到 Chat 执行，Agent 完成或阻塞后通过 source_collection_stage_writeback_tool 回写团队阶段。资料寻找负责搜索、获取和本地登记；资料提炼负责内容提炼和资料审查；资料关系整理负责候选关系和证据链预览；资料入库负责最终审核并通过治理门禁写入正式 Team Knowledge。旧本地执行按钮、手工 CollectionOutput 和历史私聊入口只保留在详情/调试兜底区，不再作为阶段主路径。",
  ...
  architecture: [
    ...
    ["图谱展示层", "资料关系整理首版由 Teams 工作台读取 CandidateStore candidate_graph payload；正式图谱复用 /api/memory/knowledge-graph。"],
  ],
  milestones: [
    ...
    ["M0.1", "Team 页面科研流程入口", "已在 Teams 工作台为 research-team / 科研组织团队读取 TeamWorkflowOrchestration、最近 CandidateStore 候选和知识入库状态；当前入口统一为知识搜集、实验、迭代三阶段，旧的协调、入库、资料关系整理、候选资料、团队沟通和组织画布不再作为一级索引。", "只读展示，不触发状态转移、审批、正式 Team Knowledge/RAG/图谱写入；普通非科研团队保持原画布优先布局，不被动初始化挑战杯 workflow。"],
    ...
    ["M6.4", "资料搜集批次启动入口", "新增 /api/teams/{teamId}/workflow-orchestration/source-collection-runs，一次创建 DataProcessingRun 和功能 Agent CollectionAssignment。", "Research Coordination Agent 可以启动可追踪资料搜集批次，默认分配 source_finder；功能 Agent 回写 DataRecord 后再导入 source_manifest。"],
    ...
    ["M6.6", "Teams 知识搜集执行台", "research-team 页面将原资料搜集索引折叠进知识搜集阶段页，可启动 source collection run、查看最近批次、run status、assignment、assignedQueries，并手工提交 CollectionOutput。", "启动批次会从 Team canvas 读取 source_finder/source_extractor/source_relation_mapper/source_ingestor 等角色绑定的团队 agentId；手工回写会先写入 DataRecord，再通过 Team workflow 导入桥转成 source_manifest 候选；仍不触发真实外部搜索，不写正式 Team Knowledge/RAG/official graph。"],
    ...
    ["M6.11", "资料质量筛选执行台", "新增 source-quality/status 与 source-quality/assess；Teams 候选资料页可由 资料提炼 Agent 对 source_manifest 标记通过筛选或退回补资料。", "评估结果写入 CandidateStore metadata.sourceQualityAssessment/sourceQualityAssessments，并更新 source_quality_approved/source_quality_needs_revision/source_quality_rejected；不写正式 Team Knowledge/RAG/official graph。"],
    ...
    ["M6.13", "科研三阶段索引统一", "Teams research-team 一级索引统一为知识搜集、实验、迭代三阶段；每个阶段点击进入独立页面并提供返回团队页。旧资料关系整理、候选资料、团队沟通、知识入库和组织画布转为阶段内部能力或附属视图，不再与阶段同级展示。", "知识搜集阶段复用现有 source collection 对话式链路；实验和迭代阶段先提供规划页、启动按钮、轮次状态、模块边界和正式知识/RAG/official graph 写入边界。"],
    ...
    ["M6.17", "知识搜集搜索执行最小闭环", "新增 source-collection-runs/{runId}/search/execute，知识搜集页提供“搜索下一批”按钮；系统可按 assignment/query 调用无 key 多源公开检索，生成执行事件、DataRecord，并自动导入 source_manifest 候选；低相关、域名不匹配或明显噪声结果会在入库前被质量门禁拒绝。", "这是 metadata-only 最小执行器：只下载来源元数据和引用，不抓取全文网页/PDF；结果仍停在候选资料仓库，不写正式 Team Knowledge、不写 RAG、不写 official graph。"],
    ...
    ["M6.23", "控制台状态驱动操作卡", "知识搜集卡接入 source collection run 状态，显示未开始、待继续搜索、搜索中、待筛选、可进入实验、等待回写，并展示当前批次、待处理、候选和查询数。", "主按钮不再叫笼统的继续搜集，而是按状态变为开始知识搜集、搜索下一批、进入资料提炼或进入搜集工作台；旁边保留新一轮搜集和阶段详情，方便在总览操控流程、在详情查看内容。"],
    ...
    ["M6.26", "搜索状态收口", "通用 DataProcessingRun 在所有采集 assignment 关闭且已有记录后进入 reviewing；科研控制台把“搜索中”限定为真实请求执行中，把有待处理 assignment 的状态显示为“待继续搜索”。", "用户能区分后台正在执行、下一批可手动触发、以及已进入资料提炼；这不写正式 Team Knowledge、不写 RAG、不写 official graph。"],
    ["M6.27", "知识搜集控制台客观状态", "知识搜集独立页首屏提升为流程控制台：左上角显示正在团队搜索/待继续搜索/待筛选/失败等运行态，关键按钮直接展示搜索下一批、新一轮搜集、打开结果目录和刷新状态。", "四个步骤按进行中、已完成、失败、未进行、待处理映射为边框/绿色/红色/灰色/提示色；对话流继续承载证据与存放位置，后端搜索、筛选、正式知识/RAG/图谱边界不改变。"],
    ...
    ["M6.29", "资料提炼入口常驻", "知识搜集独立页顶部操作条和右侧下一步面板固定显示资料提炼按钮；有 source_manifest 候选时点击进入候选资料提炼页，没有候选时保持禁用并显示暂无候选。", "资料提炼不再藏在模块数字或候选页深处；真实筛选仍复用 source-quality/assess 的通过筛选/退回补资料链路，不自动写正式 Team Knowledge、RAG 或 official graph。"],
    ["M6.30", "步骤卡主操作入口", "知识搜集独立页移除顶部操作条和右侧下一步面板的重复按钮，把资料寻找、资料提炼、资料关系整理、资料入库四个步骤卡改成主操作入口。", "点击资料提炼不再跳回通用候选页，而是在知识搜集工作台内打开页内筛选详情；每个步骤继续按进行中、已完成、失败、未进行、待处理显示客观状态，正式知识/RAG/official graph 写入边界不变。"],
    ["M6.31", "资料提炼页内反馈修复", "资料提炼步骤卡点击后会展开本页筛选详情、滚动右侧控制台并短暂高亮目标区域；按钮文案从开始资料提炼改为打开资料提炼，避免误解为后台批量筛选。", "本轮只修复前端可见反馈和页内导航，不新增自动筛选执行器；通过筛选、退回补资料和生成分块仍复用已有 source-quality/assess 与 paper-note-chunks/plan 链路。"],
    ["M6.32", "资料提炼 Agent 一键执行", "新增 source-quality/assess-batch：资料提炼步骤卡点击后由资料提炼 Agent 批量处理待筛 source_manifest，复用单条 source-quality/assess 的默认评分与候选写入逻辑。", "批量筛选只写 CandidateStore 的 sourceQualityAssessment/sourceQualityAssessments 和候选状态；仍不写正式 Team Knowledge、不写 RAG、不写 official graph，人工单条通过/退回保留为覆盖修正入口。"],
    ["M6.33", "候选库可视化二级索引", "知识搜集阶段的资料寻找、资料提炼、资料关系整理、资料入库四张步骤卡都可点击进入对应页内详情；资料关系整理主按钮改为查看可视化候选库，不再直接打开 candidate_store/index.json。", "底层 candidate_store/index.json 仍保留在“更多证据文件”中作为维护入口；用户主路径只看到候选卡片、筛选状态、图谱预览和资料入库，不再被 JSON 文件打断。"],
    ["M6.35", "知识搜集阶段子页控制台", "知识搜集阶段的四张步骤卡升级为页内子页索引，点击资料寻找、资料提炼、资料关系整理或资料入库时，底部只显示该阶段内容。", "右侧控制台按当前阶段切换主操作按钮和关键数量；资料寻找保留配置/批次/回写折叠区，其余阶段只显示对应查看、筛选、刷新或前审动作，避免所有阶段信息同屏堆叠。"],
    ["M6.36", "资料提炼 Agent 复审入口", "资料提炼已完成时，步骤卡和右侧控制台主按钮不再只显示查看结果，而是显示 Agent 重新筛选，并通过 source-quality/assess-batch force=true 重新审查已筛候选。", "资料提炼详情页保留查看结果入口，同时新增显眼的 Agent 批量审查按钮；复审仍只写 CandidateStore 的 sourceQualityAssessment，不写正式 Team Knowledge、RAG 或 official graph。"],
    ...
    ["M6.38", "知识搜集运行闭环根治", "data-processing run 列表支持按 profileId、teamId 和 startedFrom 服务端过滤，Teams 知识搜集页不再先取全局最近批次后本地过滤；后台搜索 accepted 后把 activeWorkRun 合成到控制台和对话流，资料搜集卡保持进行中直到 runStatus 收口。", "四个知识搜集子步骤的右侧控制台展示当前步骤相关功能 Agent，并复用 Agent 管理页进行配置；这轮只修复批次可见性、后台状态可见性和 Agent 绑定可操作性，不写正式 Team Knowledge、RAG 或 official graph。"],
    ...
    ["M6.40", "知识搜集一键执行闭环", "stage-rounds/start 创建知识搜集轮次后立即调用既有 source-collection 后台搜索入口，并在响应中返回 sourceCollectionSearchExecution.activeWorkRun；前端把该快照合成到控制台和对话流。", "用户点击知识搜集后不再只看到计划和任务分配，而是能直接看到第一批资料搜索已排队/正在执行；AppShell 顶部运行态优先显示知识搜集，资料搜集步骤只展示 source_finder 等功能 Agent，不再把协调人误作执行 Agent。仍只写 DataRecord/CandidateStore 候选，不写正式 Team Knowledge、RAG 或 official graph。"],
    ["M6.41", "步骤 Agent 右侧配置槽", "知识搜集页移除顶部统一“本阶段 Agent”面板，也不再把 Agent 列表塞进四张步骤卡；资料寻找、资料提炼、资料关系整理、资料入库的功能 Agent 跟随当前步骤显示在右侧详情底部。", "用户操作某个步骤时，只在对应详情里看到该步骤 Agent 的职责、Agent 名称、模型、状态和配置入口；顶部步骤卡只保留状态、数量和主按钮，避免重复信息挤占流程阅读空间。"],
    ["M6.42", "资料搜集过程与结果分层", "知识搜集页的资料搜集子页改为两块：当前搜集动作只显示一条正在进行或最近完成的动作，结果区集中展示本轮 DataRecord 原始记录。", "原始记录与候选资料分账展示：资料搜集子页显示原始记录、已入候选和待导入候选，资料关系整理子页单独展示本轮 source_manifest；顶部阶段卡和总览指标使用中文业务词，避免把 record/query/assignment 等内部计数混在一处。"],
    ["M6.43", "搜集结果状态徽标适配", "资料搜集结果列表中的资料状态独立为横向徽标，避免“需补资料”“已通过”等中文状态在窄列里被挤成单字。", "状态标签保留在结果卡右侧，但按内容撑开并禁止竖排；长状态最多省略，不再影响标题、摘要和来源时间的阅读。"],
    ["M6.44", "搜索运行态与后续任务拆分", "source collection work-run 和 Teams 控制台拆分 searchOpenAssignmentCount、downstreamOpenAssignmentCount；只有资料寻找这类真实搜索任务运行时才显示“正在团队搜索”。", "资料提炼、资料关系整理和资料入库的待处理不再把顶部状态误置为搜索中；控制台展示可搜索任务、后续任务和搜集结果，用户能判断当前是否真的有 Agent 在跑。"],
    ["M6.45", "知识搜集信息架构降噪", "知识搜集页顶部四张步骤卡删除重复 Agent 槽，右侧详情底部集中展示当前步骤 Agent 配置；搜集结果区改名为已收集资料，并把统计拆成已搜到、可点击来源、本地文件和待 Agent 筛选。", "用户不需要理解 DataRecord、query、source_manifest 等内部术语，也能判断哪些是流程步骤、哪些是实际资料结果、当前步骤由哪个 Agent 和模型承接。"],
    ...
    ["M6.47", "资料卡片点击追溯", "资料搜集、资料提炼和资料关系整理列表中的资料卡片可点击选中，右侧详情展示当前资料的 DOI/网页/本地路径、搜索问题、资料记录、批次、分工和搜索源。", "外部来源直接打开 DOI 或网页；本地来源通过既有 source-collection storage 白名单入口打开批次目录、搜索步骤、搜集记录和候选镜像，不允许前端传任意本地路径。"],
    ["M6.48", "资料来源与搜集证据分层", "右侧资料详情把用户可读来源和机器搜索证据分开：主按钮只打开论文 DOI、网页来源或本地文件，OpenAlex/arXiv/GitHub 等公开索引或机器 API 查询放入“查看搜索证据”折叠区。", "用户不会再把公开索引 JSON 当成论文页面；开发者仍可在证据区打开 API 原文、批次目录、搜索步骤、搜集记录和候选镜像追溯 Agent 搜集过程。"],
    ["M6.49", "四阶段完整结果与来源过滤", "资料寻找、资料提炼、资料关系整理、资料入库四个子页统一接入来源类型过滤：全部、PDF、论文网页/DOI、数据集、本地文件、缺少来源。", "资料提炼和资料关系整理不再只显示少量预览；资料关系整理按过滤条件裁剪节点/边并补节点列表；资料入库显示候选资料和前审问题，所有卡片保留 DOI、网页、本地文件或候选详情追溯入口。"],
    ["M6.50", "知识搜集可读操作台根治", "知识搜集独立页改为实体工作台：主题背景不再穿透正文，顶部四阶段卡只承担导航、状态和主按钮，不再展示输入/输出小框。", "当前阶段子页只保留结果工作台；Agent 审查与执行过程通过阶段深链跳转到对应 Agent 私有对话页，右侧控制区固定宽度，只保留下一步操作、当前资料详情和当前步骤 Agent 配置。"],
    ["M6.51", "阶段分页点击稳定", "知识搜集阶段结果面板的数量标识和分页控件只作为状态/翻页入口，不再触发 details summary 折叠或外层布局重排。", "点击 1-8/20、上一页、下一页或阶段标题右侧数量时，页面保持当前阶段布局；只有明确的阶段卡、Agent 私聊深链和分页按钮会改变内容。"],
    ["M6.52", "资料关系整理与前审一键 Agent 节点", "资料关系整理步骤卡从刷新改为 Agent 生成图谱，只使用已通过资料质量筛选的候选；资料入库步骤卡从检查状态改为 Agent 完成前审，自动汇总通过候选和资料关系整理生成 steward_pack_draft 前审包。", "资料关系整理仍停在 CandidateStore candidate-only 层；知识库管理员 memory 阶段 approved writeback 可经治理门禁写入正式 Team Knowledge，不给普通阶段 Agent 正式入库权限。"],
    ["M6.53", "KV 缓存模型与执行 Agent 模型分层", "知识搜集过程流不再写“本轮使用某模型”来描述 prompt-cache policy，而是明确显示 KV 缓存模型；真实执行资料寻找的功能 Agent 和模型仍在右侧当前步骤 Agent 配置中展示。", "当后端因请求模型不支持 prompt cache 而自动兜底到其他 KV 模型时，页面会标注“自动兜底”；用户不会再把 relay/openai/gpt 类缓存模型误认为资料寻找、资料提炼或资料关系整理 Agent 的执行模型。"],
    ["M6.54", "阶段 Agent 与私聊深链契约", "资料关系整理步骤绑定 source_relation_mapper / 资料关系整理 Agent，资料入库步骤绑定 source_ingestor / 资料入库 Agent；阶段页不再内嵌 Agent 过程列表。", "Teams 知识搜集页为每个阶段提供对应 Agent 私聊深链；用户点击进入私有对话页审查真实 Agent 执行过程。"],
    ["M6.55", "知识搜集四步闭环入库", "知识搜集阶段收敛为资料寻找、资料提炼、资料关系整理、资料入库四张用户步骤卡。", "第一阶段可从资料寻找推进到正式入库闭环：资料寻找写 DataRecord/source_manifest，资料提炼完成内容提炼和资料审查，资料关系整理生成候选关系，资料入库由 source_ingestor approved 回写后经 Team Knowledge 治理门禁创建 KnowledgeItem。"],
    ["M6.57", "资料提炼 Agent 一键阶段", "新增 /api/teams/{team_id}/workflow-orchestration/knowledge-collection/extract，由 source_extractor / 资料提炼 Agent 批量读取本轮 DataRecord，并通过既有导入桥幂等转为 source_manifest 候选。", "资料提炼步骤卡不再只是查看结果；用户可点击 Agent 提炼资料或 Agent 重新提炼，页面显示提炼进行中、失败和完成状态，并刷新本轮候选库、Assignment、候选审查状态和证据落盘位置；仍不触发外部搜索、不写正式 Team Knowledge/RAG/official graph。"],
    ["M6.58", "资料入库入口贯通候选资料", "资料入库步骤不再只看已通过资料数量；只要本轮已有候选资料，就允许进入资料入库阶段任务。", "前端把候选资料数作为待入库包输入上限；后端会先补齐资料提炼、候选关系和 steward_pack_draft 证据，再由 source_ingestor approved writeback 触发治理门禁，避免用户被卡在必须手动跑完每个中间门禁，同时不绕过最终入库审核。"],
    ["M6.59", "资料提炼结果滚动与审查交接", "资料寻找结果区和资料提炼候选区都接入独立纵向滚动容器；资料提炼统计和候选状态统一显示为已审查、需补资料或待 Agent 审查。", "Agent 重新提炼会同步本轮原始资料为候选资料，并由 source_extractor 在同一阶段给出保留、补充或无效结论。"],
    ...
    ["M6.62", "阶段状态同步根因修复", "删除 team_workflow_orchestration_service 中重复定义的 source collection 阶段同步函数，避免旧实现覆盖 needs_continue / needs_screening 终态判断。", "get_research_stage_round_status 会在读取时用最新 source_collection_run work-run 终态自恢复旧 running 轮次；已有页面刷新即可退出假运行中，测试锁定同步函数只能有一个实现。"],
    ["M6.63", "挑战杯 Agent 提示词按能力收敛", "Agent Directory 为 challenge_cup_coordinator、source_finder、source_extractor、source_relation_mapper 和 source_ingestor 绑定专属 prompt template 与 persona/task profile。", "提示词明确各角色现有工具边界：协调人只读协调，资料寻找负责搜索获取和登记，资料提炼负责内容提炼和价值判断，资料关系整理负责候选关系预览，资料入库负责最终审核和治理门禁；均不声称拥有 Shell、Git 或越权写入权限。"],
    ["M6.64", "四阶段 Agent 私聊入口与来源元数据标准化", "知识搜集四张步骤卡新增 Agent 私聊入口，资料关系整理恢复为独立步骤，前审步骤显示为资料入库；DataRecord 导入 source_manifest 时统一写入 sourceRunId、sourceRecordId、sourceCategory、doi、sourceUrl、sourcePath、query、assignmentId 和 searchProvider。", "用户可从每个阶段直接进入对应 Agent 私有对话页审查执行过程；四个子页可按 PDF、论文网页/DOI、数据集、本地文件和缺少来源过滤，并能从资料卡追溯 DOI、网页、本地路径或搜索证据。"],
    ["M6.65", "科研团队 Agent 结构性重绑", "research-team 增加标准 4 个知识搜集角色修复入口：资料寻找、资料提炼、资料关系整理、资料入库；旧成员、旧画布节点和重复 challengeCupTeamId Agent 可被物理清理。", "阶段 Agent 必须拥有 directSessionId 且在会话页可见；每张步骤卡只绑定一个新角色，阶段私聊入口只跳转到对应 Agent 私有对话页。"],
    ["M6.66", "阶段卡 Agent 会话任务桥", "知识搜集四张步骤卡的主按钮不再直接执行本地阶段动作，而是创建对应功能 Agent 的 direct session task 并跳转到会话页执行。", "任务落盘到 source_collection_runs/<runId>/stage_session_tasks.json；Chat URL 保留 returnTo/returnLabel；Agent 先调用 task_list_tool / task_create_tool 绑定阶段 checklist，再用 source_collection_context_tool 读取受控资料上下文，并用 source_collection_stage_writeback_tool 结构化回写 sourceCollectionStageSessionTasks；团队阶段再展示结果；不通过 file:// 或 localhost fetch 猜本地资料。"],
    ["M6.67", "阶段卡重复点击新任务", "知识搜集四张步骤卡每次主按钮点击都创建新的 Agent direct session task，并把新的任务消息投递到对应私聊会话。", "同一轮搜索、提炼、审查或入库的结果继续叠加到候选资料仓库，依赖既有 DOI/URL/标题指纹去重；stage-session-tasks 只在调用方显式传入同一 idempotencyKey 时复用同一次请求，避免后续点击被旧任务拦截。"],
    ["M6.68", "阶段任务受控上下文与工具回写", "新增 source_collection_context_tool 和 source_collection_stage_writeback_tool，四个阶段 Agent 可在私聊任务里读取本轮受控上下文并回写完成、阻塞或失败。", "阶段任务消息不再把本地路径或 localhost API 当作主要执行入口；Agent 工具策略允许新四角色调用各自需要的上下文/回写工具，避免任务长期停留 running。"],
    ["M6.69", "资料提炼 Evidence Ledger", "source_collection_stage_writeback_tool 回写 candidateExtractions[] / recordExtractions[] 时会把结构化提炼物化为 metadata.contentExtraction.evidenceLedger，保留 claims、keyFindings、citations、sourceRefs、evidenceRefs、limitations、uncertainty、riskFlags、supportLevel 和 nextAction。", "候选 writeback 会统计 evidenceLedgerCandidateCount、evidenceReadyCandidateCount 和 missingEvidenceAnchorCount；Teams 资料卡、资料提炼详情和资料关系整理节点会显示 evidence_ready / missing_evidence_anchor，缺锚点结构化证据降级为 needs_review，避免无来源结论进入 paper_note、机制提取或资料关系整理。"],
    ["M6.70", "资料提炼证据上下文与定向补证", "source_extractor 首轮使用 source_collection_context_tool 的 evidence 模式，分页读取真实 candidateId、搜集阶段保存的受控摘要和可复制 evidenceRefs；覆盖缺口使用 retry_missing，覆盖完整但存在 blocked 或 missing_evidence_anchor 时使用 retry_evidence。", "retry_evidence 只返回 evidenceGapCandidateIds，避免重新处理 evidence_ready 候选；摘要或元数据只能支撑其明确范围，不能据此虚构全文、页码或直接引语，跨站 web_fetch 安全边界保持不变。"],
    ...
    ["M7.3", "Full-run 与知识入库请求", "新增 Teams 实验账本 full-run 结果登记和 knowledge-ingestion-request 前端入口，复用已有 full-run-result 与 知识库管理员 通知 API。", "smoke passed 后可登记外部 full-run 证据；full-run passed 后可生成 experiment_result_pack 并通知知识库管理员；正式知识/RAG/official graph 仍等待 Steward 审批。"],
    ["M8", "Research Loop 模板化实验迭代", "新增 Teams Research Loop 面板：读取 research-loop/templates/status，创建 loop，登记 evidence，提交 decision，并在迭代页复用同一模板切换面板。", "实验与迭代合并为模板化模块，可按算法模型、仿真模拟、数据集基准、环境探针等研究内容动态切换；不接沙盒 runner、不自动训练。"],
    ["M8.1", "实验与迭代操作型 Agent", "新增 experiment_planner、experiment_ledger、iteration_planner、iteration_versioning 四个挑战杯功能 Agent，并绑定 challenge_cup_experiment_*、challenge_cup_iteration_*、challenge_cup_versioning_* 专用工具。", "四个 Agent 默认无网络、无代码执行、无正式知识写入；只写 team_workflow_ledger、Research Loop 和 candidate version ledger，提示词与 ToolPolicy 同步收敛。"],
  ],
  schemas: [
    "DataProcessingRun",
    "DataRecord",
    ...
    "smoke_result",
    "full_run_result",
    "experiment_result_pack",
    "research_loop",
    "candidate_versioning_ledger",
    "review_record",
    "candidate_graph",
    ...
    "content_extraction_evidence_ledger",
    "source_collection_prompt_cache_policy",
    "source_collection_stage_session_task",
  ...
  services: [
    ...
    ["team_workflow_orchestration_service", "已落地：Team 级 workflowOrchestration、ResearchStageRound、CandidateStore、transfer request/decision、source-collection run 启动、阶段 Agent direct session task 启动、阶段任务重复点击新建、结构化 writeback 回团队阶段、DataSearchPlan/query seed 契约、prompt cache/KV 启动门禁、DataRecord -> source_manifest 幂等导入桥、source_quality assessment、contentExtraction Evidence Ledger、paper_note chunk planning、official_model_evidence 证据登记/status，以及 experiment_plans 实验计划账本、active baseline artifact、smoke result、full-run result、experiment result pack 和 Research Loop 手工证据/决策账本。"],
    ["challenge_cup_operations_tools", "已落地：challenge_cup_experiment_context/writeback、challenge_cup_iteration_context/writeback、challenge_cup_versioning_context/writeback 六个专用工具；实验和迭代 Agent 通过工具读取受控上下文并结构化回写，显式拒绝 run_smoke、run_training、full_run 等执行型操作。"],
    ["challenge_cup_versioning_service", "已落地：candidate_versions/index.json 轻量版本账本，支持 record_version、supersede、derive、reject，输出 versionHistory、relations、rejectionArchive 和正式知识边界。"],
    ["team_workflows API", "已落地：/api/teams/{team_id}/workflow-orchestration、stage-rounds/status、stage-rounds/start、stage-rounds/{stageRoundId}/coordination/retry、stage-rounds/{stageRoundId}/memory-record/retry、source-collection-runs（含 searchPlan/querySeeds/assignedQueries/resultWritebackContract）、source-collection-runs/{runId}/stage-session-tasks、stage-session-tasks/{taskId}/writeback、candidates/source、data-processing/runs/{runId}/records/{recordId}/source-candidate、candidates/{candidate_id}/source-extraction、source-quality/status、candidates/{candidate_id}/source-quality/assess、paper-note-chunks/status、candidates/{candidate_id}/paper-note-chunks/plan、candidates/{candidate_id}/paper-note-draft（支持 chunkId）、candidates、candidates/validation、candidate-graph、transfers、decide、knowledge-ingestion/status、coordination/status、official-model-evidence/status、official-model-evidence、experiments/status、experiments/plan、experiments/plans/{planId}/baseline-artifact、experiments/plans/{planId}/smoke-result、experiments/plans/{planId}/full-run-result、experiments/plans/{planId}/knowledge-ingestion-request、research-loop/templates、research-loop/status、research-loop/loops、research-loop/loops/{loopId}/evidence、research-loop/loops/{loopId}/decision；source_collection_stage_writeback_tool 可回写 contentExtraction Evidence Ledger；coordination/status 返回 communicationBrief。"],
    ["TeamsRoute workflow workspace", "已落地入口：Teams 页顶部固定横向切换条只暴露 AI 搜索范围团队和 ai科学研究团队；选择 ai科学研究团队后直接进入 research-team 专属科研控制台。知识搜集页位于 /teams?team=research-team&researchView=knowledge_collection，只展示资料寻找、资料提炼、资料关系整理、资料入库四张步骤卡；每张卡只绑定一个功能 Agent，主按钮创建 direct session task 并跳转 Chat，执行结果以 writeback 合同回写 sourceCollectionStageSessionTasks 后挂回 ResearchStageRound；资料卡、资料提炼详情和资料关系整理节点已显示 Evidence Ledger 状态。"],
    ...
    ["memory_ingestion_bridge", "已复用 Team Knowledge source review、create_ingestion_package、review_refinement_proposal、rating suggestion review/create 和 KnowledgeItem metadata patch；知识库管理员 memory 阶段 approved writeback 会由后端创建或复用 steward_pack_draft，串起 pending proposal 与 review/apply gate，审批通过后创建正式 KnowledgeItem、迁移待审评分建议，并写入 officialResearchGraph 正式科研边。"],
  ...
  localModel: {
    ...
    nodeAssignments: [
      ...
      ["07 知识治理入库", "本地模型只生成 steward_pack_draft 草稿；知识库管理员 memory 阶段 approved writeback 才可触发后端治理门禁，不得绕过 gate 请求 officialSync/applyNow/writeOfficialGraph。"],
    ],
    hardBoundaries: [
      "不得直接写正式 Team Knowledge、正式 RAG 或正式知识图谱。",
      "不得替代 Evidence Review Agent、知识库管理员或 Ingestion Approval Gate 的最终裁决。",
  ...
  deliveredSlice: [
    ...
    ["资料搜集批次启动", "/api/teams/{teamId}/workflow-orchestration/source-collection-runs", "Research Coordination Agent 可一次创建 DataProcessingRun、searchPlan 和 source_finder 等功能 Agent assignments。"],
    ...
    ["实验计划状态", "/api/teams/{teamId}/workflow-orchestration/experiments/status", "只读聚合实验轮次、algorithm_hypothesis 候选、计划草稿、readiness、gaps 和执行边界；不创建实验结果。"],
    ["实验计划草稿", "/api/teams/{teamId}/workflow-orchestration/experiments/plan", "基于可用 algorithm_hypothesis 与实验阶段轮次生成 experiment_plan 草稿，挂回 ResearchStageRound；不触发训练 runner。"],
    ["Active baseline artifact 登记", "/api/teams/{teamId}/workflow-orchestration/experiments/plans/{planId}/baseline-artifact", "登记 baseline 工件路径、复现命令、评估命令和指标快照；只解除 smoke 前置 blocker，不创建 ExperimentResult。"],
    ["Smoke 结果登记", "/api/teams/{teamId}/workflow-orchestration/experiments/plans/{planId}/smoke-result", "登记 smoke 指标、结果路径、日志引用和 gateDecision；passed 后只开放 full-run 证据登记，不触发训练 runner。"],
    ["Full-run 结果登记", "/api/teams/{teamId}/workflow-orchestration/experiments/plans/{planId}/full-run-result", "登记外部 full-run 指标、结果路径、日志和配置引用；passed 后开放 知识库管理员 入库请求。"],
    ["实验结果入库请求", "/api/teams/{teamId}/workflow-orchestration/experiments/plans/{planId}/knowledge-ingestion-request", "生成 experiment_result_pack 并通知知识库管理员；正式知识、RAG 和 official graph 仍由 Steward 审批。"],
    ["Research Loop 模板", "/api/teams/{teamId}/workflow-orchestration/research-loop/templates", "列出算法模型、仿真模拟、数据集基准、环境探针、资料审查等动态模板及必需证据类型。"],
    ["Research Loop 状态", "/api/teams/{teamId}/workflow-orchestration/research-loop/status", "只读聚合 active loop、证据缺口、决策状态和执行边界。"],
    ["Research Loop 证据/决策", "/api/teams/{teamId}/workflow-orchestration/research-loop/loops + /evidence + /decision", "创建 loop、登记人工证据和提交迭代决策；commandPreview 仅作计划引用，不自动执行。"],
    ...
    ["资料质量筛选写回", "/api/teams/{teamId}/workflow-orchestration/candidates/{candidateId}/source-quality/assess", "资料提炼 Agent 对 source_manifest 写入 metadata.sourceQualityAssessment，并更新 source_quality_approved/source_quality_needs_revision/source_quality_rejected；不写正式知识/RAG/图谱。"],
    ["资料提炼 Evidence Ledger", "source_collection_stage_writeback_tool", "资料提炼 Agent 回写 candidateExtractions[] 或 recordExtractions[] 时会写入 metadata.contentExtraction.evidenceLedger；结构化提炼缺 sourceRef/page/citation/evidenceRef 锚点时标记 missing_evidence_anchor 并进入 needs_review。"],
    ...
    ["服务文件", "core/web/services/team_workflow_orchestration_service.py", "Team 编排、资料搜集批次启动、候选登记、DataRecord 批量/单条导入、资料质量筛选、contentExtraction Evidence Ledger、paper_note 分块计划、模型证据登记、转移请求、协调 Agent 裁决和轻量运行事件日志。"],
    ["路由文件", "core/web/routes/team_workflows.py", "提供工作流查询/确保、资料搜集批次启动、候选资料登记、DataRecord 单条导入、资料提炼批量执行、资料质量筛选状态/写回、paper_note 分块状态/计划、模型证据状态/登记、转移提交、转移裁决 API；Evidence Ledger 经阶段任务回写工具进入 CandidateStore。"],
    ...
    ["阶段 Agent 会话任务索引", "workspace/teams/<teamId>/source_collection_runs/<runId>/stage_session_tasks.json", "保存四张知识搜集步骤卡启动的 direct session task、writebackContract、Chat turn 摘要和结构化回写结果；任务引用同步挂回 ResearchStageRound.sourceCollectionStageSessionTasks。"],
    ["实验计划索引", "workspace/teams/<teamId>/experiment_plans/index.json", "保存实验计划草稿、selectedHypotheses、baselineSelection、activeBaselineArtifact、smoke/full-run 结果、experimentResultPack、readinessChecklist 和 knowledge-ingestion blocker。"],
    ...
    ["第一阶段闭环门", "/api/teams/{teamId}/workflow-orchestration/source-collection/summary?runId={runId}", "以当前 source_run 的四张阶段卡生成 phaseCloseGate；只有 finding/extraction/relations/ingestion 全部 closed_loop 且阶段轮次已收口时才通过。knowledge-ingestion、source-quality、coordination 和 model-evidence 的全局状态均标记 team_aggregate，不可替代本轮 gate。"],
    ...
    ["阶段 Agent 会话任务启动", "/api/teams/{teamId}/workflow-orchestration/source-collection-runs/{runId}/stage-session-tasks", "根据 stageId/agentId/agentRole 创建 source_collection_stage_session_task，把任务消息投递到 Agent directSessionId，并返回带 returnTo/returnLabel 的 chatRoute；任务消息要求先调用 source_collection_context_tool 读取受控上下文，再调用 source_collection_stage_writeback_tool 回写结果；每次用户点击默认新建任务，只有显式相同 idempotencyKey 才复用同一次请求；不写正式 Team Knowledge/RAG/official graph。"],
    ["阶段 Agent 会话任务回写", "source_collection_stage_writeback_tool / /api/teams/{teamId}/workflow-orchestration/stage-session-tasks/{taskId}/writeback", "只接受结构化 status/summary/result/evidenceRefs/nextActions，把结果写入 stage_session_tasks.json 并同步 ResearchStageRound.sourceCollectionStageSessionTasks；自然语言会话不是团队阶段结果来源；blocked/failed 也必须回写，避免任务长期 running。"],
    ["阶段 Agent 上下文读取", "source_collection_context_tool", "只读返回指定 team/run/task 的 bounded DataRecord、source_manifest 候选、assignment、writebackContract 和边界摘要；不读取任意本地路径、不消耗 API 额度、不要求 Agent 用 web_fetch_tool 访问 file:// 或 localhost。"],
    ["知识搜集阶段页", "TeamsRoute knowledge collection stage", "可在 /teams?team=research-team&researchView=knowledge_collection 独立打开知识搜集页，查看 source collection run、DataSearchPlan/querySeeds/assignment/assignedQueries 和阶段 Agent 会话任务；四张步骤卡主按钮启动对应 Agent direct session task 并跳转 Chat，旧 Agent 私聊/手工 CollectionOutput 作为详情与兜底入口保留；页面会把 KV 门禁状态、promptCacheMode、稳定前缀、promptCachePartition 和 source_collection_runs/<runId> 证据落盘位置放入搜集对话流，并可通过 storage/open 白名单接口打开对应位置。"],
    ...
    ["steward_pack 待审入库", "/api/teams/{teamId}/workflow-orchestration/steward-packs/{candidateId}/knowledge-ingestion", "有效 steward_pack_draft 可由知识库管理员提交到指定 knowledgeBaseId，创建 SourceArtifact、pending RefinementProposal 和可选 pending ratingSuggestion；正式 KnowledgeItem 继续由 review/apply 门禁创建。"],
    ...
    ["验证", "tests/test_data_processing_service.py + tests/test_data_processing_routes.py + tests/test_team_workflow_orchestration_service.py + tests/test_team_workflow_routes.py + TeamsRoute.layout.test.ts", "覆盖通用数据处理首切、ResearchStageRound 三阶段启动/续做/新一轮、source-collection run 启动、DataSearchPlan/query seed 契约、DataRecord 导入 source_manifest、Teams 知识搜集独立页/对话流契约、主路径、非 ownerAgent 不能写最终状态、本地模型任务包、输出校验、知识入库状态总览、模型证据链、资料质量筛选、paper_note 分块计划和实验计划账本前端契约。"],
  ],
};
...
const currentResearchRun = {
  ...
  status: "候选搜集与首轮提炼已完成，等待审查/治理",
  updatedAt: "2026-06-08 09:30 CST",
  team: "ai科学研究团队 / research-team",
  flowState: [
    ["DataProcessingRun", "4 个 assignment 完成", "source_finder、source_extractor、source_relation_mapper、source_ingestor 均已提交输出。"],
    ["DataRecord", "10 条 collected", "全部带 sourceRef、summary、qualitySignals、collectionTrace。"],
    ["source_manifest", "10 个 source_registered / source_quality_*", "全部为 candidate-only；可由 资料提炼 Agent 写回 sourceQualityAssessment。"],
    ["contentExtraction.evidenceLedger", "后端合约已落地", "新一轮资料提炼回写可沉淀 evidence_ready / missing_evidence_anchor 证据账本。"],
...
const knowledgeNodeRunbook = {
  "01": {
    ...
    agent: "资料寻找 Agent",
    agentStatus: "四阶段知识搜集主线已收敛：资料寻找、资料提炼、资料关系整理、资料入库各绑定一个功能 Agent；阶段卡主按钮创建 Agent 私聊任务，结构化 writeback 回团队阶段，结果在同一轮候选资料仓库中去重叠加。",
    features: ["ResearchStageRound 建立", "科研控制台一键进入知识搜集", "资料寻找/资料提炼/资料关系整理/资料入库四步控制台", "阶段 Agent 会话任务启动", "受控上下文工具", "结构化 writeback 回团队阶段", "Chat 返回链接", "右侧当前步骤 Agent 配置", "阶段结果工作台", "来源类型过滤", "DataProcessingRun 建立", "DataSearchPlan/querySeeds 搜索计划", "CollectionAssignment 搜索资料任务", "DataRecord 自动导入 source_manifest", "sourceQualityAssessment 审查", "contentExtraction.evidenceLedger", "资料入库 Agent 治理门禁"],
  ...
  "07": {
    ...
    agent: "资料入库 Agent / source_ingestor",
    ...
    localModelUse: "本地 9B 模型只能生成 steward_pack_draft 草稿；正式入库只能由 source_ingestor ingestion 阶段 approved writeback 触发后端治理门禁，禁止绕过 gate 请求正式写入。",
    humanGate: "必须经 source_ingestor / Ingestion Approval Gate 确认，不能由资料寻找、资料提炼、资料关系整理或本地模型直接入库。",
    ...
    entry: "review_record 或资料关系整理已给出可追溯 candidateIds，且具备目标知识域。",
    operation: "生成带 sourceTrace、riskSummary、proposalPayload、ratingSuggestion、approvalRequired=true 的 CandidateStore 草稿包；source_ingestor ingestion 阶段回写 approved 候选后，后端提交到指定 knowledgeBaseId 的待审入库队列并执行治理门禁；status API 汇总 pendingProposalCount、formalKnowledgeItemCount、actionItems 和 officialBoundary。",
    exit: "approved 回写经门禁后进入 official_synced、记录 officialSyncRecord 和 officialResearchGraph，并创建 KnowledgeItem 级 pending ratingSuggestion；重复回写复用已有 official sync；status=ready 表示入库链路已跑通。",
  ...
  "08": {
    ...
    agentStatus: "后端/API 已可生成资料关系整理快照；Teams 工作台绑定独立 candidate_graph Agent，并把 agentProcess 显示到当前步骤过程页。",
    features: ["candidate_graph 候选快照生成", "候选节点/边预览 payload", "断链报告", "未审节点清单", "候选/正式边界标识", "Teams SVG 资料关系整理面板", "candidate_graph Agent 过程追踪"],
    tools: ["/api/teams/{teamId}/workflow-orchestration/candidate-graph", "CandidateStore", "TeamsRoute 资料关系整理渲染"],
    humanGate: "确认资料关系整理只作预览，不作为正式知识证据使用。",
    ...
    operation: "由 candidate_graph Agent 从已通过筛选的 source_manifest 与后续候选链路重建候选关系；Teams 面板可请求最新 candidate_graph，渲染 evidence -> candidate 的阅读向箭头，并在过程页展示输入、输出和下一步。",
    ...
    <main class="shell">
      ...
      <section id="overview" class="review-dashboard">
        ...
        <aside class="dashboard-panel">
          <h2>审核摘要</h2>
          <div class="kpi-grid">
            <div class="kpi"><b>当前阶段</b><strong>M6.69</strong><span>资料提炼回写会沉淀 Evidence Ledger，并用锚点状态拦住无来源结论。</span></div>
        ...
        <div class="workbench-grid">
          ...
          <aside class="flow-review-panel">
            ...
            <div class="review-stack">
              ...
              <div><b>06-09</b><span>审稿、知识治理、资料关系整理、正式同步边界。</span></div>
        ...
        <div class="plan-library-intro">
          ...
          <div class="dashboard-panel">
            ...
            <div class="focus-list">
              ...
              <div class="focus-item"><b>治理</b><span>只有知识库管理员 approved 回写可触发正式入库门禁。</span></div>
