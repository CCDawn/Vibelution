...
const nodes = [
  ...
  },
  {
    ...
    graph: "审批通过后正式 KnowledgeItem metadata 记录 officialResearchGraph；Memory Graph 通过 include=officialResearchGraph 显式展开正式科研 trace，RAG 可从正式 KnowledgeItem 继续读取。",
    risks: ["绕过审批门禁", "目标知识域错误", "把 ingestion pack 草稿误当正式入库记录"],
    openQuestions: ["是否为 officialResearchGraph 增加专门筛选/图例，而不是仅用 Memory Graph 默认节点样式。"],
  },
  {
    id: "08",
  ...
  },
  {
    ...
    openQuestions: ["是否在 Memory Graph 详情栏增加 officialResearchGraph 专用摘要。"],
  },
  {
    id: "10",
...
const knowledgeNodeRunbook = {
  ...
  "07": {
    ...
    gap: "pending ratingSuggestion 到正式 KnowledgeItem rating review 的承接已接入；正式 supports/maps_to/inspires/approved_for_ingestion 边已写入 officialResearchGraph，Memory Graph 显式展开已接入。后续可补图例和筛选。",
  ...
  "09": {
    ...
    gap: "ratingSuggestion 自动迁移到 KnowledgeItem 待审评级已接入；正式 supports/maps_to/inspires/approved_for_ingestion 边已接入 officialResearchGraph，并可在 Memory Graph 画布展开。后续可补专用摘要与筛选。",
