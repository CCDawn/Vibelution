...
const nodes = [
  {
    ...
    actions: [
      "Data Intake Coordinator Agent 定义搜集范围、批次验收标准、去重规则和返工路径。",
      "Data Discovery Agent 从搜索工具、工作区文件、历史记忆和已配置目录发现候选资料。",
      "Source Acquisition Agent 只登记或获取原始 sourceRef/rawLocation，不改写内容。",
      "Content Extraction Agent 对文件/PDF/网页生成 checksum、anchors、excerpt 和 metadata。",
      "Source Deduplication Agent 合并重复 URL、文件 hash、标题或近似片段，只保留 trace alias。",
      "Source Quality Agent 标记 relevance、credibility、freshness、permission 和风险信号。",
    ...
    risks: ["资料来源不明", "PDF 抽取失败", "Agent 把发现结果误标为可信来源", "联网搜索结果混入未审核资料"],
    openQuestions: ["长 PDF 是否需要自动分批抽取与章节级拆分规则。", "通用 data-processing 的 CollectionAssignment 何时替代当前 source_manifest 专用登记。"],
  },
  {
    id: "02",
    slug: "paper-note",
...
const implementationBlueprint = {
  ...
  milestones: [
    ...
    ["M6.2", "通用数据处理底座", "计划新增 domain-neutral data-processing substrate，把 intake、extract、transform、validate、review、publish 做成通用 pipeline/profile/adapter，并把数据搜集作为 Agent assignment 流程；挑战杯科研只是其中一个 profile。", "通用层不写 challenge cup、神经学、steward 或 qwen 专名；数据搜集由 Data Intake Coordinator / Discovery / Acquisition / Extraction / Deduplication / Quality / Review 等功能 Agent 协作，正式 Team Knowledge/RAG/图谱写入通过 publish adapter 和审批门禁完成。"],
  ],
  schemas: [
    "source_manifest",
    "paper_note",
  ...
  services: [
    ...
    ["data_processing_substrate", "待实施：通用数据处理底座，提供 DataPipelineProfile、DataProcessingRun、DataRecord、DataArtifact、CollectionAssignment、SourceQualitySignal、ValidationReport、ReviewDecision、PublishTarget 和 ProcessingTrace；挑战杯通过 profile/adapter 复用。"],
  ],
  communicationRoles: [
  ...
  deliveredSlice: [
    ...
    ["通用数据处理底座计划", "docs/plans/2026-06-07-general-data-processing-substrate.md", "下一步先实现通用 data-processing API/存储/状态机/CollectionAssignment/适配器/UI，再把挑战杯科研流程作为 challenge_cup_research_ingestion profile 接入；不把数据搜集能力写死到科研或神经学场景。"],
...
const knowledgeNodeRunbook = {
  "01": {
    ...
    agentStatus: "现有 source-extraction 后端已接入；通用 CollectionAssignment Agent 分工待实现",
    features: ["Data Intake Coordinator 范围定义", "Data Discovery 候选发现", "Source Acquisition 原始 sourceRef/rawLocation 登记", "Content Extraction checksum/anchors/excerpt 抽取", "Source Deduplication 去重别名", "Source Quality 可信度/权限/风险信号", "source_manifest 写入"],
    tools: ["web_search_tool（待按 Agent ToolPolicy 授权）", "research_knowledge_query_tool", "/api/teams/{teamId}/workflow-orchestration/candidates/{candidateId}/source-extraction", "/api/data-processing/runs/{runId}/collection-assignments（待实施）"],
    ...
    operation: "由功能 Agent 协作完成发现、获取、抽取、去重、质量评分和 intake review；当前已能建立 source_manifest 记录，并对本地 PDF 运行 source-extraction，计算 sha256、生成 pageAnchors/excerpt，不改写原文，不自动混入联网资料。",
