...
const nodes = [
  ...
  },
  {
    ...
    outputs: ["steward_pack_draft 候选", "SourceArtifact", "pending RefinementProposal", "proposal ratingSuggestion", "KnowledgeItem ratingSuggestion", "officialResearchGraph", "official_sync_record", "knowledge_ingestion_status"],
...
const knowledgeRunbook = {
  ...
  featureMatrix: [
    ...
    ["入库状态总览", "knowledge-ingestion/status 聚合 CandidateStore、校验报告、候选图摘要、pending proposals、formal KnowledgeItem 和 officialBoundary", "已接入后端/API"],
...
const implementationBlueprint = {
  ...
  milestones: [
    ...
    ["M6", "知识入库状态总览", "新增 knowledge-ingestion/status 只读聚合 API，把 CandidateStore、候选校验、候选图摘要、Team Knowledge stats 和 officialBoundary 汇成 stages/actionItems。", "团队协调员可看到 source_collection、candidate_screening、steward_pack、knowledge_review、official_sync 的 ready/needs_review/blocked 状态；查询不会创建 KnowledgeItem、不会写 RAG、不会生成 candidate_graph 快照。"],
  ],
  schemas: [
    "source_manifest",
    "paper_note",
  ...
  services: [
    ...
    ["team_workflows API", "已落地：/api/teams/{team_id}/workflow-orchestration、candidates/source、candidates/{candidate_id}/source-extraction、candidates/{candidate_id}/paper-note-draft、candidates、candidates/validation、candidate-graph、transfers、decide、knowledge-ingestion/status。"],
  ...
  deliveredSlice: [
    ...
    ["知识入库状态总览", "/api/teams/{teamId}/workflow-orchestration/knowledge-ingestion/status", "只读聚合 source_collection、candidate_screening、steward_pack、knowledge_review、official_sync 状态；返回 actionItems 与 officialBoundary，不写正式知识、不写 RAG、不生成候选图快照。"],
...
const knowledgeNodeRunbook = {
  ...
  "07": {
    ...
    gap: "pending ratingSuggestion 到正式 KnowledgeItem rating review 的承接已接入；正式边已写入 officialResearchGraph；status API 已能显示 pending/ready/actionItems。后续可补 Teams 前端状态漏斗。",
  ...
  "09": {
    ...
    gap: "ratingSuggestion 自动迁移到 KnowledgeItem 待审评级已接入；正式边已接入 officialResearchGraph；status API 已区分 candidate_graph_preview_only 与 official_research_trace_synced。后续可补前端摘要与筛选。",
