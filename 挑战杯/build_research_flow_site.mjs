...
const implementationBlueprint = {
  ...
  activeTeam: {
    ...
    defaultView: "选中 ai科学研究团队后，团队内容区显示科研控制台：研究主题、当前阶段、知识搜集/实验/迭代三张阶段卡、每卡一个主按钮和一个阶段详情入口；知识搜集卡会按未开始、待继续搜索、搜索中、待筛选、可进入实验、等待回写显示状态，并提供“新一轮搜集”；知识搜集独立页的资料搜集、资料筛选、候选入库、候选图谱、共享记忆前审五张步骤卡各自承载开始/查看/刷新按钮，资料筛选留在页内详情区，不再跳回通用候选页；模型证据、候选、路径、校验和内部状态默认移到阶段详情页或高级信息中。",
    ...
    <main class="shell">
      ...
      <section id="overview" class="review-dashboard">
        ...
        <aside class="dashboard-panel">
          <h2>审核摘要</h2>
          <div class="kpi-grid">
            <div class="kpi"><b>当前阶段</b><strong>M6.30</strong><span>知识搜集页五张步骤卡承载主操作；资料筛选在页内详情完成，不再跳回通用候选页。</span></div>
