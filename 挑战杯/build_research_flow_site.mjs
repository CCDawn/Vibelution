...
const implementationBlueprint = {
  ...
  services: [
    ...
    ["team_workflow_orchestration_service", "已落地：Team 级 workflowOrchestration、CandidateStore、transfer request/decision、source-collection run 启动，以及 DataRecord -> source_manifest 幂等导入桥。"],
    ["team_workflows API", "已落地：/api/teams/{team_id}/workflow-orchestration、source-collection-runs、candidates/source、data-processing/runs/{runId}/records/{recordId}/source-candidate、candidates/{candidate_id}/source-extraction、candidates/{candidate_id}/paper-note-draft、candidates、candidates/validation、candidate-graph、transfers、decide、knowledge-ingestion/status、coordination/status；coordination/status 返回 communicationBrief 但不发送消息。"],
  ...
  deliveredSlice: [
    ...
    ["资料搜集批次启动", "/api/teams/{teamId}/workflow-orchestration/source-collection-runs", "Research Coordination Agent 可一次创建 DataProcessingRun 和 data_discovery/source_acquisition/content_extraction 等功能 Agent assignments。"],
    ...
    ["验证", "tests/test_data_processing_service.py + tests/test_data_processing_routes.py + tests/test_team_workflow_orchestration_service.py + tests/test_team_workflow_routes.py", "覆盖通用数据处理首切、source-collection run 启动、DataRecord 导入 source_manifest、主路径、非 ownerAgent 不能写最终状态、本地模型任务包、输出校验和知识入库状态总览。"],
  ],
};
...
const knowledgeNodeRunbook = {
  "01": {
    ...
    gap: "资料搜集批次启动、通用资料搜集 assignment/output 和 DataRecord 到 source_manifest 导入桥已接入；长 PDF 自动分批、章节识别和非 PDF 资料解析仍待补。",
    entry: "用户提供 PDF、论文、赛题或补充资料；文件可在挑战杯工作区定位。",
    operation: "由 Research Coordination Agent 调用 source-collection-runs 启动批次，系统建立通用 DataProcessingRun，并让 data_discovery/source_acquisition/content_extraction 等功能 Agent 领取 CollectionAssignment、提交 CollectionOutput；再通过 Team workflow 导入桥把合格 DataRecord 幂等转成 source_manifest 候选。对本地 PDF 运行 source-extraction，计算 sha256、生成 pageAnchors/excerpt，不改写原文，不自动混入联网资料。",
