...
const implementationBlueprint = {
  ...
  deliveredSlice: [
    ...
    ["验证", "tests/test_data_processing_service.py + tests/test_data_processing_routes.py + tests/test_team_workflow_orchestration_service.py + tests/test_team_workflow_routes.py", "覆盖通用数据处理首切、主路径、非 ownerAgent 不能写最终状态、本地模型任务包、输出校验和知识入库状态总览。"],
  ],
};
...
const knowledgeNodeRunbook = {
  "01": {
    ...
    tools: ["/api/data-processing/runs", "/api/data-processing/runs/{runId}/collection-assignments", "/api/data-processing/runs/{runId}/collection-assignments/{assignmentId}/outputs", "research_knowledge_query_tool", "/api/teams/{teamId}/workflow-orchestration/candidates/{candidateId}/source-extraction"],
    localModelUse: "本地 9B 模型适合做标题/摘要/片段初筛，输出 relevanceScore、topicTags、excludeReason，不直接决定正式纳入。",
    humanGate: "确认资料允许分析、来源可信度和是否纳入本轮。",
    gap: "通用资料搜集 assignment/output 已可落库；DataRecord 到 challenge_cup source_manifest 候选的转换桥、长 PDF 自动分批、章节识别和非 PDF 资料解析仍待补。",
    entry: "用户提供 PDF、论文、赛题或补充资料；文件可在挑战杯工作区定位。",
    operation: "先建立通用 DataProcessingRun，由 data_discovery/source_acquisition/content_extraction 等功能 Agent 领取 CollectionAssignment 并提交 CollectionOutput；再把合格 DataRecord 转成 source_manifest 候选。对本地 PDF 运行 source-extraction，计算 sha256、生成 pageAnchors/excerpt，不改写原文，不自动混入联网资料。",
