...
const nodes = [
  ...
  },
  {
    ...
    title: "实验验证闭环",
    status: "smoke 结果登记 API 已接入",
    statusKind: "active",
    role: "Experiment Planning Agent / Experiment Ledger Service",
    summary: "已把可用算法假设推进到实验计划草稿、active baseline artifact 和 smoke 结果账本；真实训练 runner 仍由后续切片承接。",
    objective: "规划 baseline/candidate 算法实验、指标、smoke 门禁和结果回写边界，先形成可审查计划，不自动训练。",
    inputs: ["experiment ResearchStageRound", "algorithm_hypothesis", "experimentPlan", "baseline/metric/dataset/smokePlan"],
    actions: [
      ...
      "Teams 实验阶段页展示实验计划账本、候选算法假设、readyForSmoke/readyForFullRun 门禁和阻塞项；smoke UI 表单待 TeamsRoute claim 释放后接入。",
      "强制边界：不创建 ExperimentResult，不触发训练 runner，不写正式 Team Knowledge/RAG/official graph。",
    ],
    outputs: ["experiment_plan 草稿", "active_baseline_artifact", "smoke_result", "readinessChecklist", "baselineSelection", "promotion/reject 前置门禁"],
    memory: "实验计划只进入 Team workflow runtime memory；实验结果是否入库仍取决于后续 smoke/full run、证据复核 Agent 和知识治理 Agent。",
    graph: "smoke_result 仍是计划账本证据，不创建 ExperimentResult 边；未来正式边仍是 AlgorithmHypothesis evaluated_by ExperimentResult。",
    risks: ["计划草稿被误认为已训练", "active baseline artifact 缺失", "smoke 指标不能验证假设", "baseline 不公平"],
    openQuestions: ["下一刀在 TeamsRoute UI claim 释放后接 smoke 表单，再评估是否需要轻量 smoke runner。"],
  },
  {
    id: "11",
...
const knowledgeRunbook = {
  ...
  featureMatrix: [
    ...
    ["Smoke 结果登记", "experiments/plans/{planId}/smoke-result 登记 smoke 指标、结果路径、日志引用和 gateDecision；passed 后 readyForFullRun=true，但仍不自动训练", "已接入后端/API"],
...
const implementationBlueprint = {
  ...
  milestones: [
    ...
    ["M7.2", "Smoke 结果登记", "新增 experiments/plans/{planId}/smoke-result：登记 smoke 指标、结果路径、日志引用和 gateDecision，并更新 activeSmokeResult。", "status=passed 才让 readyForFullRun=true；failed/needs_review 继续阻塞 full-run；接口仍不创建 ExperimentResult、不触发训练 runner。"],
  ],
  schemas: [
    "DataProcessingRun",
    "DataRecord",
  ...
  services: [
    ...
    ["TeamsRoute workflow workspace", "已落地入口：Teams 页顶部固定横向切换条只暴露 AI 搜索范围团队和 ai科学研究团队，左侧栏已删除；监督进化/自进化模式页各自读取系统 Team 并展示只读团队画布；选择 ai科学研究团队后直接进入 research-team 专属科研控制台；总览只显示知识搜集、实验、迭代三张阶段卡，每张卡一个主操作和一个阶段详情入口。知识搜集卡会显示当前批次、待处理、候选和查询数，主按钮按状态执行开始、下一批搜索、筛选入口或打开工作台，另有新一轮搜集入口。知识搜集页位于 /teams?team=research-team&researchView=knowledge_collection，默认展示高价值状态摘要、状态驱动主操作和对话式 source collection 过程流；点击知识搜集主按钮会通过 stage-rounds/start 创建轮次并立即排队第一批后台 source collection 搜索；执行 source collection 搜索时会写入 source_collection_run 运行快照，让 AppShell 顶部“正在进行”优先显示知识搜集；资料搜集 run 列表通过 data-processing 服务端 teamId/startedFrom 过滤，避免被其它团队批次挤掉；后台 accepted 后 activeWorkRun 会进入控制台和对话流，直到 runStatus 收口；资料搜集、资料筛选、候选入库、候选图谱、共享记忆前审五张步骤卡各自带主按钮并作为页内子页索引，顶部卡片只保留状态、关键数量和主操作，不再显示输入/输出小框；底部只显示当前阶段内容，并拆成结果与 Agent过程；右侧只显示当前步骤操作、当前资料详情、关键数量和当前步骤 Agent 配置；资料筛选会调用资料质量评估 Agent 批量执行，完成后展开、滚动并高亮页内筛选详情；KV/prompt cache 由后端从当前模型库解析，不再依赖旧固定模型 ID，过程流把 KV 缓存模型与执行 Agent 模型分层展示；稳定前缀、动态增量、assignment、query、存储路径和手工回写保留在可展开详情中；默认层移除解释性小字且使用实体工作台背景，避免主题壁纸压住正文；实验页新增实验计划账本，可生成计划草稿并登记 active baseline artifact，显示 baseline/metric/dataset/smoke 缺口、readyForSmoke 和 full-run 阻塞；迭代页先提供规划启动、轮次状态和边界说明；旧 source_collection 参数兼容映射到知识搜集。"],
