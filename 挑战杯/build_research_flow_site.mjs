...
const knowledgeNodeRunbook = {
  "01": {
    ...
    agentStatus: "可复用通用 data-processing assignment / source-extraction / source-quality 后端已接入；Teams 资料搜集执行台、候选资料筛选台、三阶段启动台、KV 缓存对话轨迹、启动复用反馈和 metadata-only 搜索执行闭环已接入",
    features: ["ResearchStageRound 建立", "三阶段启动台一键进入知识搜集", "active 知识搜集轮次复用反馈", "自动启动团队协调 round", "DataProcessingRun 建立", "DataSearchPlan/querySeeds 搜索计划", "KV/prompt cache 启动门禁", "稳定前缀/动态增量契约", "CollectionAssignment 资料搜集任务", "assignedQueries 角色分配", "promptCachePartition 角色分区", "Teams 启动搜集批次", "Teams 查看 run/assignment/query/KV 状态", "Teams 执行下一批搜索", "Crossref 元数据搜索执行器", "搜索执行事件进入搜集对话流", "DataRecord 通用记录", "DataRecord 自动导入 source_manifest", "metadata-only 下载边界", "手工 CollectionOutput 回写", "本地资料登记", "source_manifest 写入", "文件路径/页码/来源可信度记录", "本地 PDF sha256/pageAnchors/excerpt 抽取", "sourceQualityAssessment 筛选", "可选 research_knowledge_query_tool 查重"],
    ...
    <main class="shell">
      ...
      <section id="overview" class="review-dashboard">
        ...
        <aside class="dashboard-panel">
          <h2>审核摘要</h2>
          <div class="kpi-grid">
            <div class="kpi"><b>当前阶段</b><strong>M6.17</strong><span>知识搜集已具备 metadata-only 搜索执行闭环，可把搜索、回写和候选导入展示为对话流。</span></div>
