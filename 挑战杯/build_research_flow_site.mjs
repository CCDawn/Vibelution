...
const implementationBlueprint = {
  ...
  activeTeam: {
    ...
    defaultView: "选中 ai科学研究团队后，主工作区顶部显示科研流程二级导航：科研总览、资料搜集、团队协调、知识入库、候选图谱、候选资料、团队沟通。",
  ...
  milestones: [
    ...
    ["M6.6", "Teams 资料搜集执行台", "research-team 页面新增独立资料搜集索引页，可启动 source collection run、查看最近批次、run status、assignment、assignedQueries，并手工提交 CollectionOutput。", "启动批次会从 Team canvas 读取 data_discovery/source_acquisition/content_extraction/source_quality 等角色绑定的团队 agentId；手工回写会先写入 DataRecord，再通过 Team workflow 导入桥转成 source_manifest 候选；仍不触发真实外部搜索，不写正式 Team Knowledge/RAG/official graph。"],
  ],
  schemas: [
    "DataProcessingRun",
    "DataRecord",
  ...
  deliveredSlice: [
    ...
    ["Team 前端入口", "/teams?team=research-team", "读取 /workflow-orchestration、/candidates?limit=8、/knowledge-ingestion/status 和通用 data-processing run/status/assignment；左侧只做团队选择/创建，research-team 主工作区顶部提供科研流程二级导航，并按当前索引节点单页展示科研总览、资料搜集执行台、候选仓库、校验摘要、知识入库漏斗、actionItems 和 officialBoundary；非科研团队显示占位，不初始化 workflow。"],
