...
const implementationBlueprint = {
  ...
  milestones: [
    ...
    ["M6.50", "知识搜集可读操作台根治", "知识搜集独立页改为实体工作台：主题背景不再穿透正文，顶部五阶段卡只承担导航、状态和主按钮，不再展示输入/输出小框。", "当前阶段子页只保留结果工作台；Agent 审查与执行过程通过阶段深链跳转到对应 Agent 会话页，右侧控制区固定宽度，只保留下一步操作、当前资料详情和当前步骤 Agent 配置。"],
    ["M6.51", "阶段分页点击稳定", "知识搜集阶段结果面板的数量标识和分页控件只作为状态/翻页入口，不再触发 details summary 折叠或外层布局重排。", "点击 1-8/20、上一页、下一页或阶段标题右侧数量时，页面保持当前阶段布局；只有明确的阶段卡、Agent 会话深链和分页按钮会改变内容。"],
    ...
    ["M6.54", "阶段 Agent 与会话深链契约", "候选图谱步骤绑定 candidate_graph / Candidate Graph Preview Agent，前审步骤绑定 knowledge_steward / 共享记忆前审 Agent；阶段页不再内嵌 Agent 过程列表。", "Teams 知识搜集页为每个阶段提供对应 Agent 会话深链；用户点击进入会话页审查真实 Agent 讨论与执行过程，候选图谱和前审不再借用资料质量评估 Agent。"],
    ...
    ["M6.64", "五阶段 Agent 会话入口与来源元数据标准化", "知识搜集五张步骤卡新增 Agent 会话入口，候选图谱恢复为独立步骤，前审步骤显示为共享记忆前审；DataRecord 导入 source_manifest 时统一写入 sourceRunId、sourceRecordId、sourceCategory、doi、sourceUrl、sourcePath、query、assignmentId 和 searchProvider。", "用户可从每个阶段直接进入对应 Agent 会话页审查执行过程；五个子页可按 PDF、论文网页/DOI、数据集、本地文件和缺少来源过滤，并能从资料卡追溯 DOI、网页、本地路径或搜索证据。"],
  ...
  services: [
    ...
    ["TeamsRoute workflow workspace", "已落地入口：Teams 页顶部固定横向切换条只暴露 AI 搜索范围团队和 ai科学研究团队，左侧栏已删除；监督进化/自进化模式页各自读取系统 Team 并展示只读团队画布；选择 ai科学研究团队后直接进入 research-team 专属科研控制台；总览只显示知识搜集、实验、迭代三张阶段卡，每张卡一个主操作和一个阶段详情入口。知识搜集卡会显示当前批次、待处理、候选和查询数，主按钮按状态执行开始、下一批搜索、筛选入口或打开工作台，另有新一轮搜集入口。知识搜集页位于 /teams?team=research-team&researchView=knowledge_collection，默认展示高价值状态摘要和状态驱动主操作；点击知识搜集主按钮会通过 stage-rounds/start 创建轮次并立即排队第一批后台 source collection 搜索；执行 source collection 搜索时会写入 source_collection_run 运行快照，让 AppShell 顶部“正在进行”优先显示知识搜集；资料搜集 run 列表通过 data-processing 服务端 teamId/startedFrom 过滤，避免被其它团队批次挤掉；后台 accepted 后 activeWorkRun 会进入控制台，直到 runStatus 收口；资料搜集、资料筛选、候选入库、候选图谱、共享记忆前审五张步骤卡各自带主按钮并作为页内子页索引，顶部卡片只保留状态、关键数量和主操作，不再显示输入/输出小框；底部只显示当前阶段结果内容，不再内嵌 Agent 过程；每个阶段详情头部提供进入/创建对应 Agent 会话的深链，Agent 审查和执行过程统一到会话页追踪；右侧只显示当前步骤操作、当前资料详情、关键数量和当前步骤 Agent 配置；资料筛选会调用资料质量评估 Agent 批量执行，完成后展开、滚动并高亮页内筛选详情；KV/prompt cache 由后端从当前模型库解析，不再依赖旧固定模型 ID；稳定前缀、动态增量、assignment、query、存储路径和手工回写保留在可展开详情中；默认层移除解释性小字且使用实体工作台背景，避免主题壁纸压住正文；实验页新增实验计划账本，可生成计划草稿并登记 active baseline artifact、smoke result、full-run result，full-run passed 后可通知 Knowledge Steward Agent 处理 experiment_result_pack；实验页和迭代页共享 Research Loop 模板面板，可按算法模型、仿真模拟、数据集基准、环境探针等模板登记证据和决策；旧 source_collection 参数兼容映射到知识搜集。"],
    ...
    <main class="shell">
      ...
      <section id="overview" class="review-dashboard">
        ...
        <aside class="dashboard-panel">
          <h2>审核摘要</h2>
          <div class="kpi-grid">
            <div class="kpi"><b>当前阶段</b><strong>M6.64</strong><span>五阶段步骤卡已接入 Agent 会话入口，资料来源元数据已标准化，便于过滤和追溯。</span></div>
