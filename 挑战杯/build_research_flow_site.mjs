...
const nodes = [
  {
    ...
    title: "资料进入工作区",
    status: "Evidence Ledger 与页码锚点已接入",
    statusKind: "active",
    role: "资料寻找 Agent",
    ...
    openQuestions: ["章节识别和非 PDF 全文解析仍待补；Evidence Ledger 先作为 paper_note 与资料关系整理的候选证据输入。"],
  },
  {
    id: "02",
    slug: "paper-note",
...
const implementationBlueprint = {
  ...
  milestones: [
    ...
    ["M6.66", "阶段卡 Agent 会话任务桥", "知识搜集四张步骤卡的主按钮不再直接执行本地阶段动作，而是创建对应功能 Agent 的 direct session task 并跳转到会话页执行。", "任务落盘到 source_collection_runs/<runId>/stage_session_tasks.json；Chat URL 保留 returnTo/returnLabel；Agent 先用 source_collection_context_tool 读取受控资料上下文，再用 source_collection_stage_writeback_tool 结构化回写 sourceCollectionStageSessionTasks，团队阶段再展示结果；不通过 file:// 或 localhost fetch 猜本地资料。"],
...
const knowledgeNodeRunbook = {
  "01": {
    ...
    gap: "科研总览控制台、资料搜集阶段轮次、资料搜集批次启动、active 轮次复用反馈、DataSearchPlan/query seed 契约、KV/prompt cache 启动门禁、缓存分区对话轨迹、通用资料搜集 assignment/output、Teams 高价值摘要层、metadata-only 搜索执行器、DataRecord 到 source_manifest 导入桥、source-quality 筛选台和 Evidence Ledger 写回已接入；前端 Evidence Ledger 可视化、全文网页/PDF 下载、长 PDF 自动分批、章节识别和非 PDF 资料解析仍待补。",
  ...
  "02": {
    ...
    gap: "paper_note schema 与页码引用校验已接入 CandidateStore；本地 PDF 页码摘录已可由 source-extraction 提供，资料提炼 Evidence Ledger 已可由阶段回写沉淀，sourceExtraction -> paper_note 自动草稿桥已接入；Evidence Ledger 接入 paper_note 草稿输入、多个 chunk draft 的合并/去重和前端证据账本展示仍待接。",
      ...
      <section id="overview" class="review-dashboard">
        ...
        <aside class="dashboard-panel">
          ...
          <div class="focus-list">
            ...
            <div class="focus-item"><b>下一步</b><span>把 Evidence Ledger 展示到资料卡和资料提炼详情；继续扩展全文下载/网页抽取、质量评分和 cache hit/miss 运行证据。</span></div>
          </div>
        </aside>
      </section>
