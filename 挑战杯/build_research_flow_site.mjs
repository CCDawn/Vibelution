...
const nodes = [
  ...
  },
  {
    ...
    outputs: ["steward_pack_draft 候选", "SourceArtifact", "pending RefinementProposal", "proposal ratingSuggestion", "KnowledgeItem ratingSuggestion", "officialResearchGraph", "official_sync_record"],
    memory: "待审阶段不创建正式 KnowledgeItem；审批通过后复用 Team Knowledge review/apply 创建正式 KnowledgeItem，并把评分建议和正式科研图谱引用承接到 KnowledgeItem。",
    graph: "审批通过后正式 KnowledgeItem metadata 记录 officialResearchGraph；Memory Graph/RAG 可从正式 KnowledgeItem 继续读取，前端显式渲染仍待接。",
    risks: ["绕过审批门禁", "目标知识域错误", "把 ingestion pack 草稿误当正式入库记录"],
    openQuestions: ["是否把 KnowledgeItem metadata.officialResearchGraph 显式渲染到 Memory Graph 画布。"],
  },
  {
    id: "08",
  ...
  },
  {
    ...
    summary: "只有审核入库后的知识进入正式图谱 trace 与 RAG。",
    ...
    outputs: ["正式知识图谱 trace", "正式 RAG 可检索上下文", "officialKnowledgeItemId", "officialResearchGraph"],
    memory: "只同步 ingested 内容，正式图谱 trace 挂在 KnowledgeItem metadata 上。",
    graph: "正式图谱 trace 必须能追溯到 sourceTrace、candidateIds、reviewRecordIds 和正式 KnowledgeItem。",
    risks: ["候选误同步", "sourceFiles 丢失", "正式图谱缺审核记录"],
    openQuestions: ["前端 Memory Graph 是否直接展开 officialResearchGraph 边，还是先作为 KnowledgeItem detail 展示。"],
  },
  {
    id: "10",
...
const knowledgeRunbook = {
  ...
  featureMatrix: [
    ...
    ["正式图谱同步", "approved 后进入 Team Knowledge/RAG，并把正式科研边写入 KnowledgeItem metadata.officialResearchGraph；Memory Graph 显式展开仍待前端接线", "已接线+待展示"],
  ],
  projectAlignment: [
...
const implementationBlueprint = {
  ...
  milestones: [
    ...
    ["M5", "知识治理与正式同步", "steward_pack_draft 门禁、待审入库桥、Ingestion Approval Gate、评分建议迁移和 officialResearchGraph 正式边已落地：有效草稿批准后创建正式 KnowledgeItem、承接待审评级并记录正式科研 trace，拒绝后退回修订。", "正式 RAG 通过已审核 KnowledgeItem 检索；正式图谱边先落在 KnowledgeItem metadata，前端显式展开仍待接。"],
  ],
  schemas: [
    "source_manifest",
    "paper_note",
...
const knowledgeNodeRunbook = {
  ...
  "07": {
    ...
    features: ["sourceTrace 校验", "proposalPayload", "ratingSuggestion", "approvalRequired=true", "SourceArtifact", "pending RefinementProposal", "KnowledgeItem rating suggestion migration", "officialResearchGraph", "officialSyncRecord", "official write 禁止项"],
    ...
    gap: "pending ratingSuggestion 到正式 KnowledgeItem rating review 的承接已接入；正式 supports/maps_to/inspires/approved_for_ingestion 边已写入 officialResearchGraph，前端显式图谱展开仍待补。",
    ...
    exit: "合格草稿提交后进入 steward_pending_knowledge_review；批准后进入 official_synced、记录 officialSyncRecord 和 officialResearchGraph，并创建 KnowledgeItem 级 pending ratingSuggestion；拒绝后进入 steward_needs_revision。",
  ...
  "09": {
    ...
    agentStatus: "授权审批门禁已接入 Team Knowledge review/apply、KnowledgeItem metadata patch；平台负责正式 KnowledgeItem 的读取、RAG 检索和 Memory Graph 展示",
    features: ["正式审核确认", "正式 KnowledgeItem 创建/同步", "KnowledgeItem rating review", "officialResearchGraph", "RAG 可检索", "official_sync_record", "Memory Graph 可见"],
    tools: ["knowledge_query_tool", "knowledge_rag_retrieve_tool", "Team Knowledge 审核/应用流程（平台能力）"],
    humanGate: "必须通过授权审批门禁批准 ingestion pack。",
    gap: "ratingSuggestion 自动迁移到 KnowledgeItem 待审评级已接入；正式 supports/maps_to/inspires/approved_for_ingestion 边已接入 officialResearchGraph；Memory Graph 画布显式展开仍待接。",
    entry: "授权审批门禁已批准 pending RefinementProposal。",
    operation: "复用 Team Knowledge review/apply 创建 KnowledgeItem；旧 proposal 级 ratingSuggestion 标记 applied，新 KnowledgeItem 级 ratingSuggestion 保持 pending 等待评级审阅；sourceTrace/candidateIds 被翻译为 officialResearchGraph 正式边；RAG 通过 reviewed Team Knowledge 检索。",
    exit: "knowledgeItemIds、ratingSuggestionMigration、officialResearchGraph、ragStatus、graphStatus、reviewedByAgentId 和 officialSyncRecord 均已记录。",
