...
- **写入前必须同时做本地复用评估与仓外成熟方案调研（二者都要做，不是二选一）**：凡会改代码、行为、架构或验证边界的任务，禁止盲目开写。先定位 owning surface 与可复用实现，并评估它是否真的合适——本地能复用 ≠ 本地就是好方案，也 ≠ 可以原样照搬；不够好就要**改造后再复用**，或换更合适的路径。同时对照 GitHub、论文、官方文档/标准等成熟方案。调研之后必须**评估排序**：按项目贴合度排出候选，只抽取**最符合本项目、最值得借鉴的部分**再复用或改造后复用；禁止把整仓原样拷进 Vibelution 产品代码，禁止只堆链接不裁决。高价值公开 GitHub 仓：先读 `activePaths.memory/github-projects/INDEX.md`；未命中则克隆默认主干最新提交进该目录，**再**对本地仓调研。不要把网页/API 当结论，也不要把整仓正文写入正式知识库。深度随分级缩放，两扇门都不得整段跳过。细则见 [development-standard.md §2.2](docs/standards/development-standard.md)。
- **实现变更必须留下机器可复核的复用研究证据**：在任务 `closeout` 前用 `scripts/reuse_research_evidence.py record` 记录功能、local owner 与复用裁决、成熟候选、实际借鉴切片、拒绝理由、实现边界和验证策略。候选 URL、本地路径、固定 HEAD SHA、许可证和 ready 状态只能由 active GitHub library registry 自动填充，Agent 不得手填。已有合适候选时直接复用，不要求每个任务新增仓库或凑数量；缺少证据、候选漂移、脏 clone、许可证不兼容或 manifest 证据失效都会阻断 `closeout` / `verify-manifest`。纯文档、测试、fixture 和 example 变更默认豁免。细则见 [development-standard.md §2.2](docs/standards/development-standard.md) 与 [loop.md](docs/guides/loop.md)。
