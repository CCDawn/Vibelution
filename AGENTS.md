...
- **Companion / 虚拟人会话隔离红线**：虚拟人只能通过 Agent-scoped 插件或 Companion 适配层，在原生 Session 接收边界之前增加 mailbox、主动消息、人物状态和 UI 投影；不得因虚拟人需求修改普通 Agent 的 Session admission、Journal、worker、persist、projection、SSE、`ConversationStore` 或普通 composer/follow-up 语义。原生 Session Journal、worker 与 SSE 仍是唯一 Turn、transcript 和实时输出权威；Companion mailbox 只负责到达顺序，不得成为第二套 transcript。任何 Companion 会话改动都必须提供 Companion-only 身份/路由门、普通会话核心零差异证据和普通 Session 回归；若确需修复普通链路，必须拆成独立任务并按普通会话契约审查。细则见 [development-standard.md §23.2](docs/standards/development-standard.md)。
