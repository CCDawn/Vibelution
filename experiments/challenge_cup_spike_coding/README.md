...
root (`$env:VIBELUTION_OPERATOR_DATA_ROOT`). The experiment excludes units
marked `heldout`, preserves the dataset's train/validation split, and records a
count-preserving temporal shuffle control.

Example (paths are placeholders; do not commit machine-local absolute paths):

```powershell
$root = $env:VIBELUTION_OPERATOR_DATA_ROOT
$env:PYTHONPATH = "$root/experiments/sci096_spike_coding/python_packages"
$python = $env:VIBELUTION_PYTHON
$input = "$root/experiments/sci096_spike_coding/source/sub-Jenkins_ses-small_desc-train_behavior+ecephys.nwb"
$output = "$root/experiments/sci096_spike_coding/runs/sci096-dandi000140-probe-v1/result.json"
...
$python = $env:VIBELUTION_PYTHON
$input = "$root/experiments/sci096_spike_coding/source/sub-Jenkins_ses-small_desc-train_behavior+ecephys.nwb"
$output = "$root/experiments/sci096_spike_coding/runs/sci096-dandi000140-epoch-discrimination-v3/result.json"
...
```powershell
$root = $env:VIBELUTION_OPERATOR_DATA_ROOT
$qualification = "$root/experiments/sci096_spike_coding/qualification/dandi000121/multisession_manifest_v2.json"
$sourceRoot = "$root/experiments/sci096_spike_coding/source/dandi000121"
$plan = "$root/experiments/sci096_spike_coding/qualification/dandi000121/download_plan_v1.json"
