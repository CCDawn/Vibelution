...
(function (root, factory) {
  ...
  function normName(n) {
    ...
    // See-through emits pre-separated semantic parts with -l/-r suffixes.
    // Normalize them to the base slot; spatial splitting restores side data.
    n = n.replace(/[-_](l|r)$/,'');
    return n;
  }
  ...
  var SLOTS = {
    ...
    'tail':        { depth: 0.72, group: 'body' },
    'bottomwear':  { depth: 0.88, group: 'body' },
    'footwear':    { depth: 0.89, group: 'body' },
    ...
    'eyebrow':     { depth: 1.30, group: 'head', split: true },
    'irides':      { depth: 1.08, group: 'head', split: true, fade: 'eyeOpen' },
    'eye_detail':  { depth: 1.13, group: 'head', split: true, fade: 'eyeOpen' },
  ...
  function buildRig(psd, opts) {
    ...
    // See-through PSD order is semantic rather than render-safe. Establish
    // stencil and occlusion order from slot depths before assigning z.
    parts.sort(function (a, b) { return a.depth - b.depth || a.z - b.z; });
