const styles = {
  ...
  operationDetailLabel:
    "vui-components-conversationview operationDetailLabel min-w-0 pt-0.5 [font-size:var(--vui-font-xs)] font-semibold leading-tight text-[var(--fg-tertiary)] [overflow-wrap:anywhere]",
  operationDetailDescription:
    "vui-components-conversationview operationDetailDescription m-0 min-w-0",
  operationDetailValue:
    "vui-components-conversationview operationDetailValue m-0 min-w-0 max-w-full max-h-44 overflow-auto whitespace-pre-wrap break-words bg-transparent p-0 font-[var(--font-mono)] [font-size:var(--vui-font-xs)] leading-[1.45] text-[var(--fg-secondary)] [overflow-wrap:anywhere] focus-visible:rounded-[var(--radius-control)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[color-mix(in_srgb,var(--accent-cool)_34%,transparent)]",
} as const;
