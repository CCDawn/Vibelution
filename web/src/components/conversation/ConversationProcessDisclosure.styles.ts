...
const styles = {
  ...
  summary: cx(
    "summary",
    "flex min-h-7 w-full cursor-pointer items-center justify-start gap-1 border-b border-[var(--vui-border-subtle)] py-1 text-left [font-size:var(--vui-font-xs)] leading-5 text-[var(--fg-tertiary)] [&::-webkit-details-marker]:hidden hover:text-[var(--fg-secondary)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[color-mix(in_srgb,var(--accent-cool)_42%,transparent)] focus-visible:ring-inset",
  ),
  chevron: cx(
    "chevron",
    "shrink-0 text-[var(--fg-tertiary)] transition-transform duration-150 group-open:rotate-90",
  ),
  content: cx("content", "grid min-w-0 content-start gap-0 pt-3"),
} as const;
