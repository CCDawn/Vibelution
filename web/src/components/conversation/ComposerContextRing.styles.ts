const styles: Record<string, string> = {
  ...
  hitEdge:
    "vui-components-conversation-composercontextring hitEdge flex h-1 overflow-hidden rounded-full bg-[color-mix(in_srgb,var(--vui-border-subtle)_85%,transparent)]",
  hitSeg:
  ...
  rows:
    "vui-components-conversation-composercontextring rows grid gap-0.5",
  row:
    "vui-components-conversation-composercontextring row grid min-w-0 grid-cols-[8px_minmax(0,1fr)_auto] items-center gap-1.5 rounded-[6px] px-1 py-0.5 text-[11px] hover:bg-[color-mix(in_srgb,var(--vui-control-muted)_55%,transparent)]",
  swatch:
