const styles = {
  ...
  imagePreviewDialog:
    "vui-components-conversationview imagePreviewDialog grid max-h-[calc(100vh-2rem)] w-[min(100vw-2rem,72rem)] min-w-0 grid-rows-[auto_minmax(0,1fr)] gap-3 overflow-hidden rounded-[var(--radius-panel)] border border-[var(--vui-border-subtle)] bg-[var(--vui-surface-panel)] p-3 shadow-[var(--vui-shadow-floating)]",
  imagePreviewLarge:
  ...
  imagePreviewOverlay:
    "vui-components-conversationview imagePreviewOverlay fixed inset-0 z-50 grid min-w-0 place-items-center overflow-y-auto bg-[color-mix(in_srgb,var(--vui-surface-base)_72%,transparent)] p-4 backdrop-blur-sm",
  imagePreviewTitle:
