...
const styles = {
  ...
  imagePreviewCloseButton:
    "vui-components-conversationview imagePreviewCloseButton inline-flex size-[var(--vui-control-height-sm)] shrink-0 items-center justify-center rounded-[var(--radius-control)] border border-[var(--vui-border-subtle)] bg-[var(--vui-control-muted)] p-0 text-[var(--fg-secondary)] hover:border-[var(--vui-control-hover-border)] hover:bg-[var(--vui-control-hover-bg)] hover:text-[var(--vui-control-hover-fg)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-cool)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--vui-surface-panel)] disabled:cursor-default disabled:opacity-55",
  imagePreviewDialog: `vui-components-conversationview imagePreviewDialog grid max-h-[calc(100vh-2rem)] w-[min(100vw-2rem,72rem)] min-w-0 grid-rows-[auto_minmax(0,1fr)] gap-3 overflow-hidden ${vuiFlatPanelClass} p-3 shadow-[var(--vui-shadow-floating)]`,
  imagePreviewLarge: `vui-components-conversationview imagePreviewLarge block max-h-[calc(100vh-8rem)] max-w-full min-w-0 justify-self-center ${vuiGlassPanelClass} object-contain shadow-[var(--vui-shadow-hairline)]`,
  imagePreviewOverlay:
    "vui-components-conversationview imagePreviewOverlay fixed inset-0 z-50 grid min-w-0 place-items-center overflow-y-auto !bg-[var(--vui-surface-base)] p-4 backdrop-blur-sm",
  imagePreviewTitle:
