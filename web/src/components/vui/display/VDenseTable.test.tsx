...
describe("VDenseTable", () => {
  ...
  it("lets a fill column absorb leftover space so trailing actions keep their pixel track", () => {
    ...
    expect(html).toContain("var(--vui-window-width, 100svw)");
    ...
    expect(denseTableFillGridStyle("minmax(0, 1fr) 170px")).toEqual({
      contain: "inline-size",
      display: "grid",
      gridTemplateColumns: "minmax(0, 1fr) 170px",
      width: "min(100%, var(--vui-window-width, 100svw))",
      maxWidth: "min(100%, var(--vui-window-width, 100svw))",
      minWidth: "0px",
    });
  });
