...
- 角色与显示偏好保存于当前 origin 的 `vibelution.desktop-pet.preferences.v1`；关闭重开恢复角色，但不改变默认不自动启动的生命周期。
- 人物附近的设置按钮复用同一 HUD 区域，使用既有 `VCheckbox` 与 `VInput` 设置状态提示、标题隐私、完成提示和空闲文案。保存失败明确提示仅本次有效；提示文案是纯文本，不进入模型。
- 会话列表支持滚动查看全部活动及历史异常。跳转必须得到主进程 `opened: true` 才收起，失败保留列表并提示。
...
- 异常仅在最近五分钟内占用聚合提醒优先级；历史异常保留列表，排在当前活动之后。待审批不超时隐藏。已确认运行的新轮次优先于上一轮失败。
- 完成仅由成功结果触发，暂停、取消、停止不庆祝；统一使用 Session 时间解析。关闭完成提示时不播放庆祝；状态读取失败显示不可用并停止展示缓存的工作动作。
