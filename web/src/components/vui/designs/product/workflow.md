...
- 节点拖动时，边每帧读取 React Flow 当前端点，并使用本地正交回退路线实时跟随；源端和目标端都先沿 Handle 方向保留 `32px` 笔直引线，再进入主体通道，避免线头重合后互相遮挡。拖卡片时先按相邻卡片的边/中线对齐（阈值 `8px`，出现参考线并吸附）；参考线画在 `ViewportPortal` 里（与卡片同一视口变换），禁止用盖住 `.react-flow__renderer` 的全屏兄弟层。未贴齐的轴仍落到 `16px` 网格，点状背景间距与该网格相同。禁止只用网格、错过中线或邻边贴齐。
