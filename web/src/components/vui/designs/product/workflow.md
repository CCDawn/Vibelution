...
空间画布内部的「聚焦阶段」缩放操作使用所选节点的 stageId；未选择节点时使用当前工作节点的 stageId，仅将该阶段节点适配进视口，最大缩放为 1。它不隐藏节点、不修改阶段状态或连线，后续状态更新不得覆盖用户的阶段视口。无有效阶段时不提供该操作；「适应全部」可恢复全图总览。

布局 Worker 初始化期间显示「正在加载布局引擎」，不启动布局计算超时计时，也不向尚未就绪的 Worker 排队发送旧图。通过 ELK 查询握手后，只对最新图执行布局；计算仍保留原有 3 秒降级与迟到结果恢复语义。资源加载失败显示初始化诊断，不冒充计算超时。
...
`VWorkflowCanvas.viewMode="stage"` 使用与空间画布完全相同的 `graph`、
`selectedNodeId`、`runtimeCurrentNodeIds`。默认定位当前任务所在阶段；明确选择节点后
显示该节点所属阶段。卡片保持可读字号和最小 122px 点击区，根据容器宽度展示一或两列。
状态词与颜色复用 `workflowCanvasState`，选择态永远不改变运行态。
依赖入口只来自真实 `graph.edges`，保留条件与跨阶段目标；卡片顺序不代表串行执行。
全流程继续使用 React Flow/ELK，聚焦视图不写入运行、进度或布局持久化。
renderer 内实现 `WorkflowStageView`，业务页面只使用 VUI 产品 API。
