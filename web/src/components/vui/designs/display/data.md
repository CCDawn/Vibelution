...
| columns[].fill | 弹性列标记 | 可缩放表用 `role="table"` 的 CSS Grid + 行级 `subgrid`，避免在 `<table>`/`<tr>` 上使用 `display:contents`（Chromium 会丢掉后列）。fill 列 `minmax(0, 1fr)`，非 fill 列像素 track。首帧表宽 `100%`；挂载后用 `position:fixed` 探针量可见窗口，再和 `visualViewport`/`innerWidth` 取较小值写成像素 `width`/`max-width`。祖先被内容撑宽时 `%`、`100vw` 和 `innerWidth` 都会跟着溢出文档走。`min-width` 不得超过该上限。调用方 flex 中间层也要 `min-w-0`。不要给 fill 设 `width:100%` |
