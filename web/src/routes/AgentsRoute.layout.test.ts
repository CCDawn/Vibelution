...
import agentArchiveProtectionSource from "./agentArchiveProtection.ts?raw";
...
const agentMutationSource = [
  configDraftMutationsSource,
  workbenchMutationsSource,
].join("\n");
...
describe("AgentsRoute layout contract", () => {
  ...
  it("opens deep-linked Agent configuration and offers a governed return action", () => {
    ...
    expect(routeSource).toContain("safeAgentCenterReturnTo");
    ...
    expect(agentRouteBulkModelSource).toContain('normalized === "self_evolution"');
    expect(agentRouteBulkModelSource).toContain("返回自进化");
    expect(agentRouteBulkModelSource).toContain('normalized === "tools"');
    expect(agentRouteBulkModelSource).toContain("返回工具配置");
    expect(agentRouteBulkModelSource).toContain('normalized === "teams"');
    expect(agentRouteBulkModelSource).toContain("返回团队");
    expect(agentRouteBulkModelSource).toContain('normalized === "chat"');
    expect(agentRouteBulkModelSource).toContain("返回会话");
    expect(agentRouteBulkModelSource).toContain('normalized === "memory"');
    expect(agentRouteBulkModelSource).toContain("返回记忆库");
    expect(agentRouteBulkModelSource).toContain('normalized === "research_flow"');
    expect(agentRouteBulkModelSource).toContain("返回科研流程画布");
    expect(agentsRouteCopySource).toContain("returnBannerTitle: \"返回跳转前页面\"");
  ...
  it("keeps common Agent filters prominent and folds low-frequency filters away", () => {
    ...
    expect(agentsRouteCopySource).toContain("groupLabels:");
    ...
    expect(agentsRouteCopySource).toContain('team_index: "团队索引"');
    expect(agentsRouteCopySource).toContain('source_scope: "来源范围"');
    expect(agentsRouteCopySource).toContain('team_index: "Team indexes"');
    expect(agentsRouteCopySource).toContain('source_scope: "Source scope"');
    expect(agentsRouteCopySource).toContain('moreFilters: "更多筛选"');
    expect(agentsRouteCopySource).toContain('moreFilters: "More filters"');
  ...
  it("shows LLM names in model selectors instead of role-prefixed profile labels", () => {
    ...
    expect(agentsRouteCopySource).toContain("按 Agent 自己配置对话、心智模型、摘要、子 Agent 和视觉等 LLM 槽位");
    expect(agentsRouteCopySource).toContain("设置页只维护模型库资产");
  ...
  it("keeps create wizard and tool-governance labels off the Agents shell static graph (U1/U2)", () => {
    expect(routeSource).toContain('import("./agent-create/AgentCreateWizardDialog")');
    expect(routeSource).not.toMatch(/import \{ AgentCreateWizardDialog \} from "\.\/agent-create\/AgentCreateWizardDialog"/);
    expect(routeSource).toContain("{createOpen ? (");
  ...
  it("owns pure list/presentation helpers outside AgentsRoute (D3/M1/M2)", () => {
    ...
    expect(agentRoutePolicyDraftModelSource).toContain("function memoryPolicyDraftFromAgent");
    expect(agentRoutePolicyDraftModelSource).toContain("function membershipDraftFromWorkspace");
    expect(agentRoutePolicyDraftModelSource).toContain("function delegationPolicyDraftFromAgent");
    expect(agentRoutePolicyDraftModelSource).toContain("function supervisionPolicyDraftFromAgent");
    expect(agentRouteBulkModelSource).toContain("function bulkConfigDraftFromAgents");
    expect(agentRouteBulkModelSource).toContain("function agentArchiveProtected");
    expect(agentRouteDraftModelSource).toContain("function configDraftEqualsDraft");
    expect(routeSource).toContain("buildAgentModelChoices");
  ...
  it("keeps permanent Agent deletion behind the archived-state safety gate", () => {
    ...
    expect(workbenchMutationsSource).toContain("已彻底删除归档 Agent");
  ...
  it("creates Agents through tool bundle presets instead of raw tool strings", () => {
    ...
    expect(agentCreateContractSource).toContain("selectedPreferredTools.filter((tool) => allowedTools.includes(tool))");
    expect(agentCreateContractSource).not.toContain("REQUIRED_SESSION_AGENT_ALLOWED_TOOLS");
    expect(agentCreateContractSource).not.toContain("REQUIRED_SESSION_AGENT_PREFERRED_TOOLS");
  ...
  it("uses user-facing Chinese labels instead of internal workspace terms", () => {
    const workbenchLabelsSource = `${routeSource}\n${agentsRouteCopySource}`;
    expect(workbenchLabelsSource).toContain("系统编号");
    expect(workbenchLabelsSource).toContain("工具能力");
    expect(workbenchLabelsSource).toContain("记忆设置");
    expect(workbenchLabelsSource).toContain("工作空间");
    expect(workbenchLabelsSource).toContain("私人工作区");
    expect(workbenchLabelsSource).toContain("共享资料区");
    expect(workbenchLabelsSource).toContain("默认保存位置");
    expect(workbenchLabelsSource).toContain("使用位置");
    expect(workbenchLabelsSource).toContain("协作助手");
    expect(workbenchLabelsSource).toContain("工具能力模板");
    expect(workbenchLabelsSource).toContain("记忆范围模板");
    expect(workbenchLabelsSource).not.toContain("后台编号");
    expect(workbenchLabelsSource).not.toContain("工具权限");
    expect(workbenchLabelsSource).not.toContain("记忆策略");
    expect(workbenchLabelsSource).not.toContain("工作领地");
    expect(workbenchLabelsSource).not.toContain("私有写入根");
    expect(workbenchLabelsSource).not.toContain("共享读取区");
    expect(workbenchLabelsSource).not.toContain("默认写入边界");
    expect(workbenchLabelsSource).not.toContain("模式归属");
    expect(workbenchLabelsSource).not.toContain("策略注册表待接入");
    expect(workbenchLabelsSource).not.toContain("记忆边界模板");
    expect(workbenchLabelsSource).not.toContain("工具权限模板");
  });

  it("edits the minimal Agent card fields through the Agent PATCH endpoint", () => {
    expect(routeSource).toContain("AgentConfigDraft");
    expect(agentMutationSource).toContain("useMutation");
    expect(routeSource).toContain("updateAgentMutation");
    ...
    expect(configDraftMutationsSource).toContain("displayName: payload.draft.displayName");
    expect(configDraftMutationsSource).toContain("llmBindings: options.normalizeAgentLlmBindings(payload.draft.llmBindings)");
    expect(configDraftMutationsSource).toContain("promptTemplateId: payload.draft.promptTemplateId");
  ...
  it("edits Agent persona profile from AgentDirectory without recommendation automation", () => {
    ...
    expect(workbenchMutationsSource).toContain("personaProfile: options.personaProfileFromDraft(payload.draft)");
    expect(routeSource).toContain("updatedAgentWorkspaceCache");
    expect(workbenchMutationsSource).toContain("updatedAgentWorkspaceCache");
    expect(workbenchMutationsSource).toContain("options.setPersonaDraft(options.personaDraftFromAgent(agent))");
    expect(workbenchMutationsSource).toContain("options.draftSyncSourceRef.current = options.draftSyncSourceFromAgent(options.getWorkspace(), agent)");
  ...
  it("edits Agent task profile from AgentDirectory without automatic routing", () => {
    ...
    expect(workbenchMutationsSource).toContain("taskProfile: options.taskProfileFromDraft(payload.draft)");
  ...
  it("edits Agent memory policy from the same detail card", () => {
    ...
    expect(workbenchMutationsSource).toContain("memoryPolicy: {");
    expect(workbenchMutationsSource).toContain("readSharedGroups: options.sortedIds(payload.draft.readSharedGroups)");
    expect(workbenchMutationsSource).toContain("writeSharedGroups: options.sortedIds(payload.draft.writeSharedGroups)");
    expect(workbenchMutationsSource).toContain("readKnowledgeBaseIds: options.sortedIds(payload.draft.readKnowledgeBaseIds)");
    expect(workbenchMutationsSource).toContain("proposeKnowledgeBaseIds: options.sortedIds(payload.draft.proposeKnowledgeBaseIds)");
    expect(workbenchMutationsSource).toContain("reviewKnowledgeBaseIds: options.sortedIds(payload.draft.reviewKnowledgeBaseIds)");
    expect(workbenchMutationsSource).toContain("rateKnowledgeBaseIds: options.sortedIds(payload.draft.rateKnowledgeBaseIds)");
  ...
  it("includes work-session Agent setup copy for model instructions and workspace boundaries", () => {
    ...
    expect(routeSource).toContain("const sectionOrder = [\"status\", \"boundary\", \"team_index\"] as const");
    expect(routeSource).toContain("const sectionOrder = [\"source_scope\", \"mode\", \"reference\"] as const");
    expect(agentsRouteCopySource).toContain("managementModelPrompt:");
    expect(agentsRouteCopySource).toContain("managementWorkspace:");
    expect(agentsRouteCopySource).toContain("nextSetupModelPrompt:");
    expect(agentsRouteCopySource).toContain("nextSetupWorkspace:");
    expect(agentsRouteCopySource).toContain("Model / instructions");
    expect(agentsRouteCopySource).toContain("Check workspace boundary");
    expect(agentsRouteCopySource).toContain("配置模型与项目指令");
    expect(agentsRouteCopySource).toContain("检查工作区边界");
    expect(agentsRouteCopySource).toContain("Session entry Agents");
    expect(agentsRouteCopySource).toContain("Team / research role Agents");
    expect(agentsRouteCopySource).toContain("会话入口 Agent");
    expect(agentsRouteCopySource).toContain("团队/科研角色 Agent");
    ...
    expect(agentsRouteCopySource).toContain("nonSessionAgentColumn:");
    ...
    expect(agentsRouteCopySource).toContain("非会话 Agent");
    expect(agentsRouteCopySource).toContain("Non-session Agents");
  });
  ...
  it("adds task-oriented Agent management filters for configuration gaps", () => {
    ...
    expect(agentsRouteCopySource).toContain("managementFilterMissingPersona:");
    expect(agentsRouteCopySource).toContain("managementFilterMissingTask:");
    expect(agentsRouteCopySource).toContain("managementFilterMissingTools:");
    expect(agentsRouteCopySource).toContain("managementFilterNoTeam:");
    expect(agentsRouteCopySource).toContain("managementFilterPendingInbox:");
    expect(agentsRouteCopySource).toContain("managementFilterMaintenance:");
  });
  ...
  it("edits Agent runtime delegation and supervision policies from the activity pane", () => {
    ...
    expect(workbenchMutationsSource).toContain("delegationPolicy: {");
    expect(workbenchMutationsSource).toContain("supervisionPolicy: {");
  ...
  it("optimistically removes single-Agent archive and purge actions before backend confirmation", () => {
    expect(agentRouteBulkModelSource).toContain("function optimisticArchivedAgent(agent: AgentConfigWorkspaceAgent)");
    expect(routeSource).toContain("optimisticArchivedAgent");
    expect(workbenchMutationsSource).toContain("queryClient.cancelQueries({ queryKey: queryKeys.agentConfigWorkspace() })");
    expect(workbenchMutationsSource).toContain("const previousSelectedAgentId = options.getSelectedAgentId()");
  ...
  it("offers a governed per-Agent debug reset without archive or membership cleanup", () => {
    ...
    expect(workbenchMutationsSource).toContain("queryKeys.agentRuntimeEvidence(agent.agentId)");
    expect(routeSource).toContain("selectedAgent.status !== \"archived\"");
  ...
  it("reconciles stale direct-session caches after resetting an Agent session", () => {
    ...
    expect(workbenchMutationsSource).toContain("options.reconcileResetDirectSession(result.resetSummary)");
    expect(workbenchMutationsSource).toContain("queryClient.removeQueries({ queryKey: queryKeys.session(previousDirectSessionId), exact: true })");
    expect(workbenchMutationsSource).toContain("options.chatWorkspaceCache.afterChatWorkspaceReset()");
  });
  ...
  it("keeps per-Agent avatar, governance, and inbox actions scoped to their target object", () => {
    ...
    expect(workbenchMutationsSource).toContain("(current) => options.updatedAgentWorkspaceCache(current, agent)");
    expect(workbenchMutationsSource).toContain("const agent = result.agent as AgentConfigWorkspaceAgent");
    ...
    expect(workbenchMutationsSource).toContain("queryKeys.agentMessages(variables.agentId, \"pending\")");
  ...
  it("supports bulk Agent selection with prompt editing and protected safe archive", () => {
    ...
    expect(agentRouteBulkModelSource).toContain("agentMetadataArchiveProtected(agent)");
    expect(agentArchiveProtectionSource).toContain('metadataFlag(agent, "fixedRole")');
    expect(agentArchiveProtectionSource).toContain('metadataString(agent, "supervisedRole")');
    ...
    expect(routeSource).toContain("bulkUpdateAgentPromptTemplate<");
    expect(routeSource).toContain("bulkUpdateAgentConfig<");
    ...
    expect(routeSource).toContain("promptTemplateId: bulkPromptTemplateId");
    expect(routeSource).toContain("bulkUpdatedAgentWorkspaceCache");
    expect(routeSource).toContain("requestBulkArchiveAgents<");
    expect(routeSource).toContain("requestBulkPurgeAgents<");
