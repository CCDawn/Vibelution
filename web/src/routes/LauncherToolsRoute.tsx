...
export function LauncherToolsRoute() {
  ...
  const registryClassificationSummary = (["stale", "orphan", "conflict", "unknown"] as const)
    .map((classification) => {
  ...
  const portConflictSummary = cleanupSnapshot?.portConflicts.length
    ? cleanupSnapshot.portConflicts
      ...
      .join(", ")
    : "";
  const automaticCleanupSummary = cleanupSnapshot?.lastCompletedAt && cleanupSnapshot.cleanedCount > 0
  ...
  const worktreeDryRunSummary = cleanupSnapshot?.worktreeDryRun.length
    ? String(cleanupSnapshot.worktreeDryRun.length)
    : "";
  ...
  const nextReconcileSummary = stateQuery.data?.nextReconcileAt
    ...
    : "";
  const emptyRegistrySpec = uiLang === "zh" ? "无" : "None";
  const controlPlaneSpecs = [
    ...
    ...(portConflictSummary
      ...
      : []),
    ...(automaticCleanupSummary
      ? [{ label: uiLang === "zh" ? "最近自动清理" : "Recent automatic cleanup", value: automaticCleanupSummary }]
      : []),
    ...(worktreeDryRunSummary
      ? [{ label: "Worktree dry-run", value: worktreeDryRunSummary }]
      : []),
