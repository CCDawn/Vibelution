...
import knowledgeReviewPanelStyles from "./MemoryKnowledgeReviewPanel.styles";
...
describe("MemoryRoute layout contract", () => {
  it("cancels stale entry queries when the memory view changes", () => {
    const entryQuerySource = workbenchQueriesSource.slice(
      workbenchQueriesSource.indexOf("export function useMemoryCoreQueries"),
      workbenchQueriesSource.indexOf("export function useMemoryKnowledgeQueries"),
    );
    expect(entryQuerySource.match(/queryFn: \(\{ signal \}\) =>/g)?.length ?? 0).toBeGreaterThanOrEqual(5);
    expect(entryQuerySource).not.toContain("queryFn: () =>");
    expect(entryQuerySource.match(/\{ signal \}/g)?.length ?? 0).toBeGreaterThanOrEqual(10);
  });

  it("cancels stale deep queries when memory selections change", () => {
    // Read queries live in workbench hooks; route keeps one deferred item-detail query.
    expect(workbenchQueriesSource.match(/queryFn: \(\{ signal \}\) =>/g)?.length ?? 0).toBeGreaterThanOrEqual(20);
    expect(workbenchQueriesSource).not.toContain("queryFn: () =>");
    expect(routeSource.match(/queryFn: \(\{ signal \}\) =>/g)?.length ?? 0).toBe(1);
  ...
  it("keeps the source audit filter stable instead of deriving focus from the first item", () => {
    expect(routeSource).toMatch(/const activePair =\s*activeItemId\s*\?\s*flatVisibleItems\.find\(\(\{ item \}\) => item\.id === activeItemId\) \?\? null\s*:\s*null;/);
    ...
    expect(routeSource).toMatch(/onSelectAllSections=\{\(\) => \{\s*setActiveItemId\(\"\"\);\s*setActiveSectionId\(\"\"\);\s*\}\}/);
    expect(routeSource).toMatch(/onSelectSection=\{\(sectionId\) => \{\s*setActiveItemId\(\"\"\);\s*setActiveSectionId\(sectionId\);\s*\}\}/);
  ...
  it("keeps dense Memory workspaces bounded with internal scrolling", () => {
    ...
    // Scrollport lives on overview parent (reviewQueueScroll); list itself is content-start.
    ...
    expect(matrixPanelStyles.matrixCardButton).toContain("text-left");
    expect(matrixPanelStyles.matrixCardButton).not.toContain("inline-flex");
  ...
  it("preserves a compact narrow Memory layout without turning every detail pane full width", () => {
    expect(managePanelStyles.manageWorkspace).toContain("grid-cols-[minmax(300px,0.76fr)_minmax(0,1fr)]");
    expect(managePanelStyles.manageWorkspace).toContain("grid-rows-[minmax(0,0.58fr)_minmax(0,1fr)]");
    expect(managePanelStyles.manageWorkspace).toContain("h-full");
    expect(managePanelStyles.manageWorkspace).toContain("overflow-hidden");
    expect(managePanelStyles.manageWorkspace).toContain("[&_.manageListPanel]:row-span-2");
    expect(managePanelStyles.manageWorkspace).toContain("[&_.detailPanel]:col-start-2");
    expect(managePanelStyles.manageWorkspace).toContain("[&_.detailPanel]:row-start-2");
    expect(managePanelStyles.manageWorkspace).toContain("[&_.detailPanel]:max-h-none");
    expect(reviewQueuePanelStyles.reviewReasonList).toContain("flex");
  });
  ...
  it("lets routed Memory subviews fill the route slot before their own panels scroll", () => {
    ...
    for (const workspaceClassName of [agentMemoryPanelStyles.agentMemoryWorkspace, styles.knowledgeWorkspace, graphViewPanelStyles.graphWorkspace]) {
  ...
  it("keeps restored MemoryRoute grids from the CSS module migration", () => {
    const restoredGridExpectations: Array<[string, string]> = [
      [matrixPanelStyles.matrixCard, "grid-cols-[minmax(0,1fr)_auto]"],
      [projectMemoryQueuePanelStyles.projectMemoryProposalTitleLine, "grid-cols-[minmax(0,1fr)_auto]"],
      [reviewQueuePanelStyles.reviewQueueTitleLine, "grid-cols-[minmax(0,1fr)_auto]"],
      [managePanelStyles.manageSourceFilters, "grid-cols-[repeat(auto-fit,minmax(82px,1fr))]"],
      [sourceAndItemPanelStyles.sourceButton, "grid-cols-[24px_minmax(0,1fr)_auto]"],
      [knowledgeUsageContractPanelStyles.contractDomainRow, "grid-cols-[minmax(116px,1fr)_minmax(96px,0.8fr)_auto]"],
      [knowledgeSourceGovernancePanelStyles.collapsedFormButton, "grid-cols-[auto_minmax(0,1fr)_auto]"],
      [knowledgeSourceGovernancePanelStyles.sourceRecordHeader, "grid-cols-[minmax(0,1fr)_auto]"],
      [knowledgeReviewPanelStyles.queueToolbar, "grid-cols-[repeat(auto-fit,minmax(min(100%,11rem),max-content))]"],
      [knowledgeRagPanelStyles.ragHealthStrip, "grid-cols-[repeat(auto-fit,minmax(108px,1fr))]"],
      [cleanupPanelStyles.cleanupTargetRow, "grid-cols-[18px_minmax(0,1fr)]"],
      [detailPanelStyles.visibilityHeader, "grid-cols-[22px_minmax(0,1fr)]"],
    ];
  ...
  it("wires the team knowledge platform to a dashboard snapshot plus scoped action APIs", () => {
    ...
    expect(workbenchQueriesSource).toContain("fetchKnowledgeDashboardSnapshot<KnowledgeDashboardSnapshotPayload>({");
    expect(workbenchQueriesSource).toContain("recommendationLimit: 6");
    expect(knowledgeApiSource).toContain("/api/knowledge/dashboard-snapshot?");
    ...
    expect(workbenchQueriesSource).toContain("queryKeys.knowledgeSourceInbox(");
    expect(workbenchQueriesSource).toContain("queryKeys.knowledgeCentralSources(");
    expect(workbenchQueriesSource).toContain("listKnowledgeSourceInbox<KnowledgeSourceInboxPayload>({");
    expect(workbenchQueriesSource).toContain("listKnowledgeCentralSources<KnowledgeCentralSourceRegistryPayload>({");
    expect(knowledgeApiSource).toContain("/api/knowledge/sources/inbox");
    expect(knowledgeApiSource).toContain("/api/knowledge/sources/registry");
    expect(knowledgeMutationsSource).toContain("createKnowledgeCentralSourceArtifact<");
    expect(knowledgeApiSource).toContain("/central-source-artifacts");
    ...
    expect(knowledgeMutationsSource).toContain("createKnowledgeRefinementProposal<");
    expect(routeSource).not.toContain("/ingestion-packages");
    expect(knowledgeMutationsSource).toContain("reviewKnowledgeRefinementProposal<");
    expect(knowledgeApiSource).toContain("/refinement-proposals");
    expect(workbenchQueriesSource).toContain("searchKnowledgeItems<KnowledgeSearchPayload>({");
    expect(knowledgeApiSource).toContain("/api/knowledge/search?");
    expect(workbenchQueriesSource).toContain("queryKeys.knowledgeSearch(");
    expect(workbenchQueriesSource).toContain("queryKeys.knowledgeItems(activeKnowledgeBaseForItems, activeKnowledgeActorAgentId)");
    expect(routeSource).toContain("knowledgeSearchQuery");
    expect(routeSource).toContain("knowledgeItemsQuery");
    ...
    expect(workbenchQueriesSource).toContain("queryKeys.knowledgeRagHealth(activeKnowledgeActorAgentId)");
    expect(workbenchQueriesSource).toContain("fetchKnowledgeRagHealth<KnowledgeRagHealthPayload>({");
    expect(knowledgeApiSource).toContain("/api/knowledge/rag/health?");
    expect(workbenchQueriesSource).toContain("queryKeys.knowledgeRagRetrieve");
    expect(workbenchQueriesSource).toContain("retrieveKnowledgeRag<KnowledgeRagRetrievalPayload>({");
    expect(knowledgeApiSource).toContain("/api/knowledge/rag/retrieve?");
    expect(knowledgeApiSource).toContain('params.set("agentId", options.agentId)');
    ...
    expect(routeSource).toContain("knowledgeRagHealthQuery");
    expect(routeSource).toContain("knowledgeRagRetrieveQuery");
    ...
    expect(workbenchQueriesSource).toContain("fetchKnowledgeGovernanceTasks<KnowledgeGovernanceTasksPayload>({");
    expect(knowledgeApiSource).toContain("/api/knowledge/governance/tasks?");
    expect(knowledgeApiSource).toContain("knowledge/governance/tasks?");
    ...
    expect(workbenchQueriesSource).toContain("listKnowledgeIngestionAdapters<KnowledgeIngestionAdaptersPayload>({ signal })");
    expect(knowledgeApiSource).toContain("/api/knowledge/ingestion-adapters");
    expect(workbenchQueriesSource).toContain("fetchKnowledgeTrace<KnowledgeTracePayload>(");
    expect(knowledgeApiSource).toContain("/trace/");
    expect(workbenchQueriesSource).toContain("queryKeys.knowledgeTrace(activeKnowledgeBaseForItems, activeKnowledgeActorAgentId, traceTargetId)");
    expect(workbenchQueriesSource).toContain("listKnowledgeRatingSuggestions<KnowledgeRatingSuggestionsPayload>(");
    expect(knowledgeApiSource).toContain("/rating-suggestions");
    expect(workbenchQueriesSource).toContain("queryKeys.knowledgeRatingSuggestions(");
    expect(knowledgeMutationsSource).toContain("bulkReviewKnowledgeRatingSuggestions<");
    expect(knowledgeApiSource).toContain("/rating-suggestions/review-batch");
    expect(workbenchQueriesSource).toContain("fetchKnowledgePermissionAudit<KnowledgePermissionAuditPayload>({");
    expect(knowledgeApiSource).toContain("/api/knowledge/permissions/audit?agentId=");
    expect(routeSource).toContain("governanceTasksQuery");
    expect(routeSource).toContain("ingestionAdaptersQuery");
    expect(routeSource).toContain("knowledgeTraceQuery");
    expect(routeSource).toContain("ratingSuggestionsQuery");
    expect(routeSource).toContain("permissionAuditQuery");
    ...
    expect(knowledgeReviewPanelStyles.knowledgeFormGrid).toContain("grid-cols-[repeat(auto-fit,minmax(min(100%,11rem),1fr))]");
    expect(knowledgeReviewPanelStyles.knowledgeFormGrid).toContain("[&_label]:min-w-0");
    expect(knowledgeReviewPanelStyles.knowledgeFormGrid).toContain("[&_textarea]:max-w-full");
    expect(knowledgeReviewPanelStyles.queueToolbar).toContain("grid-cols-[repeat(auto-fit,minmax(min(100%,11rem),max-content))]");
    expect(knowledgeReviewPanelStyles.queueToolbar).toContain("justify-start");
    expect(knowledgeReviewPanelStyles.bulkActionBar).toContain("[&>button]:w-fit");
  ...
  it("keeps explanatory Memory platform copy out of persistent paragraphs", () => {
    ...
    // panelLead removed from route shell after Wave 8A prune; form panel still hides helper paragraphs.
    expect(managePanelStyles.manageFormPanel).toContain("[&>p]:hidden");
  });
  ...
  it("keeps team knowledge governance compact and hides internal policy tokens behind readable labels", () => {
    ...
    expect(knowledgeModeTabsStyles.knowledgeModeTabs).toContain("flex flex-wrap");
    expect(knowledgeModeTabsStyles.knowledgeModeTabs).toContain("max-w-full");
    expect(knowledgeModeTabsStyles.knowledgeModeTabs).toContain("overflow-x-hidden");
    expect(knowledgeModeTabsStyles.knowledgeModeTabs).toContain("max-[640px]:grid");
    expect(knowledgeModeTabsStyles.knowledgeModeTabs).not.toContain("grid-cols-[repeat(5,minmax(0,1fr))]");
    expect(knowledgeModeTabsStyles.knowledgeModeTab).toContain("shrink-0");
    expect(knowledgeModeTabsStyles.knowledgeModeTab).toContain("max-w-full");
    expect(knowledgeModeTabsStyles.knowledgeModeTab).not.toContain("inline-flex");
    expect(knowledgeModeTabsStyles.knowledgeModeTab).not.toContain("w-fit");
    ...
    expect(knowledgeStewardPanelStyles.stewardRecommendations).toContain("hidden");
    expect(knowledgeStewardPanelStyles.stewardWorkbench).toContain("hidden");
    expect(knowledgeStewardPanelStyles.stewardMission).toContain("[&_small]:hidden");
    expect(knowledgeStewardPanelStyles.stewardMetric).toContain("[&_small]:hidden");
