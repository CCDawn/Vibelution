const styles = {
  ...
  resetOptionField: "grid [grid-template-columns:auto_minmax(0,_1fr)] [align-items:start] [gap:8px] min-w-0 [min-height:58px] [padding:7px_8px] [border:1px_solid_color-mix(in_srgb,_var(--vui-border-subtle)_86%,_transparent)] [border-radius:var(--radius-control)] [background:color-mix(in_srgb,_var(--vui-surface-row)_58%,_transparent)] [color:var(--fg-secondary)] [&_input]:[margin-top:3px] [&_span]:grid [&_span]:[gap:2px] [&_span]:min-w-0 [&_strong]:min-w-0 [&_strong]:[overflow-wrap:anywhere] [&_small]:min-w-0 [&_small]:[overflow-wrap:anywhere] [&_strong]:[color:var(--fg-primary)] [&_strong]:[font-size:var(--vui-font-xs)] [&_small]:[color:var(--fg-tertiary)] [&_small]:[font-size:var(--vui-font-xs)] [&_small]:[line-height:1.32]",
