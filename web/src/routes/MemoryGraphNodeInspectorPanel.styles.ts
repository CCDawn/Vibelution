...
const styles = {
  ...
  graphRelationPanel: `graphRelationPanel min-w-0 ${vuiFlatPanelClass} p-2`,
  graphResponsibilityPanel: `graphResponsibilityPanel min-w-0 ${vuiFlatPanelClass} p-2`,
  panelEyebrow:
    "panelEyebrow min-w-0 [font-size:var(--vui-font-xs)] leading-tight text-[var(--fg-tertiary)]",
  rawPanel: `rawPanel min-w-0 ${vuiFlatPanelClass} p-2 [&_pre]:whitespace-pre-wrap [&_pre]:break-words`,
  selectedConfigSummary: `selectedConfigSummary min-w-0 ${vuiOpaqueRowClass} bg-[color-mix(in_srgb,var(--accent-cool)_10%,var(--vui-surface-row))] p-2 text-[var(--accent-cool)]`,
} as const;
