...
const styles = {
  ...
  statusOnline: "font-medium text-[var(--state-success)]",
  statusOffline: "font-medium text-[var(--state-warning)]",
  instanceSection: "mt-2 min-w-0",
  sectionHeader: "min-w-0 [&>p]:m-0 [&>p]:mt-0.5 [&>p]:[font-size:var(--vui-font-xs)] [&>p]:text-vui-fg-tertiary",
  sectionHeaderWithPager: "flex min-w-0 flex-wrap items-end justify-between gap-2",
  sectionTitleRow: "flex min-w-0 items-center gap-1.5 [&>h2]:m-0 [&>h2]:[font-size:var(--vui-font-sm)] [&>h2]:font-semibold [&>h2]:text-vui-fg-primary",
  sectionCount: "inline-flex min-w-5 items-center justify-center rounded-full border border-vui-border-subtle bg-vui-surface-muted px-1.5 py-0.5 [font-size:var(--vui-font-2xs)] text-vui-fg-secondary",
  toolbar:
