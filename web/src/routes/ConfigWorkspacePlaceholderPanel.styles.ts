...
const styles = {
  ...
  loadingBoardHeader: "vui-routes-configroute loadingBoardHeader [display:grid] [grid-template-columns:minmax(180px,0.9fr)_minmax(100px,0.32fr)_minmax(100px,0.32fr)] [gap:7px] [&_span]:[display:block] [&_span]:[border-radius:6px] [&_span]:[background:var(--vui-gradient-route-soft)] [&_span]:[min-height:32px]",
  loadingMetricGrid: "vui-routes-configroute loadingMetricGrid [&_strong]:[display:block] [&_strong]:[border-radius:6px] [&_strong]:[background:var(--vui-gradient-route-soft)] [display:grid] [grid-template-columns:repeat(4,minmax(0,1fr))] [gap:7px] [&_span]:[display:grid] [&_span]:[gap:5px] [&_span]:[min-height:54px] [&_span]:[padding:8px] [&_span]:[border:1px_solid_var(--vui-border-subtle)] [&_span]:[border-radius:8px] [&_span]:[background:var(--vui-surface-row)] [&_small]:[color:var(--vui-fg-tertiary)] [&_small]:[font-size:var(--vui-font-xs)] [&_small]:[text-transform:uppercase] [&_strong]:[min-height:16px]",
  loadingNavActive: "vui-routes-configroute loadingNavActive",
  loadingNavList: "vui-routes-configroute loadingNavList [display:grid] [gap:5px] [margin-top:4px] [&_span]:[display:flex] [&_span]:[align-items:center] [&_span]:[min-height:27px] [&_span]:[padding:0_8px] [&_span]:[border:1px_solid_var(--vui-border-subtle)] [&_span]:[border-radius:8px] [&_span]:[background:var(--vui-surface-row)] [&_span]:[color:var(--vui-fg-secondary)] [&_span]:[font-size:var(--vui-font-xs)] [&_.loadingNavActive]:[border-color:color-mix(in_srgb,var(--accent-cool)_32%,transparent)] [&_.loadingNavActive]:[background:color-mix(in_srgb,var(--accent-cool)_10%,var(--vui-surface-row))] [&_.loadingNavActive]:[color:var(--accent-warm-2)]",
  ...
  loadingSpecGrid: "vui-routes-configroute loadingSpecGrid [&_span]:[display:block] [&_span]:[border-radius:6px] [&_span]:[background:var(--vui-gradient-route-soft)] [display:grid] [grid-template-columns:repeat(2,minmax(0,1fr))] [gap:7px] [&_span]:[min-height:76px]",
