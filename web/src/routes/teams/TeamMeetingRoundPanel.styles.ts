const styles: Record<string, string> = {
  ...
  messageCard:
    "grid gap-1.5 rounded-[var(--vui-radius-panel-soft)] border border-[var(--vui-border-subtle)] bg-[var(--vui-surface-card)] p-3 [&_p]:text-sm [&_p]:leading-[1.5]",
  messageMeta:
    "flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-[var(--fg-secondary)]",
  digestGrid: "grid gap-[9px]",
  digestCard:
    "grid gap-1.5 rounded-[var(--vui-radius-panel-soft)] border border-[var(--vui-border-subtle)] bg-[var(--vui-surface-card)] p-3 [&>span]:text-xs [&>span]:font-[650] [&>span]:tracking-[0.02em] [&>span]:text-[var(--fg-secondary)] [&_p]:text-xs [&_p]:leading-[1.5]",
  digestList: "m-0 grid gap-[3px] pl-[18px] text-xs leading-[1.5]",
  decisionList:
    "grid gap-1.5 [&>div]:flex [&>div]:flex-wrap [&>div]:items-center [&>div]:gap-2 [&>div]:rounded-[var(--vui-radius-control)] [&>div]:bg-[var(--vui-surface-inset)] [&>div]:p-2 [&_code]:wrap-anywhere [&_code]:text-xs [&_code]:text-[var(--fg-secondary)]",
