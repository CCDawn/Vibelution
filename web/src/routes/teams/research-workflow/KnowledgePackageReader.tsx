...
export function KnowledgePackageReader({ runId, teamId, handoffId, lang = "zh" }: {
  ...
  return <div className="grid min-w-0 gap-3">
    {packages.map((item) => {
      ...
      return <VSurface key={item.artifactId} tone="panel" className="grid min-w-0 gap-3 p-3 text-sm leading-relaxed break-words">
        {item.status === "unavailable" ? <p role="status">{zh ? "无法核验此交接对应的正文，请检查产物引用。" : "This handoff's content could not be verified. Check its artifact reference."}</p> : <>
          <h5 className="font-semibold">{item.title || (zh ? "未命名知识包草稿" : "Untitled knowledge draft")}</h5>
          <p className="text-[var(--fg-secondary)]">{zh ? "知识包草稿 · 阅读内容不代表已审核或已入库" : "Knowledge draft · Reading does not imply approval or publication"}</p>
          {item.summary ? <section><h6 className="font-semibold">{zh ? "摘要" : "Summary"}</h6><p className="whitespace-pre-wrap">{item.summary}</p></section> : null}
          <section><h6 className="font-semibold">{zh ? "正文" : "Content"}</h6><p className="whitespace-pre-wrap">{item.content || (zh ? "产物中未记录正文。" : "No body text was recorded.")}</p></section>
          {item.riskSummary || item.uncertainties.length ? <section><h6 className="font-semibold">{zh ? "风险与待核查事项" : "Risks and open questions"}</h6><p>{item.riskSummary}</p><ul className="list-disc pl-5">{item.uncertainties.map((text, i) => <li key={i}>{text}</li>)}</ul></section> : null}
