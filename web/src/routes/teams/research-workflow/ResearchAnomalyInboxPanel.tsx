...
import { fetchHypothesisFirstAnomalyInbox } from "../../../api/hypothesisFirst";
            ...
            <section key={group.severity} className={styles.group} data-testid={`anomaly-group-${group.severity}`}>
              ...
              <ul className={styles.list}>
                {group.items.map((item) => {
                  const link = onOpenItem ? anomalyInboxDeepLink(item, normalizedQuestionId) : null;
                  ...
                  return (
                    <li key={`${item.kind}:${scopeText}:${item.lastSeenAt}`} className={styles.row}>
                      {link ? (
                        <VNativeButton
