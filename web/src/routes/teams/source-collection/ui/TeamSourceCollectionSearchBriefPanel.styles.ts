...
const styles = {
  ...
  badge:
    "inline-flex h-6 w-fit shrink-0 items-center rounded-full border border-[color:var(--border-soft)] bg-[color:var(--vui-control-muted)] px-2 text-xs font-[720] text-[var(--fg-secondary)]",
  field:
    "grid min-w-0 gap-1.5 [&>span]:flex [&>span]:items-center [&>span]:justify-between [&>span]:gap-2 [&>span]:text-xs [&>span]:font-[720] [&>span]:text-[var(--fg-secondary)]",
  required:
    "font-[650] text-[var(--state-danger)]",
  topicTextarea:
    "min-h-[6.5rem] w-full resize-y text-sm leading-relaxed",
  fieldHint:
    "flex min-w-0 items-start justify-between gap-2 text-xs leading-snug text-[var(--fg-tertiary)]",
  section:
    "grid min-w-0 gap-2 border-t border-[color:var(--border-soft)] pt-3",
  sectionHeader:
    "flex min-w-0 flex-wrap items-baseline justify-between gap-x-2 gap-y-0.5 text-sm font-[760] text-[var(--fg-primary)] [&_span]:text-xs [&_span]:font-[500] [&_span]:text-[var(--fg-tertiary)]",
  ...
  queryIndex:
    "w-5 shrink-0 select-none text-center text-xs font-[700] tabular-nums text-[var(--fg-tertiary)]",
  queryInputWrap:
    "min-w-0 flex-1",
  /** Kill VNativeInput card chrome inside the list. */
  queryInput:
    "!h-8 !min-h-8 !w-full !min-w-0 !rounded-none !border-0 !bg-transparent !px-1 !py-0 !text-sm !shadow-none !ring-0 hover:!border-0 focus-visible:!border-0 focus-visible:!ring-0 focus-visible:!ring-offset-0",
  removeButton:
  ...
  emptyQueries:
    "px-3 py-3.5 text-center text-xs leading-snug text-[var(--fg-tertiary)]",
  addQuery:
  ...
  advanced:
    "border-t border-[color:var(--border-soft)] pt-1 [&>summary]:cursor-pointer [&>summary]:py-2 [&>summary]:text-xs [&>summary]:font-[720] [&>summary]:text-[var(--fg-secondary)]",
  advancedBody:
  ...
  actionHint:
    "min-w-0 text-xs leading-snug text-[var(--fg-tertiary)]",
} as const;
