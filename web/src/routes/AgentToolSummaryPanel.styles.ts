...
const styles = {
  ...
  policySummaryGrid: "grid [grid-template-columns:repeat(4,_minmax(0,_1fr))] [gap:6px] min-w-0 [overflow-wrap:anywhere] [&_span]:min-w-0 [&_span]:[padding:6px_7px] [&_span]:[border:1px_solid_color-mix(in_srgb,_var(--vui-border-subtle)_86%,_transparent)] [&_span]:[border-radius:var(--radius-control)] [&_span]:!bg-[var(--vui-surface-row)] [&_span]:[color:var(--fg-secondary)] [&_span]:[font-size:var(--vui-font-xs)] [&_strong]:[color:var(--fg-primary)] max-[860px]:[grid-template-columns:1fr]",
