...
const styles = {
  ...
  section: `min-w-0 ${vuiOpaqueRowClass} p-2.5`,
  ...
  runtimeMeta: "mt-3 grid grid-cols-2 gap-2 max-[620px]:grid-cols-1 [&_span]:min-w-0 [&_span]:rounded-[var(--radius-control)] [&_span]:!bg-[var(--vui-surface-base)] [&_span]:px-2 [&_span]:py-1.5 [&_strong]:block [&_strong]:text-[11px] [&_strong]:font-medium [&_strong]:text-[var(--fg-tertiary)] [&_small]:block [&_small]:truncate [&_small]:text-xs [&_small]:text-[var(--fg-primary)]",
  ...
  activityItem: "grid min-w-0 grid-cols-[minmax(0,_1fr)_auto] gap-2 rounded-[var(--radius-control)] border border-[color-mix(in_srgb,var(--vui-border-subtle)_65%,transparent)] !bg-[var(--vui-surface-base)] px-2.5 py-2",
  ...
  state: "grid min-h-[132px] place-items-center rounded-[var(--radius-control)] border border-dashed border-[color-mix(in_srgb,var(--vui-border-subtle)_82%,transparent)] !bg-[var(--vui-surface-base)] p-3 text-center",
