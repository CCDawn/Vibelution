const styles = {
  ...
  reviewQueueTitleLine:
    "reviewQueueTitleLine min-w-0 max-w-full !grid grid-cols-[minmax(0,1fr)_auto] items-start gap-2 text-[var(--vui-font-sm)] font-semibold leading-snug text-[var(--fg-primary)] [&_strong]:min-w-0 [&_strong]:[overflow-wrap:anywhere] [&_strong]:[word-break:break-word] [&_span]:min-w-0 [&_span]:max-w-[12rem] [&_span]:truncate [&_span]:rounded-full [&_span]:border [&_span]:border-[var(--vui-border-subtle)] [&_span]:bg-[var(--vui-control-muted)] [&_span]:px-2 [&_span]:py-0.5 [&_span]:text-[var(--vui-font-xs)] [&_span]:font-semibold [&_span]:text-[var(--fg-secondary)]",
  reviewRank:
