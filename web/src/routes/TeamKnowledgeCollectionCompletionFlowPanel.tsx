...
export type TeamKnowledgeCollectionCompletionFlowPanelProps = {
  ...
  /**
   * board = full-width strip under canvas (legacy)
   * rail = compact 2×2 beside the next-step CTA (end-user home)
   */
  presentation?: "board" | "rail";
...
export function TeamKnowledgeCollectionCompletionFlowPanel(props: TeamKnowledgeCollectionCompletionFlowPanelProps) {
  ...
  return (
    <section
      className={[
        styles.knowledgeCompletionFlowPanel,
        rail ? styles.knowledgeCompletionFlowPanelRail : "",
      ].filter(Boolean).join(" ")}
      aria-label={lang === "zh" ? "一键流程图" : "One-click flow graph"}
    ...
    >
      ...
      <div
        className={[
          styles.knowledgeCompletionFlowNodes,
          rail ? styles.knowledgeCompletionFlowNodesRail : "",
        ].filter(Boolean).join(" ")}
      >
          ...
          return (
            <article
              ...
              className={[
                ...
                rail ? styles.knowledgeCompletionFlowNodeRail : "",
            ...
            >
              ...
              <div className={styles.knowledgeCompletionFlowNodeBody}>
                ...
                <small className={rail ? "line-clamp-1" : undefined}>
                  {lang === "zh" ? `Agent：${agentLabel}` : `Agent: ${agentLabel}`}
                </small>
                <em>
                ...
                {!rail && node.detail ? <p>{node.detail}</p> : null}
                {rail && node.detail ? (
                  <p className="line-clamp-2 m-0">{node.detail}</p>
                ) : null}
