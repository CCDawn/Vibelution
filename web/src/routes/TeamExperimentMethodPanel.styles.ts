...
const styles = {
  ...
  header:
    "min-w-0 flex flex-wrap items-start justify-between gap-2 [&>div]:grid [&>div]:min-w-0 [&>div]:gap-0.5 [&_strong]:[font-size:var(--vui-font-sm)] [&_span]:[font-size:var(--vui-font-xs)] [&_span]:leading-[var(--vui-line-readable)] [&_span]:text-[var(--fg-secondary)]",
  sourceBadge:
  ...
  adapterChoices:
    "min-w-0 grid grid-cols-[repeat(2,minmax(0,1fr))] gap-1 max-[720px]:grid-cols-[minmax(0,1fr)]",
  adapterChoice: `min-h-10 min-w-0 w-full ${vuiOpaqueRowClass} !justify-start !whitespace-normal break-all px-2.5 py-2 text-left [font-size:var(--vui-font-xs)] font-semibold leading-tight text-[var(--fg-secondary)] hover:border-[color-mix(in_srgb,var(--accent-cool)_34%,var(--vui-border-subtle))] focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[var(--accent-cool)] disabled:cursor-not-allowed disabled:opacity-55`,
  adapterChoiceActive:
    "border-[color-mix(in_srgb,var(--accent-cool)_42%,var(--vui-border-subtle))] bg-[color-mix(in_srgb,var(--accent-cool)_10%,var(--vui-surface-row))] text-[var(--fg-primary)] shadow-[var(--vui-shadow-inset-accent)]",
  ...
  actions:
    "min-w-0 flex flex-wrap items-center justify-between gap-2 border-t border-[var(--vui-border-subtle)] pt-2 [&>span]:min-w-0 [&>span]:[font-size:var(--vui-font-xs)] [&>span]:leading-[var(--vui-line-readable)] [&>span]:text-[var(--fg-tertiary)]",
  primaryAction:
    "min-h-9 rounded-[var(--radius-control)] border border-[color-mix(in_srgb,var(--accent-cool)_36%,var(--vui-border-subtle))] bg-[color-mix(in_srgb,var(--accent-cool)_12%,var(--vui-surface-panel))] px-3 [font-size:var(--vui-font-sm)] font-semibold text-[var(--fg-primary)] hover:bg-[color-mix(in_srgb,var(--accent-cool)_18%,var(--vui-surface-panel))] focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[var(--accent-cool)] disabled:cursor-not-allowed disabled:opacity-55",
  loading:
  ...
  error:
    "min-w-0 rounded-[var(--radius-control)] border border-[color-mix(in_srgb,var(--state-error)_36%,transparent)] bg-[color-mix(in_srgb,var(--state-error)_8%,transparent)] px-2.5 py-2 [font-size:var(--vui-font-xs)] leading-[var(--vui-line-readable)] text-[var(--state-error)]",
} as const;
