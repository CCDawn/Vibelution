...
const styles = {
  ...
  detailHeaderRegion:
    "grid min-w-0 [align-content:start] [gap:8px]",
  overviewLayout:
  ...
  paneContent:
    "grid min-h-0 min-w-0 [align-content:start] [gap:8px] [overflow:auto] [overscroll-behavior:contain]",
  configSectionNav: `inline-grid w-fit max-w-full [grid-auto-flow:column] [grid-auto-columns:minmax(88px,_max-content)] [gap:3px] min-w-0 [padding:3px] ${vuiOpaqueRowClass} max-[860px]:grid max-[860px]:w-full max-[860px]:[grid-auto-flow:row] max-[860px]:[grid-auto-columns:auto] max-[860px]:[grid-template-columns:repeat(2,_minmax(0,_1fr))]`,
  configSectionTab:
