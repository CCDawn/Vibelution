const styles = {
  ...
  emptyRail:
    "grid h-full min-h-0 place-content-center place-items-center gap-2 border border-dashed border-[color-mix(in_srgb,var(--vui-border-subtle)_80%,transparent)] bg-[var(--vui-surface-inset)] p-4 text-center text-[var(--fg-tertiary)] [&_strong]:[font-size:var(--vui-font-xs)] [&_strong]:text-[var(--fg-secondary)] [&_span]:[font-size:var(--vui-font-xs)] [&_span]:leading-[1.4]",
} as const;
