...
describe("runtimeScenePackageSections", () => {
  it("groups runtime scene child logs by user-facing purpose", () => {
    ...
    expect(Object.fromEntries(sections.map((section) => [section.id, section.files.map((item) => item.path)]))).toEqual({
      conversations: ["conversations/session-demo.jsonl"],
      supervised: ["agent/supervised_runs/web-supervised-demo.jsonl", "artifacts/supervised/evidence.json"],
      selfEvolution: ["agent/self_evolution_runs/web-self-demo.jsonl", "artifacts/self_evolution/rollback.json"],
      agent: ["agent/tool_calls.jsonl"],
