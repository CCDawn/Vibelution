...
- **F4b (Wave 1-A cont.):** `useChatVisibleSessionCatalog` owns visible session merge/map + activity seen; `useChatAgentSessionTabs` owns per-agent tab query; `useChatSessionIndexRailModel` owns group-only index groups. The workbench auto-loads every session-index page in the background (no load-more control).
...
- **F4d (Wave 1-A alignment):** catalog/index pure boundaries — `chatVisibleSessionCatalogModel.ts`, `chatSessionIndexRailPresentation.ts`; hook exports trimmed to workbench-used fields.
