...
const styles: Record<string, string> = {
  ...
  cacheDetailSegmentMeta:
    "vui-routes-chatcodingroute cacheDetailSegmentMeta min-w-0 flex flex-wrap items-center gap-1 [font-size:var(--vui-font-xs)] leading-tight [&_b]:rounded-full [&_b]:border [&_b]:border-[var(--vui-border-subtle)] [&_b]:bg-[color-mix(in_srgb,var(--vui-surface-panel)_80%,transparent)] [&_b]:px-1.5 [&_b]:py-0.5 [&_b]:font-medium [&_b]:text-[var(--fg-secondary)] [&_b[data-status=observed\\_hit]]:border-[color-mix(in_srgb,var(--state-success)_34%,var(--vui-border-subtle))] [&_b[data-status=observed\\_hit]]:text-[var(--state-success)] [&_b[data-status=observed\\_miss]]:border-[color-mix(in_srgb,var(--state-warning)_34%,var(--vui-border-subtle))] [&_b[data-status=observed\\_miss]]:text-[var(--state-warning)] [&_b[data-status=observed\\_partial]]:text-[var(--state-warning)] [&_b[data-status=not\\_observed]]:text-[var(--fg-tertiary)]",
