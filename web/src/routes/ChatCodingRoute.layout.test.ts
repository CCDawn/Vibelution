...
describe("ChatCodingRoute layout contract", () => {
  ...
  it("centers the direct conversation reading track when both side panes are collapsed", () => {
    ...
    expect(chatSessionWorkspacePanelStyles.conversationFrameFocus).toContain("justify-self-center");
  ...
  it("keeps the left rail status stack soft and non-nested", () => {
    ...
    expect(routeStyles.contextLineCompact).toContain("bg-[var(--vui-surface-row)]");
  ...
  it("presents previous-turn token state as compact visual metrics with hover details", () => {
    ...
    expect(routeSource.indexOf("<TokenCoreStatusPanel")).toBeGreaterThan(routeSource.indexOf("styles.runModeBlock"));
    ...
    expect(routeStyles.leftRail).toContain("[grid-column:5]");
    expect(routeSource.indexOf("<TokenCoreStatusPanel")).toBeGreaterThan(routeSource.indexOf("<aside className={statusRailCollapsed"));
  ...
  it("shows the active skill contract before the context status cards", () => {
    ...
    expect(renderedActiveSkillIndex).toBeLessThan(routeSource.indexOf("<TokenCoreStatusPanel"));
  ...
  it("shows direct-session mismatch as a status-strip notice with a switch action", () => {
    ...
    expect(routeSource.indexOf("label: t(\"sessionBinding\")")).toBeLessThan(
      routeSource.indexOf("<TokenCoreStatusPanel"),
    );
    expect(routeSource).not.toContain("label: t(\"currentTask\")");
  });
  ...
  it("opens group conversations inside the chat page instead of navigating away", () => {
    ...
    expect(routeStyles.leftRail).toContain("[grid-column:5]");
    expect(routeSource.indexOf("styles.groupManagementPanel")).toBeGreaterThan(
      routeSource.indexOf("<aside className={statusRailCollapsed"),
    );
  ...
  it("uses one lightweight Tailwind grammar for conversation index sections and rows", () => {
    ...
    expect(directSessionIndexItemStyles.sessionItem).toContain("[border:0]");
    ...
    expect(directSessionIndexItemStyles.conversationAvatar).toContain("h-[30px]");
    expect(directSessionIndexItemStyles.sessionItemMain).toContain("min-h-[46px]");
    expect(directSessionIndexItemStyles.sessionItemMain).toContain("grid-cols-[30px_minmax(0,1fr)]");
    ...
    expect(groupSessionIndexItemsStyles.sessionItem).toContain("[border:0]");
    expect(groupSessionIndexItemsStyles.sessionItem).toContain("before:absolute");
    expect(groupSessionIndexItemsStyles.conversationAvatar).toContain("h-[30px]");
    expect(groupSessionIndexItemsStyles.sessionItemMain).toContain("min-h-[46px]");
    expect(groupSessionIndexItemsStyles.sessionItemMain).toContain("grid-cols-[30px_minmax(0,1fr)]");
