const styles = {
  ...
  detailAvatar: "grid [place-items:center] [flex:0_0_auto] [border-radius:50%] [color:var(--fg-primary)] [background:color-mix(in_srgb,_var(--accent-cool)_10%,_var(--surface-card))] [font-family:var(--font-display)] [font-weight:800] [overflow:hidden] [width:46px] [height:46px] [font-size:var(--vui-font-xs)] [box-shadow:var(--focus-ring)]",
  agentAvatarImage: "block [width:100%] [height:100%] [border-radius:inherit] [object-fit:cover]",
  avatarEditorPanel: "[position:absolute] [top:52px] [left:0] [z-index:5] grid [gap:9px] [width:min(320px,_82vw)] [padding:10px] [border:1px_solid_color-mix(in_srgb,_var(--accent-cool)_26%,_var(--border-soft))] [border-radius:8px] [background:var(--surface-panel)] [box-shadow:var(--shadow-soft)] [&_p]:[margin:0] [&_p]:[color:var(--fg-secondary)] [&_p]:[font-size:var(--vui-font-xs)] [&_p]:[line-height:1.4]",
  ...
  iconButton: "grid w-[26px] h-[26px] [place-items:center] [width:26px] [height:26px] [border:1px_solid_var(--border-soft)] [border-radius:var(--radius-control)] [background:var(--surface-card)] [color:var(--fg-secondary)] [font:inherit] [font-size:1rem] [cursor:pointer]",
