...
const styles = {
  ...
  member: "inline-grid max-w-full [grid-template-columns:minmax(0,1fr)] [padding:5px_7px] [border:1px_solid_color-mix(in_srgb,var(--vui-border-subtle)_76%,transparent)] [border-radius:var(--radius-control)] [background:var(--vui-surface-row)] focus-visible:[outline:2px_solid_var(--accent-cool)] focus-visible:[outline-offset:2px] [&_strong]:min-w-0 [&_strong]:truncate [&_strong]:text-[var(--fg-secondary)] [&_strong]:[font-size:var(--vui-font-xs)]",
