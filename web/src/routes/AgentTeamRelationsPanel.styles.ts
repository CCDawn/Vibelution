...
const styles = {
  ...
  member: "inline-grid max-w-full [grid-template-columns:minmax(0,1fr)] [gap:1px] [padding:5px_7px] [border:1px_solid_color-mix(in_srgb,var(--vui-border-subtle)_76%,transparent)] [border-radius:var(--radius-control)] [background:color-mix(in_srgb,var(--vui-surface-row)_86%,transparent)] [&_strong]:min-w-0 [&_strong]:truncate [&_strong]:text-[var(--fg-secondary)] [&_strong]:[font-size:var(--vui-font-xs)] [&_small]:min-w-0 [&_small]:truncate [&_small]:text-[var(--fg-tertiary)] [&_small]:[font-size:var(--vui-font-xs)]",
