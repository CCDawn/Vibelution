const styles = {
  ...
  sessionContextMenuItem:
    "vui-routes-chatcodingroute sessionContextMenuItem min-w-0 !w-full justify-start rounded-[var(--radius-control)] border border-[var(--vui-border-subtle)] bg-[var(--vui-surface-row)] px-2 py-1.5 text-left text-[var(--fg-secondary)] hover:border-[var(--border-strong)] hover:bg-[var(--vui-control-muted-hover)] hover:text-[var(--fg-primary)] [&_[data-slot=vui-button-content]]:grid [&_[data-slot=vui-button-content]]:w-full [&_[data-slot=vui-button-content]]:grid-cols-[auto_minmax(0,1fr)] [&_[data-slot=vui-button-content]]:items-center [&_[data-slot=vui-button-content]]:justify-start [&_[data-slot=vui-button-icon]]:self-center [&_[data-slot=vui-button-label]]:block [&_[data-slot=vui-button-label]]:min-w-0 [&_[data-slot=vui-button-label]]:truncate [&_[data-slot=vui-button-label]]:text-left",
} as const;
